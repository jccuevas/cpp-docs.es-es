---
title: "Pragmas | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: ""
ms.topic: "article"
dev_langs: 
  - "C++"
  - "C"
helpviewer_keywords: 
  - "pragma (directivas), acerca de las directivas pragma"
ms.assetid: 3ce8aced-53ee-4524-87d0-f998d6977ff5
caps.latest.revision: 9
caps.handback.revision: 9
author: "mikeblome"
ms.author: "mblome"
manager: "ghogen"
---
# Pragmas
[!INCLUDE[vs2017banner](../assembler/inline/includes/vs2017banner.md)]

**ANSI 3.8.6** El comportamiento de cada directiva \#pragma reconocida.  
  
 Los siguientes [Pragmas de C](../c-language/c-pragmas.md) se definen para el compilador de C de Microsoft:  
  
|||||  
|-|-|-|-|  
|**alloc\_text**|**data\_seg**|**include\_alias**|`setlocale`|  
|**auto\_inline**|**function**|**intrinsic**|**warning**|  
|**check\_stack**|**hdrstop**|**message**||  
|**code\_seg**|**inline\_depth**|**optimize**||  
|**comment**|**inline\_recursion**|`pack`||  
  
## Vea también  
 [Preprocesar directivas](../c-language/preprocessing-directives.md)