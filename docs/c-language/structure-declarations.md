 ...
 *struct\-declarator*:  
 ...
 Cada *struct\-declaration* de una *struct\-declaration\-list* debe ser única dentro de la lista.  Sin embargo, los nombres de identificador de la *struct\-declaration\-list* no tienen que ser distintos de los nombres de variable normales o de los identificadores en otras listas de declaraciones de estructura.  
...
```  
 ...
 La estructura `employee` tiene tres miembros: `name`, `id` y `class`.  El miembro `name` es una matriz de 20 elementos, e `id` y `class` son miembros simples con el tipo `int` y **long**, respectivamente.  El identificador `employee` es el identificador de la estructura.  
...
```  
 ...
 Vea [Miembros de estructura y unión](../c-language/structure-and-union-members.md) para obtener información sobre las referencias de estructura.  
