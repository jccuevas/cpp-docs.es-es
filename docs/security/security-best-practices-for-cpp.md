---
title: Prácticas recomendadas de seguridad para C++ | Documentos de Microsoft
ms.custom: ''
ms.date: 05/08/2018
ms.technology:
- cpp-windows
...
## <a name="safeint-library"></a>Biblioteca SafeInt  
 [Biblioteca SafeInt](../windows/safeint-library.md) ayuda a evita los desbordamientos de enteros y otros errores explotables que pueden producirse cuando la aplicación realiza operaciones matemáticas. El `SafeInt` biblioteca incluye la [SafeInt (clase)](../windows/safeint-class.md), [SafeIntException (clase)](../windows/safeintexception-class.md)y varios [SafeInt (funciones)](../windows/safeint-functions.md).  
...
## <a name="checked-iterators"></a>Iteradores comprobados  
 Un iterador comprobado aplica límites de contenedor. De forma predeterminada, cuando un iterador comprobado está fuera de los límites, genera una excepción y finaliza la ejecución de programas. Un iterador comprobado proporciona otros niveles de respuesta que dependen de valores asignados al preprocesador se define como  **\_seguro\_SCL\_produce** y  **\_ITERADOR\_depurar\_nivel**. Por ejemplo, en  **\_ITERADOR\_depurar\_LEVEL = 2**, un iterador comprobado proporciona comprobaciones de corrección completa en modo de depuración, que se pone a disposición mediante el uso de aserciones. Para obtener más información, consulte [iteradores comprobados](../standard-library/checked-iterators.md) y [ \_ITERADOR\_depurar\_nivel](../standard-library/iterator-debug-level.md).  
...
## <a name="guidance-for-speculative-execution-side-channels"></a>Guía para los canales de lado de ejecución especulativa
