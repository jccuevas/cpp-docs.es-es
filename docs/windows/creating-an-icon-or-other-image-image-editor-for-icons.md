...
Puede crear una nueva imagen, mapa de bits, icono, cursor o barra de herramientas y, a continuación, utilice el **Editor de imágenes** para personalizar su apariencia. También puede crear un nuevo mapa de bits crearse un [plantilla de recursos](../windows/how-to-use-resource-templates.md).
...
Los iconos y cursores son recursos gráficos que pueden contener varias imágenes de diferentes tamaños y combinaciones de colores para distintos tipos de dispositivos de pantalla. Un cursor tiene también una zona activa, la ubicación Windows se usa para realizar un seguimiento de su posición. Iconos y cursores se crean y editan mediante el **Editor de imágenes**, como son mapas de bits y otras imágenes.

Cuando se crea un nuevo icono o cursor, la **Editor de imágenes** crea primero una imagen de un tipo estándar. La imagen se rellena inicialmente con el color de la pantalla (transparente). Si la imagen es un cursor, la zona activa es inicialmente la esquina superior izquierda con coordenadas `0,0`.

De forma predeterminada, el **Editor de imágenes** admite la creación de imágenes adicionales para los dispositivos que se muestra en la tabla siguiente. Puede crear imágenes para otros dispositivos escribiendo parámetros de ancho, alto y número de colores en el **Custom Image** cuadro de diálogo.
...
El **New &lt;dispositivo&gt; tipo de imagen** cuadro de diálogo le permite crear una nueva imagen de dispositivo de un tipo especificado. Para abrir el **New \<dispositivo > imagen** cuadro de diálogo, vaya al menú **imagen** > **nuevo tipo de imagen**. Son las siguientes propiedades incluidas **tipo de imagen de destino** y **personalizado**.

El **tipo de imagen de destino** listas de propiedades de los tipos de imagen disponible donde selecciona la imagen de tipo que desea abrir:
...
1. Vaya al menú **imagen** > **nueva imagen de dispositivo**, o haga clic en el **Editor de imágenes** panel y elija **nueva imagen de dispositivo**.
...
1. Vaya al menú **imagen** > **Abrir imagen de dispositivo** y elija una imagen de la lista de imágenes actual. Por ejemplo, elija la versión de 32 × 32, 16 colores de un icono.
...
Mientras que la imagen del icono se muestra en el **Editor de imágenes**, vaya al menú **imagen** > **Eliminar imagen de dispositivo**. Cuando se elimina la última imagen de icono en el recurso, también se elimina el recurso.

> [!NOTE]
> Cuando presiona el **SUPR** clave, las imágenes y los colores que se haya dibujado en un icono se eliminan, pero sigue siendo el icono y puede volver a diseñar, ahora. Si presiona **SUPR** por error, presione **Ctrl**+**Z** deshacer la acción.
...
1. En el **colores** ventana, elija el selector **Color de pantalla** o **Color inverso**.
...
Mediante el **Editor de imágenes**, iconos y cursores pueden provenir tamaño grande (64 x 64) con una paleta de 256 colores para elegir. Después de crear el recurso, se selecciona un estilo de imagen de dispositivo.
...
1. Vaya al menú **imagen** > **nueva imagen de dispositivo** y seleccione el estilo de imagen de 256 colores que desee.
...
La zona activa de un cursor es el punto en que Windows hace referencia en el seguimiento de la posición del cursor. De forma predeterminada, se establece la zona activa en la esquina superior izquierda del cursor con coordenadas `0,0`. El **Hotspot** propiedad en el [ventana propiedades](/visualstudio/ide/reference/properties-window) muestra las coordenadas de la zona activa.
...
1. Vaya al menú **archivo** > **abierto**, a continuación, seleccione **archivo**.
...
1. Con el mapa de bits sigue abierto en el **Editor de imágenes**, vaya al menú **archivo** > **guardar *filename*.bmp como**.
