# [Información general de la programación para Windows en C++](overview-of-windows-programming-in-cpp.md)
# [Aplicaciones de escritorio de Windows en C++](desktop-applications-visual-cpp.md)
## [Aplicaciones de consola de Windows en C++](console-applications-in-visual-cpp.md)
### [Tutorial: Crear un programa de C++ estándar (C++)](walkthrough-creating-a-standard-cpp-program-cpp.md)
### [Creación de una aplicación de consola](creating-a-console-application.md)
### [Asistente para aplicaciones Win32](win32-application-wizard.md)
#### [Configuración de la aplicación, Asistente para proyectos Win32](application-settings-win-32-project-wizard.md)
## [Tutorial: Crear aplicaciones de escritorio de Windows (C++)](walkthrough-creating-windows-desktop-applications-cpp.md)
## [Creación de una aplicación de escritorio de Windows vacía](creating-an-empty-windows-desktop-application.md)
## [Adición de archivos a una aplicación de Win32 vacía](adding-files-to-an-empty-win32-applications.md)
## [Trabajo con archivos de recursos](working-with-resource-files.md)
### [Archivos de recursos (Visual Studio)](resource-files-visual-studio.md)
#### [Cómo: Crear un nuevo archivo de script de recursos](how-to-create-a-resource-script-file.md)
#### [Cómo: Crear un recurso](how-to-create-a-resource.md)
##### [Vista de recursos (ventana)](resource-view-window.md)
##### [Agregar recurso (cuadro de diálogo)](add-resource-dialog-box.md)
##### [Nuevo recurso personalizado (cuadro de diálogo)](new-custom-resource-dialog-box.md)
#### [Cómo: Abrir un archivo de script de recursos fuera de un proyecto (independiente)](how-to-open-a-resource-script-file-outside-of-a-project-standalone.md)
#### [Cómo: Abrir un archivo de script de recursos en formato de texto](how-to-open-a-resource-script-file-in-text-format.md)
#### [Cómo: Agregar compatibilidad con MFC a los archivos de script de recursos](how-to-add-mfc-support-to-resource-script-files.md)
#### [Cómo: Incluir recursos en tiempo de compilación](how-to-include-resources-at-compile-time.md)
##### [Archivos de inclusión de recursos (cuadro de diálogo)](resource-includes-dialog-box.md)
##### [Cómo: Especificar directorios de inclusión para los recursos](how-to-specify-include-directories-for-resources.md)
#### [Cómo: Copiar recursos](how-to-copy-resources.md)
##### [Cómo: Cambiar el idioma o la condición de un recurso al copiarlo](how-to-change-the-language-or-condition-of-a-resource-while-copying.md)
#### [Cómo: Buscar símbolos en recursos](how-to-search-for-symbols-in-resources.md)
#### [Cómo: Usar plantillas de recursos](how-to-use-resource-templates.md)
#### [Cómo: Importar y exportar recursos](how-to-import-and-export-resources.md)
#### [Tipos de archivos editables para recursos](editable-file-types-for-resources.md)
#### [Edición de archivos de recursos administrados](editing-managed-resource-files.md)
#### [Archivos afectados al editar recursos](files-affected-by-resource-editing.md)
#### [Recursos del manifiesto](manifest-resources.md)
##### [Cómo: Abrir un recurso del manifiesto](how-to-open-a-manifest-resource.md)
### [Símbolos: Identificadores de recursos](symbols-resource-identifiers.md)
#### [Visualización de símbolos de recursos](viewing-resource-symbols.md)
##### [Símbolos de recursos (cuadro de diálogo)](resource-symbols-dialog-box.md)
#### [Creación de nuevos símbolos](creating-new-symbols.md)
#### [Cambio de símbolos sin asignar](changing-unassigned-symbols.md)
#### [Eliminación de símbolos sin asignar](deleting-unassigned-symbols.md)
#### [Apertura del Editor de recursos para un símbolo determinado](opening-the-resource-editor-for-a-given-symbol.md)
#### [Cambio de un símbolo o el nombre de un símbolo (ID)](changing-a-symbol-or-symbol-name-id.md)
##### [Restricciones de los nombres de símbolo](symbol-name-restrictions.md)
#### [Cambio del valor numérico de un símbolo](changing-a-symbol-s-numeric-value.md)
##### [Restricciones de los valores de símbolo](symbol-value-restrictions.md)
#### [Cambio de los nombres de los archivos de encabezado de símbolos](changing-the-names-of-symbol-header-files.md)
#### [Inclusión de símbolos compartidos (de solo lectura) o calculados](including-shared-read-only-or-calculated-symbols.md)
#### [Identificadores de símbolo predefinidos](predefined-symbol-ids.md)
##### [Símbolos predefinidos de ATL](atl-predefined-symbols.md)
##### [Símbolos predefinidos de MFC](mfc-predefined-symbols.md)
###### [Símbolos de la programación de bases de datos de MFC](mfc-database-programming-symbols.md)
###### [Símbolos de la programación para Windows de MFC](mfc-windows-programming-symbols.md)
##### [Símbolos predefinidos de Win32](win32-predefined-symbols.md)
### [Editores de recursos](resource-editors.md)
#### [Visualización y edición de recursos en un editor de recursos](viewing-and-editing-resources-in-a-resource-editor.md)
#### [Editor de aceleradores](accelerator-editor.md)
##### [Establecimiento de las propiedades de los aceleradores](setting-accelerator-properties.md)
##### [Edición de tablas de aceleradores](editing-accelerator-tables.md)
##### [Teclas de aceleración](predefined-accelerator-keys.md)
#### [Binary Editor](binary-editor.md)
#### [Editor de cuadros de diálogo](dialog-editor.md)
##### [Creación de un cuadro de diálogo](creating-a-new-dialog-box.md)
##### [Mostrar u ocultar la barra de herramientas del Editor de cuadros de diálogo](showing-or-hiding-the-dialog-editor-toolbar.md)
##### [Alternancia entre los controles de un cuadro de diálogo y el código](switching-between-dialog-box-controls-and-code.md)
##### [Controles de cuadros de diálogo](controls-in-dialog-boxes.md)
###### [Adición, edición o eliminación de controles](adding-editing-or-deleting-controls.md)
###### [Selección de controles](selecting-controls.md)
###### [Ajuste del tamaño de los controles](sizing-individual-controls.md)
###### [Adición de valores a un control de cuadro combinado](adding-values-to-a-combo-box-control.md)
###### [Distribución de los controles en los cuadros de diálogo](arrangement-of-controls-on-dialog-boxes.md)
####### [Estados del Editor de cuadros de diálogo (guías y cuadrículas)](dialog-editor-states-guides-and-grids.md)
####### [Agrupación de botones de opción en un cuadro de diálogo](grouping-radio-buttons-on-a-dialog-box.md)
####### [Alinear controles](aligning-groups-of-controls.md)
####### [Cambio del orden de tabulación de los controles](changing-the-tab-order-of-controls.md)
###### [Controles personalizados en el Editor de cuadros de diálogo](custom-controls-in-the-dialog-editor.md)
###### [Definición de teclas de acceso](defining-mnemonics-access-keys.md)
###### [Diseño de un cuadro de diálogo](testing-a-dialog-box.md)
##### [Teclas de aceleración del Editor de cuadros de diálogo](accelerator-keys-for-the-dialog-editor.md)
##### [Solucionar problemas del Editor de cuadros de diálogo](troubleshooting-the-dialog-editor.md)
##### [Definición de variables miembro para controles de cuadro de diálogo](defining-member-variables-for-dialog-controls.md)
#### [Editor de imágenes para iconos](image-editor-for-icons.md)
##### [Edición de recursos gráficos (Editor de imágenes para iconos)](editing-graphical-resources-image-editor-for-icons.md)
###### [Creación de un icono u otra imagen (Editor de imágenes para iconos)](creating-an-icon-or-other-image-image-editor-for-icons.md)
###### [Uso de una herramienta de dibujo (Editor de imágenes para iconos)](using-a-drawing-tool-image-editor-for-icons.md)
###### [Dibujo de líneas o figuras cerradas (Editor de imágenes para iconos)](drawing-lines-or-closed-figures-image-editor-for-icons.md)
###### [Selección de un área de una imagen (Editor de imágenes para iconos)](selecting-an-area-of-an-image-image-editor-for-icons.md)
###### [Edición de partes de una imagen (Editor de imágenes para iconos)](editing-parts-of-an-image-image-editor-for-icons.md)
###### [Volteo de una imagen (Editor de imágenes para iconos)](flipping-an-image-image-editor-for-icons.md)
###### [Cambio del tamaño de una imagen (Editor de imágenes para iconos)](resizing-an-image-image-editor-for-icons.md)
###### [Creación de un pincel personalizado (Editor de imágenes para iconos)](creating-a-custom-brush-image-editor-for-icons.md)
###### [Cambio de las propiedades de la imagen (Editor de imágenes para iconos)](changing-image-properties-image-editor-for-icons.md)
###### [Guardado de un mapa de bits con formato GIF o JPEG (Editar imágenes para iconos)](saving-bitmaps-as-gifs-or-jpegs-image-editor-for-icons.md)
###### [Conversión de una imagen de un formato en otro (Editor de imágenes para iconos)](converting-an-image-from-one-format-to-another-image-editor-for-icons.md)
###### [Edición de una imagen fuera de un proyecto (Editor de imágenes para iconos)](editing-an-image-outside-of-a-project-image-editor-for-icons.md)
###### [Imagen (menú) (Editor de imágenes para iconos)](image-menu-image-editor-for-icons.md)
####### [Configuración de la cuadrícula (cuadro de diálogo) (Editor de imágenes para iconos)](grid-settings-dialog-box-image-editor-for-icons.md)
###### [Barra de herramientas (Editor de imágenes para iconos)](toolbar-image-editor-for-icons.md)
###### [Paneles de la ventana (Editor de imágenes para iconos)](window-panes-image-editor-for-icons.md)
##### [Trabajo con colores (Editor de imágenes para iconos)](working-with-color-image-editor-for-icons.md)
###### [Selección del color de primer plano y el color de fondo (Editor de imágenes para iconos)](selecting-foreground-or-background-colors-image-editor-for-icons.md)
###### [Relleno de un área delimitada de una imagen con un color (Editor de imágenes para iconos)](filling-a-bounded-area-of-an-image-with-a-color-image-editor-for-icons.md)
###### [Selección de un color de una imagen para usarlo en otro sitio (Editor de imágenes para iconos)](picking-up-a-color-from-an-image-to-use-elsewhere-image-editor-for-icons.md)
###### [Selección de un fondo transparente u opaco (Editor de imágenes para iconos)](choosing-a-transparent-or-opaque-background-image-editor-for-icons.md)
###### [Inversión de los colores de una selección (Editor de imágenes para iconos)](inverting-the-colors-in-a-selection-image-editor-for-icons.md)
###### [Personalización o cambio de colores (Editor de imágenes para iconos)](customizing-or-changing-colors-image-editor-for-icons.md)
###### [Guardado y carga de diferentes paletas de colores (Editor de imágenes para iconos)](saving-and-loading-different-color-palettes-image-editor-for-icons.md)
###### [Colores (ventana) (Editor de imágenes para iconos)](colors-window-image-editor-for-icons.md)
##### [Iconos y cursores: Recursos de imagen para dispositivos de presentación (Editor de imágenes para iconos)](icons-and-cursors-image-resources-for-display-devices-image-editor-for-icons.md)
###### [Creación de una imagen de dispositivo (Editor de imágenes para iconos)](creating-a-device-image-image-editor-for-icons.md)
###### [Adición de una imagen para otro dispositivo de presentación (Editar imágenes para iconos)](adding-an-image-for-a-different-display-device-image-editor-for-icons.md)
###### [Copia de una imagen de dispositivo (Editor de imágenes para iconos)](copying-a-device-image-image-editor-for-icons.md)
###### [Eliminación de una imagen de dispositivo (Editor de imágenes para iconos)](deleting-a-device-image-image-editor-for-icons.md)
###### [Creación de zonas transparentes o inversas en las imágenes de dispositivo (Editor de imágenes para iconos)](creating-transparent-or-inverse-regions-in-device-images.md)
###### [Uso de la paleta de 256 colores (Editor de imágenes para iconos)](creating-a-256-color-icon-or-cursor-image-editor-for-icons.md)
###### [Establecimiento de la zona activa del cursor (Editor de imágenes para iconos)](setting-a-cursor-s-hot-spot-image-editor-for-icons.md)
##### [Teclas de aceleración (Editor de imágenes para iconos)](accelerator-keys-image-editor-for-icons.md)
#### [Editor de menús](menu-editor.md)
##### [Creación de un menú](creating-a-menu.md)
##### [Creación de un submenú](creating-a-submenu.md)
##### [Creación de menús emergentes](creating-pop-up-menus.md)
##### [Edición de varios menús o comandos de menú](selecting-multiple-menus-or-menu-commands.md)
##### [Asociación de comandos de menú](associating-a-menu-command-with-an-accelerator-key.md)
##### [Propiedades de los comandos de menú](menu-command-properties.md)
#### [Editor de cadenas](string-editor.md)
#### [Editor de barras de herramientas](toolbar-editor.md)
##### [Creación de nuevas barras de herramientas](creating-new-toolbars.md)
##### [Conversión de mapas de bits en barras de herramientas](converting-bitmaps-to-toolbars.md)
##### [Creación, migración y edición de botones de la barra de herramientas](creating-moving-and-editing-toolbar-buttons.md)
##### [Creación de información sobre herramientas para un botón de la barra de herramientas](creating-a-tool-tip-for-a-toolbar-button.md)
#### [Editor de la información de versión](version-information-editor.md)
# [Biblioteca de plantillas activas (ATL)](../atl/TOC.md)
# [Microsoft Foundation Classes (MFC)](../mfc/TOC.md)
# [Clases compartidas ATL y MFC](../atl-mfc-shared/TOC.md)
# [Desarrollo de .NET con C++/CLI](../dotnet/dotnet-programming-with-cpp-cli-visual-cpp.md)
# [Extensiones de componentes de .NET y UWP](component-extensions-for-runtime-platforms.md)
## [Operador de referencia de seguimiento (C++/CLI y C++/CX)](tracking-reference-operator-cpp-component-extensions.md)
## [Identificador a un operador de objeto (^) (C++/CLI y C++/CX)](handle-to-object-operator-hat-cpp-component-extensions.md)
## [abstract (C++/CLI y C++/CX)](abstract-cpp-component-extensions.md)
## [Matrices (C++/CLI y C++/CX)](arrays-cpp-component-extensions.md)
## [Conversión boxing (C++/CLI y C++/CX)](boxing-cpp-component-extensions.md)
## [Clases y structs (C++/CLI y C++/CX)](classes-and-structs-cpp-component-extensions.md)
## [Espacios de nombres de plataforma, predeterminado y CLI (C++/CLI y C++/CX)](platform-default-and-cli-namespaces-cpp-component-extensions.md)
## [Compatibilidad de compilador para type traits (C++/CLI y C++/CX)](compiler-support-for-type-traits-cpp-component-extensions.md)
## [Palabras clave contextuales (C++/CLI y C++/CX)](context-sensitive-keywords-cpp-component-extensions.md)
## [delegate (C++/CLI y C++/CX)](delegate-cpp-component-extensions.md)
## [Clase enum  (C++/CLI y C++/CX)](enum-class-cpp-component-extensions.md)
## [event (C++/CLI y C++/CX)](event-cpp-component-extensions.md)
## [Control de excepciones (C++/CLI y C++/CX)](exception-handling-cpp-component-extensions.md)
## [Invalidaciones explícitas (C++/CLI y C++/CX)](explicit-overrides-cpp-component-extensions.md)
## [ref new, gcnew (C++/CLI y C++/CX)](ref-new-gcnew-cpp-component-extensions.md)
## [Genéricos (C++/CLI y C++/CX)](generics-cpp-component-extensions.md)
### [Información general sobre genéricos en Visual C++](overview-of-generics-in-visual-cpp.md)
### [Funciones genéricas (C++/CLI)](generic-functions-cpp-cli.md)
### [Clases genéricas (C++/CLI)](generic-classes-cpp-cli.md)
### [Interfaces genéricas (C++/CLI)](generic-interfaces-visual-cpp.md)
### [Delegados genéricos (C++/CLI)](generic-delegates-visual-cpp.md)
### [Restricciones de parámetros de tipo genérico (C++/CLI)](constraints-on-generic-type-parameters-cpp-cli.md)
### [Uso de genéricos (C++/CLI)](consuming-generics-cpp-cli.md)
### [Genéricos y plantillas (C++/CLI)](generics-and-templates-visual-cpp.md)
### [Cómo: Mejorar el rendimiento con genéricos (C++/CLI)](how-to-improve-performance-with-generics-visual-cpp.md)
## [Clase interface (C++/CLI y C++/CX)](interface-class-cpp-component-extensions.md)
## [literal (C++/CLI y C++/CX)](literal-cpp-component-extensions.md)
## [Windows Runtime y plantillas administradas (C++/CLI y C++/CX)](windows-runtime-and-managed-templates-cpp-component-extensions.md)
## [New (nueva ranura en vtable) (C++/CLI y C++/CX)](new-new-slot-in-vtable-cpp-component-extensions.md)
## [nullptr (C++/CLI y C++/CX)](nullptr-cpp-component-extensions.md)
## [Especificadores de invalidación (C++/CLI y C++/CX)](override-specifiers-cpp-component-extensions.md)
## [override  (C++/CLI y C++/CX)](override-cpp-component-extensions.md)
## [partial (C++/CLI y C++/CX)](partial-cpp-component-extensions.md)
## [property  (C++/CLI y C++/CX)](property-cpp-component-extensions.md)
## [safe_cast (C++/CLI y C++/CX)](safe-cast-cpp-component-extensions.md)
## [Cadena (C++/CLI y C++/CX)](string-cpp-component-extensions.md)
## [sealed  (C++/CLI y C++/CX)](sealed-cpp-component-extensions.md)
## [typeid  (C++/CLI y C++/CX)](typeid-cpp-component-extensions.md)
## [Atributos definidos por el usuario (C++/CLI y C++/CX)](user-defined-attributes-cpp-component-extensions.md)
### [Tipos de parámetros de atributo (C++/CLI y C++/CX)](attribute-parameter-types-cpp-component-extensions.md)
### [Destinos de atributo (C++/CLI y C++/CX)](attribute-targets-cpp-component-extensions.md)
## [Extensiones específicas de C++/CLI](extensions-that-are-specific-to-cpp-cli.md)
### [__identifier (C++/CLI)](identifier-cpp-cli.md)
### [Conversiones de estilo C con /clr (C++/CLI)](c-style-casts-with-clr-cpp-cli.md)
### [interior_ptr (C++/CLI)](interior-ptr-cpp-cli.md)
#### [Cómo: Declarar y usar punteros internos y matrices administradas (C++/CLI)](how-to-declare-and-use-interior-pointers-and-managed-arrays-cpp-cli.md)
#### [Cómo: Declarar tipos de valor con la palabra clave interior_ptr (C++/CLI)](how-to-declare-value-types-with-the-interior-ptr-keyword-cpp-cli.md)
#### [Cómo: Sobrecargar funciones con punteros internos y punteros nativos (C++/CLI)](how-to-overload-functions-with-interior-pointers-and-native-pointers-cpp-cli.md)
#### [Cómo: Declarar punteros internos con la palabra clave const (C++/CLI)](how-to-declare-interior-pointers-with-the-const-keyword-cpp-cli.md)
### [pin_ptr (C++/CLI)](pin-ptr-cpp-cli.md)
#### [Cómo: Anclar punteros y matrices](how-to-pin-pointers-and-arrays.md)
#### [Cómo: Declarar punteros anclados y tipos de valor](how-to-declare-pinning-pointers-and-value-types.md)
### [Reenvío de tipos (C++/CLI)](type-forwarding-cpp-cli.md)
### [Listas de argumentos de variables (...) (C++/CLI)](variable-argument-lists-dot-dot-dot-cpp-cli.md)
## [Recursos para crear un juego mediante DirectX](resources-for-creating-a-game-using-directx.md)
## [Tutorial: Crear y utilizar una biblioteca estática (C++)](walkthrough-creating-and-using-a-static-library-cpp.md)
## [Cómo: Usar el SDK de Windows 10 en una aplicación de escritorio de Windows](how-to-use-the-windows-10-sdk-in-a-windows-desktop-application.md)
# [Aplicaciones de Windows universales (C++)](universal-windows-apps-cpp.md)
## [C++/CX](../cppcx/TOC.md)
## [Biblioteca de plantillas C++ de Windows en tiempo de ejecución (WRL)](wrl/windows-runtime-cpp-template-library-wrl.md)
### [Cómo: Activar y usar un componente de Windows Runtime mediante WRL](wrl/how-to-activate-and-use-a-windows-runtime-component-using-wrl.md)
### [Cómo: Completar operaciones asincrónicas mediante WRL](wrl/how-to-complete-asynchronous-operations-using-wrl.md)
### [Cómo: Controlar eventos mediante WRL](wrl/how-to-handle-events-using-wrl.md)
### [Tutorial: Crear una aplicación de UWP mediante WRL y Media Foundation](wrl/walkthrough-creating-a-windows-store-app-using-wrl-and-media-foundation.md)
### [Cómo: Crear un componente COM clásico mediante WRL](wrl/how-to-create-a-classic-com-component-using-wrl.md)
### [Cómo: Crear instancias de componentes WRL directamente](wrl/how-to-instantiate-wrl-components-directly.md)
### [Cómo: Usar winmdidl.exe y midlrt.exe para crear archivos .h desde metadatos de Windows](wrl/use-winmdidl-and-midlrt-to-create-h-files-from-windows-metadata.md)
### [API clave de WRL por categoría](wrl/key-wrl-apis-by-category.md)
### [Referencia de WRL](wrl/wrl-reference.md)
#### [Microsoft::WRL (espacio de nombres)](wrl/microsoft-wrl-namespace.md)
##### [ActivatableClass (macros)](wrl/activatableclass-macros.md)
##### [ActivationFactory (clase)](wrl/activationfactory-class.md)
##### [AgileActivationFactory (clase)](wrl/agileactivationfactory-class.md)
##### [Clase AgileEventSource](wrl/agileeventsource-class.md)
##### [AsWeak (función)](wrl/asweak-function.md)
##### [AsyncBase (clase)](wrl/asyncbase-class.md)
##### [AsyncResultType (enumeración)](wrl/asyncresulttype-enumeration.md)
##### [Callback (función) (WRL)](wrl/callback-function-wrl.md)
##### [CancelTransitionPolicy (enumeración)](wrl/canceltransitionpolicy-enumeration.md)
##### [ChainInterfaces (estructura)](wrl/chaininterfaces-structure.md)
##### [ClassFactory (clase)](wrl/classfactory-class.md)
##### [CloakedIid (estructura)](wrl/cloakediid-structure.md)
##### [ComposableBase (clase)](wrl/composablebase-class.md)
##### [ComPtr (clase)](wrl/comptr-class.md)
##### [CreateActivationFactory (función)](wrl/createactivationfactory-function.md)
##### [CreateClassFactory (función)](wrl/createclassfactory-function.md)
##### [DeferrableEventArgs (clase)](wrl/deferrableeventargs-class.md)
##### [EventSource (clase)](wrl/eventsource-class.md)
##### [FactoryCacheFlags (enumeración)](wrl/factorycacheflags-enumeration.md)
##### [FtmBase (clase)](wrl/ftmbase-class.md)
##### [GetModuleBase (función)](wrl/getmodulebase-function.md)
##### [Implements (estructura)](wrl/implements-structure.md)
##### [InspectableClass (macro)](wrl/inspectableclass-macro.md)
##### [Estructura InvokeModeOptions](wrl/invokemodeoptions-structure.md)
##### [Make (función)](wrl/make-function.md)
##### [MixIn (estructura)](wrl/mixin-structure.md)
##### [Module (clase)](wrl/module-class.md)
###### [Module::GenericReleaseNotifier (clase)](wrl/module-genericreleasenotifier-class.md)
###### [Module::MethodReleaseNotifier (clase)](wrl/module-methodreleasenotifier-class.md)
###### [Module::ReleaseNotifier (clase)](wrl/module-releasenotifier-class.md)
##### [ModuleType (enumeración)](wrl/moduletype-enumeration.md)
##### [operator!= (operador) (Microsoft::WRL)](wrl/operator-inequality-operator-microsoft-wrl.md)
##### [operator== (operador) (Microsoft::WRL)](wrl/operator-equality-operator-microsoft-wrl.md)
##### [operator< (operador) (Microsoft::WRL)](wrl/operator-less-than-operator-microsoft-wrl.md)
##### [RuntimeClass (clase)](wrl/runtimeclass-class.md)
##### [RuntimeClassFlags (estructura)](wrl/runtimeclassflags-structure.md)
##### [RuntimeClassType (enumeración)](wrl/runtimeclasstype-enumeration.md)
##### [SimpleActivationFactory (clase)](wrl/simpleactivationfactory-class.md)
##### [SimpleClassFactory (clase)](wrl/simpleclassfactory-class.md)
##### [WeakRef (clase)](wrl/weakref-class.md)
#### [Microsoft::WRL::Details (espacio de nombres)](wrl/microsoft-wrl-details-namespace.md)
##### [ActivationFactoryCallback (función)](wrl/activationfactorycallback-function.md)
##### [ArgTraits (estructura)](wrl/argtraits-structure.md)
##### [ArgTraitsHelper (estructura)](wrl/argtraitshelper-structure.md)
##### [AsyncStatusInternal (enumeración)](wrl/asyncstatusinternal-enumeration.md)
##### [BoolStruct (estructura)](wrl/boolstruct-structure.md)
##### [ComPtrRef (clase)](wrl/comptrref-class.md)
##### [ComPtrRefBase (clase)](wrl/comptrrefbase-class.md)
##### [CreatorMap (estructura)](wrl/creatormap-structure.md)
##### [DerefHelper (estructura)](wrl/derefhelper-structure.md)
##### [DontUseNewUseMake (clase)](wrl/dontusenewusemake-class.md)
##### [EnableIf (estructura)](wrl/enableif-structure.md)
##### [EventTargetArray (clase)](wrl/eventtargetarray-class.md)
##### [FactoryCache (estructura)](wrl/factorycache-structure.md)
##### [ImplementsBase (estructura)](wrl/implementsbase-structure.md)
##### [ImplementsHelper (estructura)](wrl/implementshelper-structure.md)
##### [InterfaceList (estructura)](wrl/interfacelist-structure.md)
##### [InterfaceListHelper (estructura)](wrl/interfacelisthelper-structure.md)
##### [InterfaceTraits (estructura)](wrl/interfacetraits-structure.md)
##### [InvokeHelper (estructura)](wrl/invokehelper-structure.md)
##### [IsBaseOfStrict (estructura)](wrl/isbaseofstrict-structure.md)
##### [IsSame (estructura)](wrl/issame-structure.md)
##### [MakeAllocator (clase)](wrl/makeallocator-class.md)
##### [MakeAndInitialize (función)](wrl/makeandinitialize-function.md)
##### [ModuleBase (clase)](wrl/modulebase-class.md)
##### [Move (función)](wrl/move-function.md)
##### [Nil (estructura)](wrl/nil-structure.md)
##### [RaiseException (función)](wrl/raiseexception-function.md)
##### [RemoveIUnknown (clase)](wrl/removeiunknown-class.md)
##### [RemoveReference (estructura)](wrl/removereference-structure.md)
##### [RuntimeClassBase (estructura)](wrl/runtimeclassbase-structure.md)
##### [RuntimeClassBaseT (estructura)](wrl/runtimeclassbaset-structure.md)
##### [Swap (función) (WRL)](wrl/swap-function-wrl.md)
##### [TerminateMap (función)](wrl/terminatemap-function.md)
##### [VerifyInheritanceHelper (estructura)](wrl/verifyinheritancehelper-structure.md)
##### [VerifyInterfaceHelper (estructura)](wrl/verifyinterfacehelper-structure.md)
##### [WeakReference (clase)](wrl/weakreference-class.md)
#### [Microsoft::WRL::Wrappers (espacio de nombres)](wrl/microsoft-wrl-wrappers-namespace.md)
##### [CriticalSection (clase)](wrl/criticalsection-class.md)
##### [Event (clase) (WRL)](wrl/event-class-wrl.md)
##### [HandleT (clase)](wrl/handlet-class.md)
##### [HString (clase)](wrl/hstring-class.md)
##### [HStringReference (clase)](wrl/hstringreference-class.md)
##### [Mutex (clase)](wrl/mutex-class.md)
##### [RoInitializeWrapper (clase)](wrl/roinitializewrapper-class.md)
##### [Semaphore (clase)](wrl/semaphore-class.md)
##### [SRWLock (clase)](wrl/srwlock-class.md)
#### [Microsoft::WRL::Wrappers::Details (espacio de nombres)](wrl/microsoft-wrl-wrappers-details-namespace.md)
##### [CompareStringOrdinal (método)](wrl/comparestringordinal-method.md)
##### [SyncLockT (clase)](wrl/synclockt-class.md)
##### [SyncLockWithStatusT (clase)](wrl/synclockwithstatust-class.md)
#### [Microsoft::WRL::Wrappers::HandleTraits (espacio de nombres)](wrl/microsoft-wrl-wrappers-handletraits-namespace.md)
##### [CriticalSectionTraits (estructura)](wrl/criticalsectiontraits-structure.md)
##### [EventTraits (estructura)](wrl/eventtraits-structure.md)
##### [FileHandleTraits (estructura)](wrl/filehandletraits-structure.md)
##### [HANDLENullTraits (estructura)](wrl/handlenulltraits-structure.md)
##### [HANDLETraits (estructura)](wrl/handletraits-structure.md)
##### [MutexTraits (estructura)](wrl/mutextraits-structure.md)
##### [SemaphoreTraits (estructura)](wrl/semaphoretraits-structure.md)
##### [SRWLockExclusiveTraits (estructura)](wrl/srwlockexclusivetraits-structure.md)
##### [SRWLockSharedTraits (estructura)](wrl/srwlocksharedtraits-structure.md)
#### [Windows::Foundation (espacio de nombres)](wrl/windows-foundation-namespace.md)
##### [ActivateInstance (función)](wrl/activateinstance-function.md)
##### [GetActivationFactory (función)](wrl/getactivationfactory-function.md)
#### [IID_PPV_ARGS_Helper (función)](wrl/iid-ppv-args-helper-function.md)
# [Biblioteca SafeInt](safeint-library.md)
## [SafeInt (clase)](safeint-class.md)
## [SafeInt (funciones)](safeint-functions.md)
## [SafeIntException (clase)](safeintexception-class.md)
# [Atributos de C++ para COM y .NET](attributes/cpp-attributes-com-net.md)
## [Preguntas más frecuentes de programación con atributos](attributes/attribute-programming-faq.md)
## [Atributos por grupo](attributes/attributes-by-group.md)
### [Atributos COM](attributes/com-attributes.md)
### [Atributos IDL](attributes/idl-attributes.md)
### [Atributos de consumidor OLE DB](attributes/ole-db-consumer-attributes.md)
### [Atributos de compilador](attributes/compiler-attributes.md)
## [Atributos por uso](attributes/attributes-by-usage.md)
### [Atributos de módulo](attributes/module-attributes.md)
### [Atributos de interfaz](attributes/interface-attributes.md)
### [Atributos de clase](attributes/class-attributes.md)
### [Atributos de método](attributes/method-attributes.md)
### [Atributos de parámetro](attributes/parameter-attributes.md)
### [Atributos de miembros de datos](attributes/data-member-attributes.md)
### [Typedef, Enum, Union y Struct (atributos)](attributes/typedef-enum-union-and-struct-attributes.md)
### [Atributos de matriz](attributes/array-attributes.md)
### [Atributos independientes](attributes/stand-alone-attributes.md)
### [Atributos personalizados](attributes/custom-attributes-cpp.md)
## [Referencia alfabética de los atributos](attributes/attributes-alphabetical-reference.md)
### [aggregatable](attributes/aggregatable.md)
### [aggregates](attributes/aggregates.md)
### [appobject](attributes/appobject.md)
### [async_uuid](attributes/async-uuid.md)
### [attribute](attributes/attribute.md)
### [bindable](attributes/bindable.md)
### [call_as](attributes/call-as.md)
### [case](attributes/case-cpp.md)
### [coclase](attributes/coclass.md)
### [com_interface_entry](attributes/com-interface-entry-cpp.md)
### [control](attributes/control.md)
### [cpp_quote](attributes/cpp-quote.md)
### [custom](attributes/custom-cpp.md)
### [db_accessor](attributes/db-accessor.md)
### [db_column](attributes/db-column.md)
### [db_command](attributes/db-command.md)
### [db_param](attributes/db-param.md)
### [db_source](attributes/db-source.md)
### [db_table](attributes/db-table.md)
### [default](attributes/default-cpp.md)
### [defaultbind](attributes/defaultbind.md)
### [defaultcollelem](attributes/defaultcollelem.md)
### [defaultvalue](attributes/defaultvalue.md)
### [defaultvtable](attributes/defaultvtable.md)
### [dispinterface](attributes/dispinterface.md)
### [displaybind](attributes/displaybind.md)
### [dual](attributes/dual.md)
### [emitidl](attributes/emitidl.md)
### [entry](attributes/entry.md)
### [event_receiver](attributes/event-receiver.md)
### [event_source](attributes/event-source.md)
### [export](attributes/export.md)
### [first_is](attributes/first-is.md)
### [helpcontext](attributes/helpcontext.md)
### [helpfile](attributes/helpfile.md)
### [helpstring](attributes/helpstring.md)
### [helpstringcontext](attributes/helpstringcontext.md)
### [helpstringdll](attributes/helpstringdll.md)
### [hidden](attributes/hidden.md)
### [id](attributes/id.md)
### [idl_module](attributes/idl-module.md)
### [idl_quote](attributes/idl-quote.md)
### [iid_is](attributes/iid-is.md)
### [immediatebind](attributes/immediatebind.md)
### [implements](attributes/implements-cpp.md)
### [implements_category](attributes/implements-category.md)
### [import](attributes/import.md)
### [importidl](attributes/importidl.md)
### [importlib](attributes/importlib.md)
### [in](attributes/in-cpp.md)
### [include](attributes/include-cpp.md)
### [includelib](attributes/includelib-cpp.md)
### [last_is](attributes/last-is.md)
### [lcid](attributes/lcid.md)
### [length_is](attributes/length-is.md)
### [library_block](attributes/library-block.md)
### [licensed](attributes/licensed.md)
### [local](attributes/local-cpp.md)
### [max_is](attributes/max-is.md)
### [module](attributes/module-cpp.md)
### [ms_union](attributes/ms-union.md)
### [no_injected_text](attributes/no-injected-text.md)
### [nonbrowsable](attributes/nonbrowsable.md)
### [noncreatable](attributes/noncreatable.md)
### [nonextensible](attributes/nonextensible.md)
### [object](attributes/object-cpp.md)
### [odl](attributes/odl.md)
### [oleautomation](attributes/oleautomation.md)
### [optional](attributes/optional-cpp.md)
### [out](attributes/out-cpp.md)
### [pointer_default](attributes/pointer-default.md)
### [pragma](attributes/pragma.md)
### [progid](attributes/progid.md)
### [propget](attributes/propget.md)
### [propput](attributes/propput.md)
### [propputref](attributes/propputref.md)
### [ptr](attributes/ptr.md)
### [public (atributos de C++)](attributes/public-cpp-attributes.md)
### [range](attributes/range-cpp.md)
### [rdx](attributes/rdx.md)
### [readonly](attributes/readonly-cpp.md)
### [ref](attributes/ref-cpp.md)
### [registration_script](attributes/registration-script.md)
### [requestedit](attributes/requestedit.md)
### [requires_category](attributes/requires-category.md)
### [restricted](attributes/restricted.md)
### [retval](attributes/retval.md)
### [satype](attributes/satype.md)
### [size_is](attributes/size-is.md)
### [source](attributes/source-cpp.md)
### [string](attributes/string-cpp.md)
### [support_error_info](attributes/support-error-info.md)
### [switch_is](attributes/switch-is.md)
### [switch_type](attributes/switch-type.md)
### [synchronize](attributes/synchronize.md)
### [threading](attributes/threading-cpp.md)
### [transmit_as](attributes/transmit-as.md)
### [uidefault](attributes/uidefault.md)
### [unique](attributes/unique-cpp.md)
### [usesgetlasterror](attributes/usesgetlasterror.md)
### [uuid (atributos de C++)](attributes/uuid-cpp-attributes.md)
### [v1_enum](attributes/v1-enum.md)
### [vararg](attributes/vararg.md)
### [version](attributes/version-cpp.md)
### [vi_progid](attributes/vi-progid.md)
### [wire_marshal](attributes/wire-marshal.md)
