---
title: "Informaci&#243;n general de las convenciones ABI de ARM | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
La interfaz binaria de aplicaciones \(ABI\) para código compilado para Windows en procesadores de ARM se basa en la norma EABI de ARM.  En este artículo se desgranan las principales diferencias entre Windows en ARM y la norma.  Para más información sobre la norma EABI de ARM, vea el tema sobre la [interfaz binaria de aplicaciones \(ABI\) en la arquitectura de ARM](http://infocenter.arm.com/help/index.jsp?topic=/com.arm.doc.subset.swdev.abi/index.html).  
...
## Paso de parámetros  
 En las funciones no variádicas, la ABI de Windows en ARM sigue las reglas de ARM para el paso de parámetros \(lo que engloba las extensiones VFP y SIMD avanzadas\).  Estas reglas siguen la [norma de llamada a procedimiento de la arquitectura ARM](http://infocenter.arm.com/help/topic/com.arm.doc.ihi0042c/IHI0042C_aapcs.pdf), consolidada con las extensiones VFP.  De forma predeterminada, se pasan en los registros los primeros cuatro argumentos de entero y hasta ocho argumentos de punto flotante o de vector; los argumentos adicionales se pasan en la pila.  Se usa el siguiente procedimiento para asignar argumentos a los registros o a la pila:  
...
### Fase C: asignación de argumentos a registros y la pila  
...
## Recorrido de la pila  
 El código de Windows se compila con los punteros de marco habilitados \([\/Oy \(Omisión de puntero de marco\)](../build/reference/oy-frame-pointer-omission.md)\) para permitir unos recorridos de pila rápidos.  Por lo general, el registro r11 apunta al siguiente vínculo de la cadena, que es un par {r11, lr} que especifica el puntero al marco anterior en la pila, así como la dirección de devolución.  Le recomendamos que su código tenga habilitados también los punteros de marco, ya que así mejorarán los perfiles y seguimientos.  
...
## Desenredado en excepciones  
 ...
 La EABI de ARM especifica un modelo de desenredado en excepciones en el que se usan códigos de desenredado.  Sin embargo, esta especificación no basta para el desenredado en Windows, donde se deben controlar casos en los que el procesador se encuentra en medio del prólogo y el epílogo de una función.  Para más información sobre el desenredado y los datos en excepciones de Windows en ARM, vea [Control de excepciones de ARM](../build/arm-exception-handling.md).  
