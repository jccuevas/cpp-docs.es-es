...
- Crear el archivo de proyecto de MSBuild de XML.
...
- Una copia de Visual Studio con la **el desarrollo de escritorio con C++** instalada de la carga de trabajo.

- Una descripción general del sistema de MSBuild.

> [!NOTE]
> No utilice este enfoque si va a editar el archivo de proyecto más adelante mediante el IDE de Visual Studio. Si crea manualmente un archivo .vcxproj, el IDE de Visual Studio no podría tener editar o cargarla, especialmente si el proyecto utiliza caracteres comodín en elementos de proyecto.

> [!NOTE]
> La mayoría de las instrucciones de compilación de bajo nivel se incluyen en el **.targets** y **.props** archivos que se definen en el directorio VCTargets, almacenado en la propiedad `$(VCTargetsPath)`. La ruta de acceso predeterminada de estos archivos en Visual Studio de 2017 Enterprise Edition es C:\\archivos de programa (x86)\\Microsoft Visual Studio\\2017\\Enterprise\\Common7\\IDE\\ VC\\VCTargets\\.

## <a name="creating-the-c-source-files"></a>Crear los archivos de origen de C++

En este tutorial creará un proyecto que tiene un archivo de origen y un archivo de encabezado. El archivo de origen main.cpp contiene la función main de la aplicación de consola. El archivo de encabezado main.h contiene código para incluir el archivo de encabezado iostream. Puede crear estos archivos C++ mediante Visual Studio o un texto de editor como código de Visual Studio.
...
Un archivo de proyecto de MSBuild es un archivo XML que contiene un elemento raíz del proyecto (\<proyecto >). En el proyecto de ejemplo siguiente, la \<proyecto > elemento contiene siete elementos secundarios:
...
6. Agregue el siguiente \<ClCompile > elemento secundario de un \<ItemGroup > elemento. El elemento secundario especifica el nombre del archivo de origen de C o C++ para compilar:
   ...
   > [!NOTE]
   > \<ClCompile > es un *crear destino* y se define en el **VCTargets** directory.
...
MSBuild crea un directorio para los archivos de salida y, a continuación, compila y vincula el proyecto para generar el programa Myproject.exe. Al finalizar el proceso de compilación, use el comando siguiente para ejecutar la aplicación:
...
MSBuild permite ejecutar destinos de compilación predefinidos, aplicar propiedades definidas por el usuario y usar herramientas personalizadas, eventos y pasos de compilación. En esta sección se muestra las tareas siguientes:
...
MSBuild proporciona varias maneras de personalizar el proceso de compilación. Los temas siguientes muestran cómo agregar pasos de compilación personalizada, las herramientas y los eventos para el proyecto de MSBuild:
