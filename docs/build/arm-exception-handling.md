...
|-----------------|----------|-------------|  
|0|0-31|*Función iniciar RVA* es la dirección RVA de 32 bits del inicio de la función. Si la función contiene código Thumb, se debe establecer el bit inferior de esta dirección.|  
|1|0-1|*Marca* es un campo de 2 bits que indica cómo interpretar los 30 bits restantes de la segunda palabra .pdata. Si *marca* es 0, entonces el resto de bits forma un *RVA de información de excepción* (con los dos bits inferiores implícitamente en 0). Si *marca* es distinto de cero, el resto de bits forma un *datos de desenredado empaquetado* estructura.|  
|1|2-31|*Información de excepción RVA* o *empaquetan datos de desenredo*.<br /><br /> *RVA de información de excepción* es la dirección de la estructura de información de excepción de longitud variable, almacenada en la sección .xdata de forma apropiada. Estos datos deben tener una alineación de 4 bytes.<br /><br /> *Empaquetado de datos de desenredo* es una descripción comprimida de las operaciones necesarias para desenredar desde una función, suponiendo una forma canónica. En este caso, no se necesita un registro .xdata.|  
...
|-----------------|----------|-------------|  
|0|0-31|*Función iniciar RVA* es la dirección RVA de 32 bits del inicio de la función. Si la función contiene código Thumb, se debe establecer el bit inferior de esta dirección.|  
|1|0-1|*Marca* es un campo de 2 bits que tiene los siguientes significados:<br /><br /> -00 = empaquetado desenredar datos no utilizados; Seleccione un registro .xdata bits restantes.<br />-01 = empaquetado datos de desenredo.<br />-10 = empaquetado datos donde se supone que la función carece de prólogo de desenredado. Esto resulta práctico a la hora de describir fragmentos de la función que no son contiguas al inicio de la función.<br />-11 = reservado.|  
...
|1|13-14|*RET* es un campo de 2 bits que indica cómo devuelve la función:<br /><br /> -00 = devolución mediante extracción {pc} (el *L* bit de marca debe establecerse en 1 en este caso).<br />-01 = devolución mediante una bifurcación de 16 bits.<br />-10 = devolución mediante una bifurcación de 32 bits.<br />-11 no = ningún epílogo en absoluto. Esto resulta práctico a la hora de describir un fragmento de función no contiguo que solo puede contener un prólogo, pero cuyo epílogo puede estar en cualquier otra parte.|  
|1|15|*H* es una marca de 1 bit que indica si la función "alberga" el parámetro de número entero registra (r0-r3) insertándolos al principio de la función y desasigna los 16 bytes de pila antes de devolver. (0 = no alberga registros, 1 = alberga registros).|  
|1|16-18|*Reg* se guarda un campo de 3 bits que indica el índice del último registro no volátil. Si el *R* bit es 0, entonces solo los registros de enteros se guardan y se suponen que están en el intervalo r4-RN, donde N es igual a 4 + *Reg*. Si el *R* bit es 1, a continuación, registros de punto flotante sólo se guardan y se suponen que están en el intervalo d8-DN, donde N es igual a 8 + *Reg*. La combinación especial de *R* = 1 y *Reg* = 7 pone de manifiesto que no se guardan registros.|  
...
|1|20|*L* es una marca de 1 bit que indica si la función guarda/restaura LR, junto con otros registros especificados por la *Reg* campo. (0 = no guarda/restaura, 1 = guarda/restaura).|  
|1|21|*C* es una marca de 1 bit que indica si la función incluye instrucciones adicionales para configurar una cadena de marcos de pila rápidos (1) o no (0). Si este bit se establece, r11 se agrega implícitamente a la lista de registros no volátiles de enteros guardados (Vea las restricciones siguiente si el *C* marca se utiliza.)|  
|1|22-31|*Ajustar la pila* es un campo de 10 bits que indica el número de bytes de la pila que se asignan para esta función, dividida entre 4. Con todo, solo se pueden codificar directamente los valores comprendidos entre 0x000-0x3F3. Las funciones que asignan más de 4044 bytes de la pila deben usar un registro .xdata completo. Si el *ajuste de pila* campo es 0x3F4 o superior, los 4 bits inferiores tienen un significado especial:<br /><br /> -Los bits 0-1 indican el número de palabras del ajuste de pila (1-4) menos 1.<br />-Bit 2 se establece en 1 si el prólogo combinó este ajuste en su operación de inserción.<br />-Bit 3 se establece en 1 si el epílogo combinó este ajuste en su operación de extracción.|  
...
-   Si el *C* marca se establece en 1:  
    ...
    -   El *L* marca también se debe establecer en 1, porque el encadenamiento de marcos requería tanto r11 como LR.  
  
    -   R11 no debe incluirse en el conjunto de registros descrito por *Reg*. Es decir, si se inserta r4 a r11, *Reg* solo deberá describir de r4 a r10, porque la *C* marca implica r11.  
...
-   Códigos de desenredado, empezando por Word 5: (que se comparten entre el prólogo y epílogo)  
    ...
    -   2 = 0xFF códigos de desenredado: final  
...
-   Códigos de desenredado, empezando por Word 2: (que se comparten entre el prólogo y epílogo)  
    ...
    -   1 = 0xDC códigos de desenredado: pop {r4 r8, lr}  
...
-   Códigos de desenredado, empezando por la palabra 1:  
    ...
    -   4 = 0xFF códigos de desenredado: final  
