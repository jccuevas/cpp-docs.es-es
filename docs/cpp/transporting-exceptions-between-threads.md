...
En las secciones siguientes se describe cómo transportar excepciones mediante el tipo de `exception_ptr` y las funciones `current_exception`, `rethrow_exception`y `make_exception_ptr`.
...
## <a name="current_exception-function"></a>current_exception función)

Llame a la función `current_exception` en un bloque **catch** . Si una excepción está en vuelo y el bloque **catch** puede detectar la excepción, la función `current_exception` devuelve un `exception_ptr` objeto que hace referencia a la excepción. De lo contrario, la función devuelve un objeto `exception_ptr` NULL.
...
Si usa la opción del compilador **/EHA** , puede detectar una excepción SEH en C++ un bloque **catch** . La función `current_exception` devuelve un objeto `exception_ptr` que hace referencia a la excepción SEH. Y la función `rethrow_exception` produce la excepción SEH si se llama con el objeto `exception_ptr` de transporte como su argumento.

La función `current_exception` devuelve un `exception_ptr` NULL si se llama en un controlador de finalización de **__finally** de SEH, un controlador de excepciones de **__except** o la expresión de filtro **__except** .
