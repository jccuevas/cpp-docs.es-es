...
![Objeto de línea&#45;de comida simulada](../cpp/media/vc38xp2.gif "Objeto de línea&#45;de comida simulada") <br/>
...
La palabra clave **virtual** garantiza que solo se incluya una copia del subobjeto `Queue` (vea la ilustración siguiente).

![Objeto de línea&#45;de comida simulada, clases base virtuales](../cpp/media/vc38xp3.gif "Objeto de línea&#45;de comida simulada, clases base virtuales") <br/>
...
![Componentes virtuales y&#45;no virtuales de una clase](../cpp/media/vc38xp4.gif "Componentes virtuales y&#45;no virtuales de una clase") <br/>
Componentes virtuales y no virtuales de la misma clase

En la ilustración, `CashierQueue` y `LunchQueue` usan `Queue` como clase base virtual. Sin embargo, `TakeoutQueue` especifica `Queue` como clase base, no como una clase base virtual. Por consiguiente, `LunchTakeoutCashierQueue` tiene dos subobjetos de tipo `Queue`: uno en la ruta de herencia que incluye `LunchCashierQueue` y otro en la ruta que incluye `TakeoutQueue`. Observe la siguiente imagen:

![Herencia virtual &&#45;no virtual en el diseño de objetos](../cpp/media/vc38xp5.gif "Herencia virtual &&#45;no virtual en el diseño de objetos") <br/>
...
Si una clase derivada reemplaza una función virtual que hereda de una clase base virtual y si un constructor o destructor para la clase derivada llama a esa función con un puntero a la clase base virtual, el compilador puede incluir campos "vtordisp" ocultos adicionales en clases con bases virtuales. La opción del compilador `/vd0` suprime la adición del miembro de desplazamiento oculto del constructor/destructor de vtordisp. La opción del compilador `/vd1`, el valor predeterminado, los habilita cuando son necesarias. Desactive los vtordisp solo si está seguro de que todos los constructores y destructores de clase llaman a funciones virtuales virtualmente.

La opción del compilador `/vd` afecta a un módulo de compilación completo. Utilice el pragma `vtordisp` para suprimir y, a continuación, volver a habilitar los campos `vtordisp` clase por clase:
...
1. Si el acceso al nombre infringe el permiso de acceso a miembros, se genera un mensaje de error. (Para obtener más información, consulte [member-access control](../cpp/member-access-control-cpp.md)).
...
![Derivación virtual y&#45;no virtual](../cpp/media/vc38xr1.gif "Derivación virtual y&#45;no virtual") <br/>
