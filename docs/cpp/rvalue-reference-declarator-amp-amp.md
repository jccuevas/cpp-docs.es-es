...
Antes de Visual Studio 2010, cada llamada a `string` **operator+** asigna y devuelve un nuevo objeto temporal (un valor r). **operator+** no puede anexar una cadena a la otra porque no sabe si las cadenas de origen son lvalues o rvalues. Si las cadenas de origen son ambas valores L, puede que se haga referencia a las mismas en otra parte del programa y, por consiguiente, no se deben modificar. Mediante el uso de referencias rvalue, **operator+** se puede modificar para tomar valores r, a los que no se puede hacer referencia en ninguna otra parte del programa. Por lo tanto, **operator+** ahora puede anexar una cadena a otra. Esto puede reducir significativamente el número de asignaciones de memoria dinámica que la clase `string` debe realizar. Para obtener más `string` información acerca de la clase, vea [basic_string (Clase).](../standard-library/basic-string-class.md)
...
Para obtener más información acerca de la introducción de la semántica de movimiento en la biblioteca estándar C++ en Visual Studio 2010, vea [Biblioteca estándar De C++](../standard-library/cpp-standard-library-reference.md).
...
Considere el ejemplo siguiente en el que se declaran cuatro tipos, `W`, `X`, `Y` y `Z`. El constructor para cada tipo toma una combinación diferente de **referencias const** y non-**const** lvalue como sus parámetros.
...
En este ejemplo se usan referencias de valor R como parámetros para la función `factory`. El propósito de la función [std::forward](../standard-library/utility-functions.md#forward) es reenviar los parámetros de la función factory al constructor de la clase de plantilla.
...
La función c++ Biblioteca estándar [std::move](../standard-library/utility-functions.md#move) permite convertir un objeto en una referencia rvalue a ese objeto. Como alternativa, puede utilizar la palabra clave **static_cast** para convertir un valor l en una referencia rvalue, como se muestra en el ejemplo siguiente:
...
Es común escribir una plantilla de función que pase (o *reenvíe)* sus parámetros a otra función. Es importante saber cómo funciona la deducción de tipos de plantilla para plantillas de función que acepten referencias de valor R.

Si el argumento de función es un valor R, el compilador deduce que el argumento será una referencia de valor R. Por ejemplo, si se pasa una referencia de valor R a un objeto de tipo `X` a una función de plantilla que acepta el tipo `T&&` como su parámetro, la deducción de argumento de plantilla deduce que `T` será `X`. Por consiguiente, el parámetro es de tipo `X&&`. Si el argumento function es un lvalue o **const** lvalue, el compilador deduce que su tipo es una referencia lvalue o **const** lvalue de ese tipo.
