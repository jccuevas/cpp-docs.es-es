...
|[CTime::FormatGmt](#formatgmt)|Convierte un `CTime` objeto en una cadena con formato, basándose en la hora UTC.|  
...
|[CTime::GetAsSystemTime](#getassystemtime)|Convierte la información de hora almacenada en el `CTime` objeto Win32 compatibles [SYSTEMTIME](http://msdn.microsoft.com/library/windows/desktop/ms724950) estructura.|  
|[CTime:: GetCurrentTime](#getcurrenttime)|Crea un `CTime` objeto que representa la hora actual (función miembro estática).|  
|[CTime::GetDay](#getday)|Devuelve el día representan por la `CTime` objeto.|  
...
|[CTime::GetLocalTm](#getlocaltm)|Divide un `CTime` objeto en componentes, basados en la zona horaria local.|  
...
|[operador ==,< ,="">](#ctime_comparison_operators)|Operadores de comparación.|  
  
## <a name="remarks"></a>Comentarios  
 `CTime`no tiene una clase base.  
  
 `CTime`los valores se basan en hora universal coordinada (UTC), que es equivalente a la de hora Universal coordinada (hora del meridiano de Greenwich, hora peninsular española). Consulte [administración del tiempo](../../c-runtime-library/time-management.md) para obtener información acerca de cómo se determina la zona horaria.  
  
 Cuando se crea un `CTime` objeto, establecer el `nDST` parámetro en 0 para indicar ese tiempo estándar está en vigor, o a un valor mayor que 0 para indicar que horario de verano está en vigor o a un valor menor que cero para que el proceso de código de biblioteca en tiempo de ejecución de C si hora estándar o el horario de verano está en vigor. `tm_isdst` es un campo obligatorio. Si no se establece, su valor es indefinido y el valor devuelto de [mktime](../../c-runtime-library/reference/mktime-mktime32-mktime64.md) es imprevisible. Si `timeptr` apunta a una estructura de tm devuelta por una llamada anterior a [asctime_s](../../c-runtime-library/reference/asctime-s-wasctime-s.md), [_gmtime_s](../../c-runtime-library/reference/gmtime-s-gmtime32-s-gmtime64-s.md), o [localtime_s](../../c-runtime-library/reference/localtime-s-localtime32-s-localtime64-s.md), el `tm_isdst` campo contiene el valor correcto.  
  
 Una clase complementaria, [CTimeSpan](../../atl-mfc-shared/reference/ctimespan-class.md), representa un intervalo de tiempo.  
  
 El `CTime` y `CTimeSpan` clases no están diseñadas para la derivación. Porque no hay ninguna función virtual, el tamaño de `CTime` y `CTimeSpan` objetos es exactamente 8 bytes. La mayoría de las funciones de miembro están alineados.  
  
> [!NOTE]
>  El límite superior de fecha es 12/31/3000. El límite inferior es 1/1/1970 12:00:00 A.M. GMT.  
  
 Para obtener más información acerca del uso de `CTime`, consulte los artículos [fecha y hora](../../atl-mfc-shared/date-and-time.md), y [administración del tiempo](../../c-runtime-library/time-management.md) en la referencia de la biblioteca de tiempo de ejecución.  
  
> [!NOTE]
>  El `CTime` estructura cambió de MFC 7.1 a MFC 8.0. Si se serializa un `CTime` estructura mediante la `operator <<` en MFC 8.0 o una versión posterior, el archivo resultante no se podrá leer en versiones anteriores de MFC.  
  
## <a name="requirements"></a>Requisitos  
 **Encabezado:** atltime.h  
  
##  <a name="a-namectimecomparisonoperatorsa--ctime-comparison-operators"></a><a name="ctime_comparison_operators"></a>Operadores de comparación CTime  
...
### <a name="parameters"></a>Parámetros  
 ...
 `time`  
 Un **__time64_t** el valor de hora, que es el número de segundos después del 1 de enero de 1970 UTC. Tenga en cuenta que esto se ajustará a la hora local. Por ejemplo, si se encuentra en Nueva York y crear un `CTime` objeto, pasando un parámetro de 0, [CTime::GetMonth](#getmonth) devuelve 12.  
  
 En versiones de Visual C++ 6.0 y anteriores, `time` era un valor de `time_t`. Visual C++ .NET y más adelante convierte un `time_t` parámetro **__time64_t**.  
...
- `nDST`establece un valor mayor que 0Daylight horario está vigente.  
...
- `nDST`se establece en un valor menor que el valor predeterminado de 0The. Calcula automáticamente si hora estándar o el horario de verano está en vigor.  
 ...
 `ft`  
 Un [FILETIME](https://www.microsoftonedoc.com/#/organizations/e6f6a65cf14f462597b64ac058dbe1d0/projects/3fedad16-eaf1-41a6-8f96-0c1949c68f32/containers/a3daf831-1c5f-4bbe-964d-503870caf874/tocpaths/979ce746-dc17-4147-89f8-41d05c5fcc5f/locales/en-us) estructura que se convertirá en un valor de fecha y hora y copiar en el nuevo `CTime` objeto.  
...
- **CTime (const FILETIME aspecto); ** Construye una `CTime` objeto a partir de un `FILETIME` estructura. Probablemente no utilizará `CTime FILETIME` inicialización directamente. Si utiliza un `CFile` objeto para manipular un archivo `CFile::GetStatus` recupera la marca de tiempo del archivo a través de un `CTime` objeto inicializado con un `FILETIME` estructura. Este constructor supone que la hora UTC según y convierte automáticamente el valor en hora local antes de almacenar el resultado.  
 ...
 Para obtener más información, consulte el [SYSTEMTIME](http://msdn.microsoft.com/library/windows/desktop/ms724950) y [FILETIME](http://msdn.microsoft.com/library/windows/desktop/ms724284) estructura en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]. Consulte también el [MS-DOS fecha y hora](http://msdn.microsoft.com/library/windows/desktop/ms724503) entrada en el [!INCLUDE[winsdkshort](../../atl-mfc-shared/reference/includes/winsdkshort_md.md)].  
...
### <a name="parameters"></a>Parámetros  
 `pszFormat`  
 Un formato de cadena similar a la `printf` cadena de formato. Códigos de formato, precedidos por un porcentaje ( `%`) inicie sesión, se reemplazan por el correspondiente `CTime` componente. Otros caracteres de la cadena de formato se copian sin cambios a la cadena devuelta. Vea la función de tiempo de ejecución [strftime](../../c-runtime-library/reference/strftime-wcsftime-strftime-l-wcsftime-l.md) para obtener una lista de códigos de formato.  
...
##  <a name="a-nameformatgmta--ctimeformatgmt"></a><a name="formatgmt"></a>CTime::FormatGmt  
 Genera una cadena con formato que se corresponde con este `CTime` objeto.  
...
### <a name="remarks"></a>Comentarios  
 El valor de tiempo no se convierte y, por tanto, refleja la hora UTC.  
...
### <a name="parameters"></a>Parámetros  
 *timeDest*  
 Una referencia a un [SYSTEMTIME](http://msdn.microsoft.com/library/windows/desktop/ms724950) estructura que contendrá el valor de fecha y hora convertido de la `CTime` objeto.  
  
### <a name="return-value"></a>Valor devuelto  
 True si es correcto; en caso contrario, false.  
  
### <a name="remarks"></a>Comentarios  
 `GetAsSystemTime`almacena la hora resultante en la que hace referencia *timeDest* estructura. El `SYSTEMTIME` estructura de datos inicializado por esta función tendrá su **wMilliseconds** miembro establecido en cero.  
...
##  <a name="a-namegetdaya--ctimegetday"></a><a name="getday"></a>CTime::GetDay  
 Devuelve el día representan por la `CTime` objeto.  
...
### <a name="return-value"></a>Valor devuelto  
 Devuelve el día de la semana basado en hora local; 1 = el domingo, 2 = el lunes, 7 = el sábado.  
...
##  <a name="a-namegetgmttma--ctimegetgmttm"></a><a name="getgmttm"></a>CTime::GetGmtTm  
 Obtiene un **struct tm** que contiene una descomposición de la hora que contiene este `CTime` objeto.  
...
### <a name="return-value"></a>Valor devuelto  
 Devuelve la hora, que se basa en la hora local, en el intervalo de 0 a 23.  
...
### <a name="return-value"></a>Valor devuelto  
 Devuelve el mes, según la hora local, en el intervalo de 1 a 12 (1 = enero).  
...
### <a name="return-value"></a>Valor devuelto  
 **GetTime** devolverá el número de segundos entre la actual `CTime` objeto y el 1 de enero de 1970.  
...
### <a name="remarks"></a>Comentarios  
 Este operador de asignación sobrecargado copia el tiempo de origen en esta `CTime` objeto. El almacenamiento interno del tiempo en un `CTime` objeto es independiente de la zona horaria. Conversión de zona horaria no es necesaria durante la asignación.  
...
### <a name="parameters"></a>Parámetros  
 *intervalo de tiempo*  
 La `CTimeSpan` objeto para sumar o restar.  
...
### <a name="remarks"></a>Comentarios  
 `CTime`los objetos representan el tiempo absoluto, `CTimeSpan` objetos representan tiempo relativo. Los dos primeros operadores permiten sumar y restar `CTimeSpan` objetos a y desde `CTime` objetos. El tercer operador permite restar uno `CTime` objeto de otra para producir un `CTimeSpan` objeto.  
...
### <a name="remarks"></a>Comentarios  
 Estos operadores permiten sumar y restar un `CTimeSpan` objeto hacia y desde este `CTime` objeto.  
...
### <a name="parameters"></a>Parámetros  
 `ar`  
 La `CArchive` objeto que desea actualizar.  
