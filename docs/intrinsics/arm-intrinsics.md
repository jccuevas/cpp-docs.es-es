---
title: "Intrínsecos ARM | Documentos de Microsoft"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
##  <a name="top"></a>NEÓN  
 ...
 Se admiten intrínsecos NEON, como se dispone en el archivo de encabezado `arm_neon.h`. La compatibilidad del compilador de Visual C++ funciones intrínsecas de NEÓN se parece del compilador ARM, que se describe en el apéndice G de la [cadena de herramientas del compilador ARM, versión 4.1 compilador referencia](http://go.microsoft.com/fwlink/p/?LinkId=251083) en el sitio Web del centro de información de ARM.  
...
|_arm_umlal|UMLAL|unsigned __int64 _arm_umlal (sin signo \__int64 _RdHiLo, _Rn int sin signo, unsigned int _Rm)|  
...
|__emit||void __emit (sin signo \__int32 opcode)<br /><br /> Inserta una instrucción especificada en la secuencia de instrucciones generada por el compilador.<br /><br /> El valor de `opcode` debe ser una expresión constante que se conozca en el momento de la compilación. El tamaño de una palabra de instrucción es de 16 bits y los 16 bits más significativos de `opcode` se omiten.<br /><br /> El compilador no intenta interpretar el contenido de `opcode` y no garantiza un estado de CPU o de memoria antes de que se ejecute la instrucción insertada.<br /><br /> El compilador supone que los estados de CPU y de memoria no se modifican después de ejecutarse la instrucción insertada. Por lo tanto, las instrucciones que cambian de estado pueden tener un impacto negativo en el código normal generado por el compilador.<br /><br /> Por este motivo, utilice `emit` solo para insertar instrucciones que afectan a un estado de CPU que el compilador no procesa normalmente (por ejemplo, el estado de coprocesador) o para implementar las funciones que se declaran mediante `declspec(naked)`.|  
|__hvc|HVC|unsigned int __hvc(unsigned int, ...)|  
|__iso_volatile_load16||__int16 \__iso_volatile_load16 (const volatile \__int16 *)<br /><br /> Para obtener más información, consulte [__iso_volatile_load/almacén las características propias](#IsoVolatileLoadStore).|  
|__iso_volatile_load32||__int32 \__iso_volatile_load32 (const volatile \__int32 *)<br /><br /> Para obtener más información, consulte [__iso_volatile_load/almacén las características propias](#IsoVolatileLoadStore).|  
|__iso_volatile_load64||__int64 \__iso_volatile_load64 (const volatile \__int64 *)<br /><br /> Para obtener más información, consulte [__iso_volatile_load/almacén las características propias](#IsoVolatileLoadStore).|  
|__iso_volatile_load8||__int8 \__iso_volatile_load8 (const volatile \__int8 *)<br /><br /> Para obtener más información, consulte [__iso_volatile_load/almacén las características propias](#IsoVolatileLoadStore).|  
|__iso_volatile_store16||void __iso_volatile_store16 (volátil \__int16 *, \__int16)<br /><br /> Para obtener más información, consulte [__iso_volatile_load/almacén las características propias](#IsoVolatileLoadStore).|  
|__iso_volatile_store32||void __iso_volatile_store32 (volátil \__int32 *, \__int32)<br /><br /> Para obtener más información, consulte [__iso_volatile_load/almacén las características propias](#IsoVolatileLoadStore).|  
|__iso_volatile_store64||void __iso_volatile_store64 (volátil \__int64 *, \__int64)<br /><br /> Para obtener más información, consulte [__iso_volatile_load/almacén las características propias](#IsoVolatileLoadStore).|  
|__iso_volatile_store8||void __iso_volatile_store8 (volátil \__int8 *, \__int8)<br /><br /> Para obtener más información, consulte [__iso_volatile_load/almacén las características propias](#IsoVolatileLoadStore).|  
...
|_CopyDoubleFromInt64||doble _CopyDoubleFromInt64 (\__int64)|  
|_CopyFloatFromInt32||float _CopyFloatFromInt32 (\__int32)|  
|_CopyInt32FromFloat||__int32 _CopyInt32FromFloat(float)|  
|_CopyInt64FromDouble||__int64 _CopyInt64FromDouble(double)|  
|_CountLeadingOnes||unsigned int _CountLeadingOnes(unsigned long)|  
|_CountLeadingOnes64||unsigned int _CountLeadingOnes64 (sin signo \__int64)|  
|_CountLeadingSigns||unsigned int _CountLeadingSigns(long)|  
|_CountLeadingSigns64||int sin signo _CountLeadingSigns64 (\__int64)|  
|_CountLeadingZeros||unsigned int _CountLeadingZeros(unsigned long)|  
|_CountLeadingZeros64||unsigned int _CountLeadingZeros64 (sin signo \__int64)|  
|_CountOneBits||unsigned int _CountOneBits(unsigned long)|  
|_CountOneBits64||unsigned int _CountOneBits64 (sin signo \__int64)|  
...
|_MoveToCoprocessor64|MCRR|void _MoveToCoprocessor64 (sin signo \__int64, unsigned int, int sin signo, unsigned int)<br /><br /> Lee datos de un coprocesador ARM mediante las instrucciones de transferencia de datos del coprocesador. Para obtener más información, consulte [_MoveToCoprocessor64](#MoveToCo64).|  
...
###  <a name="IsoVolatileLoadStore"></a>características propias de __iso_volatile_load/almacén  
...
|__nop|void __nop(void) **Nota:** plataformas en ARM, esta función genera una instrucción NOP si hay una está implementada en la arquitectura de destino; en caso contrario, es una instrucción alternativa que no cambia el estado del programa o la CPU genera, por ejemplo, `MOV r8, r8`. Esto es funcionalmente equivalente a la \__nop intrínsecos de otras arquitecturas de hardware. Dado que la arquitectura de destino podría omitir una instrucción que no tiene ningún efecto en el estado de la CPU o del programa al considerarla como una optimización, la instrucción no consume necesariamente ciclos de CPU. Por lo tanto, no utilice la \__nop intrínsecas para manipular el tiempo de ejecución de una secuencia de código, a menos que esté seguro de cómo se comportará la CPU. En su lugar, puede usar el \__nop intrínseco para alinear la siguiente instrucción en una dirección específica límite de 32 bits.|  
|__yield|void __yield(void) **Nota:** plataformas en ARM, esta función genera la instrucción YIELD, lo que indica que el subproceso está realizando una tarea que se pueda suspender temporalmente de la ejecución, por ejemplo, un bloqueo por bucle: sin que afecte negativamente el programa. Esto permite que la CPU ejecute otras tareas durante los ciclos de ejecución que, de lo contrario, se desperdiciarían.|  
...
|_byteswap_uint64|__int64 sin signo \__cdecl _byteswap_uint64 (sin signo \__int64)|  
|_byteswap_ulong|unsigned long __cdecl _byteswap_ulong(unsigned long)|  
|_byteswap_ushort|unsigned short __cdecl _byteswap_ushort(unsigned short)|  
|_disable|void __cdecl _disable(void) **Nota:** en plataformas ARM, esta función genera la instrucción CPSID; solo está disponible como función intrínseca.|  
|_enable|void __cdecl _enable(void) **Nota:** en plataformas ARM, esta función genera la instrucción CPSIE; solo está disponible como función intrínseca.|  
...
|_rotl64|__int64 sin signo \__cdecl _rotl64 (sin signo \__int64 _Value, _Shift int)|  
|_rotl8|unsigned char _rotl8(unsigned char _Value, unsigned char _Shift)|  
|_rotr|unsigned int __cdecl _rotr(unsigned int _Value, int _Shift)|  
|_rotr16|unsigned short _rotr16(unsigned short _Value, unsigned char _Shift)|  
|_rotr64|__int64 sin signo \__cdecl _rotr64 (sin signo \__int64 _Value, _Shift int)|  
...
|Exchange|Partial|Parcial|Parcial|Parcial|Partial|  
...
### <a name="interlockedbittest-intrinsics"></a>Intrínsecos _interlockedbittest  
 Los intrínsecos bittest entrelazados normales son comunes a todas las plataformas. ARM agrega `_acq`, `_rel`, y `_nf` variantes, que sólo tiene que modificar la semántica de barrera de una operación, como se describe en [sufijo _nf (sin límite)](#nf_suffix) anteriormente en este artículo.  
