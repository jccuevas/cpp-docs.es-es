...
Un `CComSafeArray` objeto puede crearse a partir de una máquina `SAFEARRAYBOUND` estructura y el número de dimensiones, o especificando el número de elementos en la matriz y el límite inferior. Si la matriz es necesario acceder desde C++, el límite inferior debe ser 0. Otros lenguajes pueden permitir otros valores para el límite inferior (por ejemplo, Visual Basic admite matrices con elementos con un intervalo de -10 a 10).
