...
Esta clase ajusta la `VARIANT` tipo, proporcionando un miembro que indica el tipo de datos almacenados.  
...
## <a name="remarks"></a>Comentarios  
 `CComVariant`ajusta el `VARIANT and VARIANTARG` tipo, que consta de una unión y miembro que indica el tipo de los datos almacenados en la unión. **VARIANT**s normalmente se utilizan en la automatización.  
  
 `CComVariant`se deriva de la **VARIANT** escriba por lo que puede ser siempre utiliza un **VARIANT** se puede usar. Por ejemplo, puede utilizar el **V_VT** macro para extraer el tipo de un `CComVariant` o puede tener acceso a la **vt** miembro directamente al igual que puede hacer con un **VARIANT**.  
...
##  <a name="attach"></a>CComVariant::Attach  
 Borra de forma segura el contenido actual de la `CComVariant` objeto, copia el contenido de `pSrc` en este objeto, a continuación, Establece el tipo variant de `pSrc` a `VT_EMPTY`.  
...
##  <a name="ccomvariant"></a>CComVariant::CComVariant  
 Cada constructor controla la inicialización segura de la `CComVariant` objeto mediante una llamada a la `VariantInit` función de Win32 o estableciendo el valor del objeto y el tipo según los parámetros pasados.  
...
### <a name="parameters"></a>Parámetros  
 *varSrc*  
 [in] El `CComVariant` o `VARIANT` utilizado para inicializar la `CComVariant` objeto. El contenido de la variante de origen se copia en el destino sin conversión.  
  
 `lpszSrc`  
 [in] La cadena de caracteres que se usa para inicializar la `CComVariant` objeto. Puede pasar una cadena terminada en cero ancha (Unicode) caracteres para la **LPCOLESTR** versión del constructor, o una cadena ANSI para el `LPCSTR` versión. En cualquier caso, la cadena se convierte en Unicode `BSTR` asignados mediante **SysAllocString**. El tipo de la `CComVariant` objeto será `VT_BSTR`.  
  
 `bSrc`  
 [in] El `bool` utilizado para inicializar la `CComVariant` objeto. El `bool` argumento se convierte en un **VARIANT_BOOL** antes de almacenarse. El tipo de la `CComVariant` objeto será `VT_BOOL`.  
  
 `nSrc`  
 [in] El `int`, **bytes**, **corto**, **largo**, **LONGLONG**, **ULONGLONG**, **entero corto sin signo**, `unsigned long`, o `unsigned int` utilizado para inicializar la `CComVariant` objeto. The type of the `CComVariant` object will be `VT_I4`, `VT_UI1`, `VT_I2`, `VT_I4`, **VT_I8**, **VT_UI8**, **VT_UI2**, **VT_UI4**, or **VT_UI4**, respectively.  
 ...
 `fltSrc`  
 [in] El **float** utilizado para inicializar la `CComVariant` objeto. El tipo de la `CComVariant` objeto será `VT_R4`.  
  
 `dblSrc`  
 [in] El **doble** utilizado para inicializar la `CComVariant` objeto. El tipo de la `CComVariant` objeto será `VT_R8`.  
  
 `cySrc`  
 [in] El **CY** utilizado para inicializar la `CComVariant` objeto. El tipo de la `CComVariant` objeto será `VT_CY`.  
 ...
 `cSrc`  
 [in] El `char` utilizado para inicializar la `CComVariant` objeto. El tipo de la `CComVariant` objeto será **VT_I1**.  
...
### <a name="parameters"></a>Parámetros  
 ...
 `pSrc`  
 [in] Un puntero a la `VARIANT` cuyo valor se convertirá en el nuevo tipo. El valor predeterminado es **NULL**, lo que significa el `CComVariant` objeto se convertirá en su lugar.  
  
### <a name="return-value"></a>Valor devuelto  
 Un valor `HRESULT` estándar.  
  
### <a name="remarks"></a>Comentarios  
 Si se pasa un valor para `pSrc`, `ChangeType` utilizará **VARIANT** como el origen de la conversión. De lo contrario, la `CComVariant` objeto será el origen.  
...
### <a name="remarks"></a>Comentarios  
 El destructor llama automáticamente a **borrar**.  
  
##  <a name="copy"></a>CComVariant::Copy  
 Libera el `CComVariant` de objetos y, a continuación, le asigna una copia del **VARIANT**.  
...
### <a name="remarks"></a>Comentarios  
 Si el `VARIANT` contiene un puntero de interfaz `GetSize` consulta `IPersistStream` o `IPersistStreamInit`. Si tiene éxito, el valor devuelto es los 32 bits de orden inferior del valor devuelto por `GetSizeMax` más el `sizeof` una `CLSID` y `sizeof(VARTYPE)`. Si el puntero de interfaz es `NULL`, `GetSize` devuelve el `sizeof` una `CLSID` plus `sizeof(VARTYPE)`. Si es mayor que el tamaño total `ULONG_MAX`, `GetSize` devuelve `sizeof(VARTYPE)` que indica un error.  
  
 En los demás casos, un archivo temporal `VARIANT` de tipo `VT_BSTR` se convierte en el actual `VARIANT`. La longitud de este `BSTR` se calcula como el tamaño de la longitud de la cadena más la longitud de la propia cadena más el tamaño del carácter nulo más `sizeof(VARTYPE)`. Si el `VARIANT` no se puede convertir a un `VARIANT` de tipo `VT_BSTR`, `GetSize` devuelve `sizeof(VARTYPE)`.  
...
### <a name="parameters"></a>Parámetros  
 *varSrc*  
 [in] El `CComVariant` o [VARIANT](http://msdn.microsoft.com/en-us/e305240e-9e11-4006-98cc-26f4932d2118) que se asignará a la `CComVariant` objeto. El contenido de la variante de origen se copia en el destino sin conversión.  
  
 `bstrSrc`  
 [in] BSTR que se va a asignar a la `CComVariant` objeto. El tipo de la `CComVariant` objeto será `VT_BSTR`.  
  
 `lpszSrc`  
 [in] La cadena de caracteres que se asignará a la `CComVariant` objeto. Puede pasar una cadena terminada en cero ancha (Unicode) caracteres para la **LPCOLESTR** versión del operador o una cadena ANSI para el `LPCSTR` versión. En cualquier caso, la cadena se convierte en Unicode `BSTR` asignados mediante **SysAllocString**. El tipo de la `CComVariant` objeto será `VT_BSTR`.  
...
### <a name="remarks"></a>Comentarios  
 Devuelve **true** si el valor y tipo de *varSrc* son iguales que el valor y tipo, respectivamente, de la `CComVariant` objeto. De lo contrario, **false**. El operador utiliza la configuración regional predeterminada del usuario para realizar la comparación.  
  
 El operador compara sólo el valor de los tipos variant. Compara cadenas, enteros y puntos flotantes, pero no las matrices o registros.  
...
### <a name="remarks"></a>Comentarios  
 Devuelve **true** si el valor o el tipo de *varSrc* no es igual al valor o tipo, respectivamente, de la `CComVariant` objeto. De lo contrario, **false**. El operador utiliza la configuración regional predeterminada del usuario para realizar la comparación.  
...
### <a name="remarks"></a>Comentarios  
 Devuelve **true** si el valor de la `CComVariant` objeto es menor que el valor de *varSrc*. De lo contrario, **false**. El operador utiliza la configuración regional predeterminada del usuario para realizar la comparación.  
...
### <a name="remarks"></a>Comentarios  
 Devuelve **true** si el valor de la `CComVariant` objeto es mayor que el valor de *varSrc*. De lo contrario, **false**. El operador utiliza la configuración regional predeterminada del usuario para realizar la comparación.  
