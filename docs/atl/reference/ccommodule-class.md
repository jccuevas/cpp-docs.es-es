...
Una instancia de `CComModule` usa un mapa de objetos para mantener un conjunto de definiciones de objetos de clase. Este mapa de objetos se implementa como una matriz de `_ATL_OBJMAP_ENTRY` estructuras y contiene información de:
...
Cuando se ejecuta ATL COM AppWizard, el asistente genera automáticamente `_Module`, una instancia global de `CComModule` o una clase derivada de él. Para obtener más información sobre el Asistente para proyectos ATL, vea el artículo [crear un proyecto ATL](../../atl/reference/creating-an-atl-project.md).

Además de `CComModule`, ATL proporciona [CComAutoThreadModule](../../atl/reference/ccomautothreadmodule-class.md), que implementa un módulo de modelo de apartamento para los servicios exe y Windows. Derive el módulo de `CComAutoThreadModule` cuando desee crear objetos en varios apartamentos.
...
El método [init](#init) establece `m_hInst` en el identificador pasado a `DLLMain` o `WinMain`.
...
El método [init](#init) establece `m_hInstResource` en el identificador pasado a `DLLMain` o `WinMain`. Puede establecer explícitamente `m_hInstResource` en el identificador de un recurso.
...
El método [init](#init) establece `m_hInstTypeLib` en el identificador pasado a `DLLMain` o `WinMain`. Puede establecer explícitamente `m_hInstTypeLib` en el identificador de una biblioteca de tipos.
...
El método [UpdateRegistryClass](#updateregistryclass) llama a `RegisterClassHelper`.
...
`DLLRegisterServer` se llamará automáticamente a `RegisterServer` para un archivo DLL o `WinMain` para un EXE ejecutado con la opción de línea de comandos `/RegServer`.
...
El método [UpdateRegistryClass](#updateregistryclass) llama a `UnregisterClassHelper`.
...
`DLLUnregisterServer` se llamará automáticamente a `UnregisterServer` para un archivo DLL o `WinMain` para un EXE ejecutado con la opción de línea de comandos `/UnregServer`.
...
> [!NOTE]
>  Para sustituir los valores de reemplazo en tiempo de ejecución, no especifique el DECLARE_REGISTRY_RESOURCE ni la macro DECLARE_REGISTRY_RESOURCEID. En su lugar, cree una matriz de estructuras `_ATL_REGMAP_ENTRIES`, donde cada entrada contiene un marcador de posición de variable emparejado con un valor para reemplazar el marcador de posición en tiempo de ejecución. A continuación, llame a `UpdateRegistryFromResourceD`, pasando la matriz para el parámetro *pMapEntries* . Así se agregan todos los valores de reemplazo de las estructuras `_ATL_REGMAP_ENTRIES` al mapa de reemplazo del registrador.
...
> [!NOTE]
>  Para sustituir los valores de reemplazo en tiempo de ejecución, no especifique el [DECLARE_REGISTRY_RESOURCE](registry-macros.md#declare_registry_resource) ni la macro [DECLARE_REGISTRY_RESOURCEID](registry-macros.md#declare_registry_resourceid) . En su lugar, cree una matriz de estructuras `_ATL_REGMAP_ENTRIES`, donde cada entrada contiene un marcador de posición de variable emparejado con un valor para reemplazar el marcador de posición en tiempo de ejecución. A continuación, llame a `UpdateRegistryFromResourceS`, pasando la matriz para el parámetro *pMapEntries* . Así se agregan todos los valores de reemplazo de las estructuras `_ATL_REGMAP_ENTRIES` al mapa de reemplazo del registrador.
