...
|[CComCurrency::operator moneda](#operator_currency)|Convierte un objeto `CURRENCY`.|  
...
## <a name="remarks"></a>Comentarios  
 ...
 El objeto `CComCurrency` da acceso a los números situados a ambos lados del separador decimal en forma de dos componentes: un componente de número entero que almacena el valor situado a la izquierda del separador decimal y un componente de fracción que almacena el valor situado a la derecha del separador decimal. El componente de fracción se almacena internamente como un valor entero entre -9.999 ( **CY_MIN_FRACTION**) y + 9.999 ( **CY_MAX_FRACTION**). El método [CComCurrency:: Getfraction](#getfraction) devuelve un valor escalado por un factor de 10.000 ( **CY_SCALE**).  
  
 Cuando se especifica el entero y los componentes fraccionarios de un **CComCurrency** de objetos, recuerde que el componente de fracción es un número en el intervalo de 0 a 9999. Esto es importante cuando se trata de una divisa como el dólar estadounidense, que expresa cantidades con solo dos dígitos después del separador decimal. Aunque no se muestren los últimos dos dígitos, se deben tener en cuenta.  
 ...
 *varSrc*  
 ...
 `szSrc`  
 Una cadena ANSI o Unicode que contiene el valor inicial. La configuración regional del subproceso actual se utiliza para realizar la conversión.  
  
### <a name="remarks"></a>Comentarios  
 El constructor establece el valor inicial de [CComCurrency::m_currency](#m_currency)y acepta una amplia variedad de tipos de datos, incluidos los números enteros, cadenas, números de punto flotante, **moneda** variables y otros `CComCurrency` objetos. Si no se proporciona ningún valor, `m_currency` se establece en 0.  
...
### <a name="remarks"></a>Comentarios  
 El componente de fracción es un valor entero de 4 dígitos entre -9.999 ( **CY_MIN_FRACTION**) y + 9.999 ( **CY_MAX_FRACTION**). `GetFraction` Devuelve este valor en la escala de 10.000 ( **CY_SCALE**). Los valores de **CY_MIN_FRACTION**, **CY_MAX_FRACTION**, y **CY_SCALE** se definen en atlcur.h.  
  
### <a name="example"></a>Ejemplo  
 [!code-cpp[NVC_ATL_Utilities#50](../../atl/codesnippet/cpp/ccomcurrency-class_1.cpp)]  
  
##  <a name="getinteger"></a>  CComCurrency::GetInteger  
 Llamar a este método para obtener el componente entero de un `CComCurrency` objeto.  
...
### <a name="return-value"></a>Valor devuelto  
 Devuelve un `CComCurrency` objeto que representa el resultado de la resta. Si se produce un error, como un desbordamiento, llama a este operador `AtlThrow` con un valor HRESULT que describe el error.  
...
### <a name="return-value"></a>Valor devuelto  
 Devuelve **true** si no es igual que el elemento que se están comparando el `CComCurrency` objeto; en caso contrario, **false**.  
...
### <a name="return-value"></a>Valor devuelto  
 Devuelve un `CComCurrency` objeto que representa el resultado de la multiplicación. Si se produce un error, como un desbordamiento, llama a este operador `AtlThrow` con un valor HRESULT que describe el error.  
...
### <a name="return-value"></a>Valor devuelto  
 Devuelve la actualización `CComCurrency` objeto. Si se produce un error, como un desbordamiento, llama a este operador `AtlThrow` con un valor HRESULT que describe el error.  
...
### <a name="return-value"></a>Valor devuelto  
 Devuelve un `CComCurrency` objeto que representa el resultado de la división. Si el divisor es 0, se producirá un error en la aserción.  
...
### <a name="return-value"></a>Valor devuelto  
 Devuelve la actualización `CComCurrency` objeto. Si el divisor es 0, se producirá un error en la aserción.  
...
### <a name="return-value"></a>Valor devuelto  
 Devuelve un `CComCurrency` objeto que representa el resultado de la suma. Si se produce un error, como un desbordamiento, llama a este operador `AtlThrow` con un valor HRESULT que describe el error.  
...
### <a name="return-value"></a>Valor devuelto  
 Devuelve **true** si los objetos son iguales (es decir, el `m_currency` miembros de datos, tanto enteros y fraccionarios, tanto en objetos tienen el mismo valor), **false** en caso contrario.  
...
##  <a name="operator_currency"></a>  CComCurrency::operator moneda  
 Estos operadores se utilizan para convertir un `CComCurrency` el objeto a un **moneda** tipo de datos.  
...
##  <a name="round"></a>  CComCurrency::Round  
 Llamar a este método para redondear la moneda en un número especificado de posiciones decimales.  
...
### <a name="parameters"></a>Parámetros  
 *nFraction*  
 El valor que se asigna para el componente de fracción de la `m_currency` miembro de datos. El inicio de sesión del componente de fracción debe ser el mismo que el componente entero y el valor debe estar en el intervalo-9999 ( **CY_MIN_FRACTION**) a + 9.999 ( **CY_MAX_FRACTION**).  
...
### <a name="parameters"></a>Parámetros  
 `nInteger`  
 El valor que se asigna para el componente entero de la `m_currency` miembro de datos. El inicio de sesión del componente entero debe coincidir con el inicio de sesión del componente de fracciones existente.  
