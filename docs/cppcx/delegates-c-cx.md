...
Al declarar delegados que se expondrán a los clientes en la interfaz binaria de la aplicación Windows en tiempo de ejecución, use [Windows::Foundation::TypedEventHandler\<TSender, TResult>](/uwp/api/windows.foundation.typedeventhandler-2). Este delegado tiene binarios de código auxiliar y proxy predefinidos que permiten su uso en clientes de JavaScript.

## <a name="consuming-delegates"></a>Usar delegados

Cuando se crea una aplicación de la Plataforma universal de Windows, a menudo se trabaja con un delegado como el tipo de un evento que expone una clase de Windows en tiempo de ejecución. Para suscribirte a un evento, crea una instancia de su tipo de delegado especificando una función, o expresión lambda, que coincida con la firma de delegado. Después, usa el operador `+=` para pasar el objeto de delegado al miembro de evento de la clase. Esto se denomina suscribirse al evento. Cuando la instancia de clase "desencadena" el evento, se llama a tu función junto con cualquier otro controlador que haya sido agregado por tu objeto o por otros objetos.
...
Puede definir sus propios delegados, definir controladores de eventos o permitir que los consumidores pasen funcionalidad personalizada al componente de Windows Runtime. Al igual que cualquier otro tipo de Windows Runtime, un delegado público no se puede declarar como genérico.
...
> [!NOTE]
> Se usa el símbolo de la opción "" cuando se hace referencia al tipo de delegado, al igual que lo hace con cualquier tipo de referencia de Windows en tiempo de ejecución.

Una declaración de evento siempre tiene un tipo de delegado. En este ejemplo se muestra una firma de tipo de delegado típica en Windows Runtime:

[!code-cpp[cx_delegates#122](../cppcx/codesnippet/CPP/delegatesevents/class1.h#122)]

El evento `Click` de la clase `Windows:: UI::Xaml::Controls::Primitives::ButtonBase` es de tipo `RoutedEventHandler`. Para más información, vea [Eventos](../cppcx/events-c-cx.md).
...
En el ejemplo siguiente, una aplicación cliente pasa un delegado personalizado a un método público en `Vector`un componente de Windows Runtime que ejecuta el delegado en cada elemento de:
...
Si una aplicación cliente carga un componente de Windows Runtime que se ejecuta en un apartamento con subprocesos y proporciona un delegado a ese componente, el delegado se invoca de forma predeterminada directamente en el subproceso STA. La mayoría de los componentes de Windows Runtime se pueden ejecutar en STA o MTA.
