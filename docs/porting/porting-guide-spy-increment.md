...
## <a name="step-1-converting-the-project-file"></a><a name="convert_project_file"></a>Paso 1. Convertir el archivo del proyecto.
...
![La solución de&#43;&#43; espía](../porting/media/spyxxsolution.PNG "La solución de&#43;&#43; espía")

Tenemos dos proyectos, uno con un gran número de archivos de C++ y otro con un archivo DLL escrito en C.

## <a name="step-2-header-file-problems"></a><a name="header_file_problems"></a> Paso 2. Problemas de archivos de encabezado
...
## <a name="step-3-linker-outputfile-setting"></a><a name="linker_output_settings"></a>Paso 3. Configuración de OutputFile del vinculador
...
WINVER se establecerá en Windows 7. Es más fácil leer el código más adelante si usa la macro para Windows 7 (_WIN32_WINNT_WIN7), en lugar del propio valor (0x0601).

```cpp
#define WINVER _WINNT_WIN32_WIN7 // Minimum targeted Windows version is Windows 7
```

## <a name="step-5-linker-errors"></a><a name="linker_errors"></a>Paso 5. Errores del vinculador
...
## <a name="step-7-updating-old-iostreams-code"></a><a name="updating_iostreams_code"></a>Paso 7. Actualizar el código antiguo de iostreams
...
Aquí está el siguiente error.

```Output
error C2039: 'freeze': is not a member of 'std::basic_stringbuf<char,std::char_traits<char>,std::allocator<char>>'
```

El siguiente problema `basic_stringbuf` es que `freeze` no tiene un método. El método `freeze` se utiliza para evitar una pérdida de memoria en el antiguo `ostream`. No lo necesitamos ahora que estamos usando el nuevo `ostringstream`archivo . Podemos eliminar la llamada a `freeze`.

```cpp
//rdbuf()->freeze(0);
```

Los siguientes dos errores se produjeron en las líneas adyacentes. El primero se `ends`queja de usar `iostream` , que es el manipulador de E/S de la biblioteca anterior que agrega un terminador nulo a una cadena. El segundo de estos errores explica `str` que la salida del método no se puede asignar a un puntero no const.
...
Mediante la nueva biblioteca de secuencias, no se requiere `ends`, ya que la cadena siempre termina en null, por lo que se puede quitar esa línea. Para el segundo problema, el `str()` problema es que ahora no devuelve un puntero a la matriz de caracteres para una cadena; devuelve el `std::string` tipo. La solución al segundo problema consiste en cambiar el tipo a `LPCSTR` y utilizar el método `c_str()` para solicitar el puntero.
...
## <a name="step-8-the-compilers-more-strict-conversions"></a><a name="stricter_conversions"></a>Paso 8. Las conversiones más estrictas del compilador
...
El problema tiene que ver con la falta de coincidencia en el puntero a tipos de función de miembro. El problema no es `CHotLinkCtrl` la conversión `CWnd` de como un tipo de clase a como el tipo de clase, ya que se trata de una conversión de derivada a base válida. El problema es el tipo de valor devuelto: UINT frente a LRESULT. LRESULT se resuelve como LONG_PTR, que es un puntero de 64 o 32 bits, según el tipo binario de destino, por lo que UINT no convierte a este tipo. Esto no es raro al actualizar código escrito antes de la versión 2005, ya que el tipo de valor devuelto de muchos métodos de asignación de mensajes cambió de UINT a LRESULT en Visual Studio 2005 como parte de los cambios de compatibilidad con 64 bits. Cambiamos el tipo de valor devuelto de UINT a LRESULT en el siguiente código:
...
Puesto que hay alrededor de diez apariciones de esta función en diferentes clases derivadas de CWnd, es útil utilizar **Ir a definición** (teclado: **F12**) y **Ir a declaración** (teclado: **Ctrl**+**F12**) cuando el cursor está en la función en el editor para localizarlos y navegar a ellos desde la ventana de herramientas **Buscar símbolo.** Por lo general, **Ir a definición** es la más útil de las dos. **Ir a declaración** busca declaraciones que no sean la declaración de clase que define, como las declaraciones de la clase friend o las referencias adelantadas.

## <a name="step-9-mfc-changes"></a><a name="mfc_changes"></a>Paso 9. Cambios en MFC
...
Este código se escribió antes de que el tipo bool integrado se admitiese en Visual C++. En dicho código, BOOL era un **typedef** para **int**. El tipo **int** es un tipo **con signo,** y la representación de bits de un **int firmado** es usar el primer bit como bit de signo, por lo que un campo de bits de tipo int podría interpretarse como que representa 0 o -1, probablemente no lo que se pretendía.
...
Es posible que se pregunte si usar el **bool** de tipo estándar en todo sería útil. Muchos de los patrones de código antiguos, como el tipo BOOL, se inventaron para resolver problemas que posteriormente se resolvieron en C+++ estándar, por lo que cambiar de BOOL al tipo integrado **bool** es solo un ejemplo de tal cambio que se considera hacer después de obtener el código que se ejecuta inicialmente en la nueva versión.
...
El error LNK1181 se produce porque se ha incluido una versión de biblioteca estática obsoleta de mfc en la entrada del vinculador. Esto ya no es necesario ya que podemos vincular MFC dinámicamente, por lo que solo necesitamos quitar todas las bibliotecas estáticas MFC de la **Input** propiedad en el **vinculador** sección de las propiedades del proyecto. Este proyecto también utiliza la opción `/NODEFAULTLIB` y en su lugar enumera todas las dependencias de la biblioteca.
...
Este es un ejemplo de código que produce esto:
...
La \_macro T tiene el efecto de hacer que un literal de cadena compile como una cadena **char** o una cadena **de wchar_t,** dependiendo de la configuración de MBCS o UNICODE. Para reemplazar todas \_las cadenas con T en Visual Studio, abra primero el cuadro **Reemplazar rápido** (teclado: **Ctrl**+**F**) o Reemplazar **en archivos** (teclado: **Ctrl**+**Shift**+**H**) y, a continuación, elija la casilla **Usar expresiones regulares.** Escriba `((\".*?\")|('.+?'))` como el texto de búsqueda y `_T($1)` como el texto de reemplazo. Si ya \_tiene la macro T alrededor de algunas cadenas, este procedimiento la agregará \_de nuevo, y `#include`también podría encontrar casos en los que no desee T, como cuando se usa , por lo que es mejor usar **Reemplazar siguiente** en lugar de **Reemplazar todo**.
...
En muchos casos tuvimos que emplear la versión genérica (macro) de una API Win32, como `GetClassName` (en lugar de `GetClassNameA`). En la instrucción switch del controlador de mensajes, algunos mensajes son MBCS o Unicode específicos, en esos casos, tuvimos que cambiar el código para llamar explícitamente a la versión de MBCS, porque reemplazamos las funciones con nombre genérico con funciones específicas **A** y **W** y agregamos una macro para el nombre genérico que se resuelve en el nombre **Correcto A** o **W** en función de si UNICODE está definido.  En muchas partes del código, cuando \_cambiamos a definir UNICODE, la versión W ahora se elige incluso cuando la versión **A** es lo que se desea.
...
Como una comprobación de nuestro trabajo, debemos restablecer el **juego** de caracteres para usar el juego de caracteres **multibyte** y asegurarnos de que el código todavía se compila utilizando MBCS, así como Unicode. Sobra decir que la aplicación recompilada tras todos estos cambios debería superar una prueba completa.
...
## <a name="step-12-porting-to-use-the-secure-crt"></a><a name="porting_to_secure_crt"></a>Paso 12. Migrar para usar las funciones seguras de CRT

A continuación se realiza la portabilidad del código para utilizar las versiones seguras (las versiones con el **sufijo _s)** de las funciones CRT. En este caso, la estrategia general es reemplazar la función con la **versión _s** y, a continuación, normalmente, agregar los parámetros de tamaño de búfer adicionales necesarios. En muchos casos esto es sencillo, ya que se sabe cuál es el tamaño. En otros casos, donde el tamaño no está disponible inmediatamente, es necesario agregar parámetros adicionales a la función que usa la función CRT, o tal vez examinar el uso del búfer de destino y ver cuáles son los límites de tamaño adecuados.
...
Algunos de los casos comunes: para `memcpy`, cuando se pasa a `memcpy_s`, agregamos con frecuencia el tamaño de la estructura a la que se copia. De forma similar, para la mayoría de cadenas y búferes, el tamaño de la matriz o el búfer se determina fácilmente a partir de la declaración del búfer o encontrando la posición en la que se asignó el búfer originalmente. Para algunas situaciones, debe determinar qué tamaño de un búfer está realmente disponible y si esa información no está disponible en el ámbito de la función que está modificando, debe agregarse como un parámetro adicional y el código de llamada debe modificarse para proporcionar la información.
...
## <a name="step-13-zcforscope--is-deprecated"></a><a name="deprecated_forscope"></a>Paso 13. /Zc:forScope- está en desuso
