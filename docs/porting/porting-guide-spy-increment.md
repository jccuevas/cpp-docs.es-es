...
```  
 ...
 Microsoft ya no ofrece soporte técnico para Windows XP, así que, aunque en Visual Studio 2015 se puede seleccionar el destino, debería ir retirando progresivamente el soporte técnico para esta versión en sus aplicaciones y animar a los usuarios a adoptar nuevas versiones de Windows.  
  
 Para deshacerse del error, defina WINVER. Para ello, establezca el valor de **Propiedades del proyecto** en la versión más antigua de Windows que quiera seleccionar como destino actualmente. Vea [aquí](http://msdn.microsoft.com/library/windows/desktop/aa383745.aspx) una tabla de valores para varias versiones de Windows.  
...
```  
 ...
 Actualmente, el proyecto se compila mediante MBCS (juego de caracteres multibyte), de forma que `char` es el tipo de datos de caracteres adecuado. En cambio, para que sea más fácil actualizar el código a Unicode UTF-16, se ha actualizado a `TCHAR`, que se resuelve en `char` o `wchar_t`, en función de si la propiedad **Juego de caracteres** de la configuración del proyecto está establecida en MBCS o Unicode.  
...
```  
 ...
 El siguiente problema es que basic_stringbuf no tiene un método de inmovilización. El método de inmovilización se utiliza para evitar una pérdida de memoria en el antiguo ostream. No lo necesitamos ahora que estamos utilizando el nuevo ostringstream. Podemos eliminar la llamada a la inmovilización.  
...
```  
 ...
 Mediante la nueva biblioteca de secuencias, no se requieren extremos, ya que la cadena siempre termina en null, por lo que se puede quitar esa línea. Para el segundo problema, el problema es que ahora str() no devuelve un puntero a la matriz de caracteres de una cadena, sino que devuelve el tipo std::string. La solución al segundo problema consiste en cambiar el tipo a LPCSTR y utilizar el método c_str () para solicitar el puntero.  
...
```  
 ...
 Aunque se ha usado la función _tcscpy, que es la función strcpy de TCHAR para copiar una cadena, el búfer que se ha asignado es un búfer `char`. Esto se puede cambiar fácilmente a TCHAR.  
