...
|[CTabCtrl::Create](#create)|Crea un control de ficha y lo adjunta a una instancia de un `CTabCtrl` objeto.|
|[CTabCtrl::CreateEx](#createex)|Crea un control de ficha con los estilos extendidos de Windows especificados y lo adjunta a una `CTabCtrl` instancia de un objeto.|
...
Este control (y, por `CTabCtrl` lo tanto, la clase) solo está disponible para programas que se ejecutan en Windows 95/98 y Windows NT versión 3,51 y versiones posteriores.

Para obtener más información sobre `CTabCtrl`el uso de, vea [controles](../../mfc/controls-mfc.md) y [usar CTabCtrl](../../mfc/using-ctabctrl.md).
...
Crea un control de ficha y lo adjunta a una instancia de un `CTabCtrl` objeto.
...
*pParentWnd*<br/>
Especifica la ventana primaria del control de pestaña, normalmente `CDialog`una. No debe ser NULL.
...
Un `CTabCtrl` objeto se crea en dos pasos. En primer lugar, llame al constructor y, `Create`a continuación, llame a, que crea el control de pestaña `CTabCtrl` y lo adjunta al objeto.
...
Para crear un control de ficha con estilos de ventana extendidos, llame a [CTabCtrl:: CreateEx](#createex) en lugar de `Create`a.
...
Crea un control (una ventana secundaria) y lo asocia al `CTabCtrl` objeto.
...
`CreateEx`crea el control con los estilos extendidos de Windows especificados por *dwExStyle*. Establecer estilos extendidos específicos de un control mediante [SetExtendedStyle](#setextendedstyle). Por ejemplo, use `CreateEx` para establecer tales estilos como WS_EX_CONTEXTHELP, pero use `SetExtendedStyle` para establecer tales estilos como TCS_EX_FLATSEPARATORS. Para obtener más información, consulte los estilos descritos en [control de pestaña estilos extendidos](/windows/win32/Controls/tab-control-extended-styles) en el Windows SDK.
...
Esta función miembro implementa el comportamiento del mensaje de Win32, [TCM_DESELECTALL](/windows/win32/Controls/tcm-deselectall), tal y como se describe en el Windows SDK.
...
De forma predeterminada, esta función miembro no hace nada. Invalide esta función miembro para implementar el dibujo de un `CTabCtrl` objeto dibujado por el propietario.
...
Representa los estilos extendidos actualmente en uso para el control de pestaña. Este valor es una combinación de [estilos extendidos de control](/windows/win32/Controls/tab-control-extended-styles)de pestañas, como se describe en el Windows SDK.

### <a name="remarks"></a>Comentarios

Esta función miembro implementa el comportamiento del mensaje de Win32 [TCM_GETEXTENDEDSTYLE](/windows/win32/Controls/tcm-getextendedstyle), tal y como se describe en el Windows SDK.
...
Si es correcto, un puntero a la lista de imágenes del control de ficha; de lo contrario, es NULL.
...
*pTabCtrlItem*<br/>
Puntero a una estructura [TCITEM](/windows/win32/api/commctrl/ns-commctrl-tcitemw) , que se usa para especificar la información que se va a recuperar. También se usa para recibir información acerca de la pestaña. Esta estructura se usa con las `InsertItem`funciones `GetItem`miembro, `SetItem` y.
...
Cuando se envía el mensaje, el `mask` miembro especifica los atributos que se van a devolver. Si el `mask` miembro especifica el valor de TCIF_TEXT, `pszText` el miembro debe contener la dirección del búfer que recibe el texto del elemento y `cchTextMax` el miembro debe especificar el tamaño del búfer.

- `mask`

   Valor que especifica los `TCITEM` miembros de estructura que se van a recuperar o establecer. Este miembro puede ser cero o una combinación de los siguientes valores:
   ...
   - TCIF_RTLREADING el texto de `pszText` se muestra mediante el orden de lectura de derecha a izquierda en los sistemas hebreo o árabe.
...
- `cchTextMax`
   ...
   Tamaño del búfer al que `pszText`apunta. Este miembro se omite si la estructura no recibe información.

- `iImage`Índice en la lista de imágenes del control de pestañas o-1 si no hay ninguna imagen para la pestaña.

- `lParam`

   Datos definidos por la aplicación asociados a la pestaña. Si hay más de cuatro bytes de datos definidos por la aplicación por pestaña, una aplicación debe definir una estructura y utilizarla en lugar de la `TCITEM` estructura. El primer miembro de la estructura definida por la aplicación debe ser una estructura [TCITEMHEADER](/windows/win32/api/commctrl/ns-commctrl-tcitemheaderw). La `TCITEMHEADER` estructura es idéntica a la `TCITEM` estructura, pero sin el `lParam` miembro. La diferencia entre el tamaño de la estructura y el tamaño de la `TCITEMHEADER` estructura debe ser igual al número de bytes adicionales por pestaña.
...
Referencia a un valor DWORD que recibe la información de estado. Puede presentar uno de los siguientes valores:
...
El miembro de la `dwState` `TCITEM` estructura especifica el estado de un elemento.
...
Un control de ficha crea un control de información sobre herramientas si tiene el estilo TCS_TOOLTIPS. También puede asignar un control de información sobre herramientas a un control de ficha mediante `SetToolTips` la función miembro.
...
Esta función miembro implementa el mensaje de Win32 [TCM_HIGHLIGHTITEM](/windows/win32/Controls/tcm-highlightitem), tal y como se describe en el Windows SDK.
...
*nMask*<br/>
Especifica los `TCITEM` atributos de estructura que se van a establecer. Puede ser cero o una combinación de los siguientes valores:
...
Esta función miembro implementa el comportamiento del mensaje de Win32 [TCM_SETCURFOCUS](/windows/win32/Controls/tcm-setcurfocus), tal y como se describe en el Windows SDK.
...
Un control de ficha no envía un mensaje de notificación TCN_SELCHANGING o TCN_SELCHANGE cuando se selecciona una ficha mediante esta función. Estas notificaciones se envían mediante WM_NOTIFY cuando el usuario hace clic o usa el teclado para cambiar las pestañas.
...
Esta función miembro implementa el comportamiento del mensaje de Win32 [TCM_SETEXTENDEDSTYLE](/windows/win32/Controls/tcm-setextendedstyle), tal y como se describe en el Windows SDK.
...
*pTabCtrlItem*<br/>
Puntero a una estructura [TCITEM](/windows/win32/api/commctrl/ns-commctrl-tcitemw) que contiene los nuevos atributos de elemento. El `mask` miembro especifica los atributos que se van a establecer. Si el `mask` miembro especifica el valor de TCIF_TEXT, `pszText` el miembro es la dirección de una cadena terminada en NULL `cchTextMax` y se omite el miembro.
...
Esta función miembro implementa el comportamiento del mensaje de Win32 [TCM_SETITEMEXTRA](/windows/win32/Controls/tcm-setitemextra), tal y como se describe en el Windows SDK.
...
*dwState*<br/>
Referencia a un valor DWORD que contiene la información de estado. Puede presentar uno de los siguientes valores:
...
Esta función miembro implementa el comportamiento del mensaje de Win32 [TCM_SETMINTABWIDTH](/windows/win32/Controls/tcm-setmintabwidth), tal y como se describe en el Windows SDK.
...
Puede obtener el control de información sobre herramientas asociado a un control de ficha realizando una llamada `GetToolTips`a.
