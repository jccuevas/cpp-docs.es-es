...
## <a name="members"></a>Miembros  
 Las funciones miembro de `CStringArray` son similares a las funciones miembro de clase [CObArray](../../mfc/reference/cobarray-class.md). Debido a esta similitud, puede utilizar la documentación de referencia de `CObArray` para obtener información específica de la función miembro. Siempre que vea un `CObject` puntero como un valor devuelto, use un [CString](../../atl-mfc-shared/using-cstring.md) objeto (no un [CString](../../atl-mfc-shared/using-cstring.md) puntero). Siempre que vea un puntero `CObject` como un parámetro de función, use un `LPCTSTR`.  
