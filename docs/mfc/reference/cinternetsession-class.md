...
- `INTERNET_OPEN_TYPE_PRECONFIG` Conectar usando valores preconfigurados en el registro. Este tipo de acceso se establece como el valor predeterminado. Para conectarse a través de un proxy TIS, establezca *dwAccessType* a este valor; a continuación, establezca el registro correctamente.
...
`CInternetSession` la primera función Internet llama a una aplicación. Inicializa las estructuras internas de datos y lo prepara para futuras llamadas de la aplicación.
...
*dwFlags*  
Cualquier combinación de la `INTERNET_FLAG_*` marcas. Vea la tabla en la **comentarios** sección de [CHttpConnection:: OpenRequest](../../mfc/reference/chttpconnection-class.md#openrequest) para obtener una descripción de *dwFlags* valores.
...
- `INTERNET_FLAG_PASSIVE` Se utiliza para un sitio FTP. Utiliza la semántica FTP pasiva. Usar con [CInternetConnection](../../mfc/reference/cinternetconnection-class.md) de `OpenURL`.
...
El puntero que `OpenURL` devuelve depende *pstrURL*del tipo de servicio. La siguiente tabla muestran los posibles punteros `OpenURL` puede devolver.
