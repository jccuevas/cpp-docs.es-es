...
|[CWnd::get_accHelp](#get_acchelp)|Llamado por el marco para recuperar un objeto **ayuda** cadena de propiedad.|  
...
|[CWnd::GetNextDlgTabItem](#getnextdlgtabitem)|Recupera el primer control con el [WS_TABSTOP](window-styles.md) estilo que sigue (o anterior a) el control especificado.|  
...
|[CWnd::OpenClipboard](#openclipboard)|Abre el Portapapeles. Otras aplicaciones no podrá modificar el Portapapeles hasta que Windows [CloseClipboard](http://msdn.microsoft.com/library/windows/desktop/ms649035) se llama la función.|  
...
|[CWnd::PreSubclassWindow](#presubclasswindow)|Permite que otras subclases necesarios para que se produzca antes de [SubclassWindow](#subclasswindow) se llama.|  
...
|[CWnd::SetTimer](#settimer)|Instala un temporizador del sistema que envía una [WM_TIMER](#ontimer) mensajes cuando se desencadena.|  
...
|[CWnd:: GetCurrentMessage](#getcurrentmessage)|Devuelve un puntero al mensaje que esta ventana está procesando actualmente. Sólo se debe llamar cuando un `On` *mensaje* función de miembro de controlador de mensajes.|  
...
|[CWnd::OnCharToItem](#onchartoitem)|Llama a un cuadro de lista secundarios con el [LBS_WANTKEYBOARDINPUT](list-box-styles.md) estilo en respuesta a una [WM_CHAR](#onchar) mensaje.|  
...
|[CWnd::OnDestroyClipboard](#ondestroyclipboard)|Se llama cuando se vacía el Portapapeles mediante una llamada a Windows [EmptyClipboard](http://msdn.microsoft.com/library/windows/desktop/ms649037) (función).|  
...
|[CWnd::OnMouseLeave](#onmouseleave)|Se llama cuando el cursor abandona el área cliente de la ventana especificada en una llamada anterior a [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265).|  
...
|[CWnd::OnNcCreate](#onnccreate)|Se llama antes de [OnCreate](#oncreate) cuando se cree el área no cliente.|  
...
|[CWnd::OnUniChar](#onunichar)|Se llama cuando se presiona una tecla. Es decir, la ventana actual tiene el foco del teclado y un [WM_KEYDOWN](http://msdn.microsoft.com/library/windows/desktop/ms646280) mensaje se convierte en el [TranslateMessage](http://msdn.microsoft.com/library/windows/desktop/ms644955) (función).|  
...
|[CWnd::OnVKeyToItem](#onvkeytoitem)|Llama a un cuadro de lista propiedad `CWnd` en respuesta a una [WM_KEYDOWN](#onkeydown) mensaje.|  
...
|[CWnd::OnWindowPosChanged](#onwindowposchanged)|Se llama cuando ha cambiado el tamaño, la posición o el orden Z como resultado de una llamada a [SetWindowPos](#setwindowpos) u otra función de administración de ventanas.|  
|[CWnd::OnWindowPosChanging](#onwindowposchanging)|Se llama cuando el tamaño, la posición o el orden Z se va a cambiar como resultado de una llamada a [SetWindowPos](#setwindowpos) u otra función de administración de ventanas.|  
...
|[CWnd::operator! =](#operator_neq)|Determina si una ventana no es la misma que la ventana cuyo identificador es [m_hWnd](#m_hwnd).|  
...
## <a name="remarks"></a>Comentarios  
 Un objeto `CWnd` es distinto de una ventana de Windows, pero los dos están estrechamente vinculados. El constructor y destructor `CWnd` crea o destruye un objeto `CWnd`. La ventana de Windows, por otro lado, es una estructura de datos interna de Windows creado por un **crear** función miembro y destruya el `CWnd` destructor virtual. El [DestroyWindow](#destroywindow) función destruye la ventana de Windows sin destruir el objeto.  
  
 El `CWnd` clase y el mecanismo de asignación de mensajes ocultar la **WndProc** (función). Los mensajes entrantes de notificación de Windows automáticamente se enrutan a través de la asignación de mensajes para el propio **en***mensaje* `CWnd` funciones miembro. Invalida una **en***mensaje* función miembro para controlar el mensaje determinado de un miembro en las clases derivadas.  
 ...
 Una ventana secundaria se crea en dos pasos. En primer lugar, llame al constructor de `CWnd` para construir la `CWnd` objeto, a continuación, llame a la [crear](#create) función de miembro para crear la ventana secundaria y adjuntarlo a la `CWnd` objeto.  
 ...
 Dentro de la biblioteca MFC (Microsoft Foundation Class), se derivan más clases de `CWnd` para proporcionar tipos de ventana específicos. Muchas de estas clases, incluidos [CFrameWnd](../../mfc/reference/cframewnd-class.md), [CMDIFrameWnd](../../mfc/reference/cmdiframewnd-class.md), [CMDIChildWnd](../../mfc/reference/cmdichildwnd-class.md), [CView](../../mfc/reference/cview-class.md), y [CDialog](../../mfc/reference/cdialog-class.md), están diseñados para la derivación adicional. El control de clases derivadas de `CWnd`, como [CButton](../../mfc/reference/cbutton-class.md), se puede usar directamente o se puede usar para aún más la derivación de clases.  
...
### <a name="return-value"></a>Valor devuelto  
 Devuelve S_OK si se ejecuta correctamente, un código de error de COM en caso de error. Consulte **devuelven valores** en [:: accDoDefaultAction](http://msdn.microsoft.com/library/windows/desktop/dd318470) en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
### <a name="remarks"></a>Comentarios  
 Esta función forma parte de MFC [Active Accessibility](http://msdn.microsoft.com/library/windows/desktop/dd373592) admite.  
  
 Reemplace esta función en su `CWnd`-clase derivada para realizar la acción predeterminada. Para obtener más información, consulte [:: accDoDefaultAction](http://msdn.microsoft.com/library/windows/desktop/dd318470) en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="parameters"></a>Parámetros  
 `xLeft`  
 Coordenada X del punto hasta alcanzar probado (en unidades de pantalla).  
  
 `yTop`  
 Coordenada Y del punto hasta alcanzar probado (en unidades de pantalla).  
  
 `pvarChild`  
 Recibe la información que identifica el objeto en el punto especificado por `xLeft` y `yTop`. Consulte *pvarID* en [:: accHitTest](http://msdn.microsoft.com/library/windows/desktop/dd318471) en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
### <a name="return-value"></a>Valor devuelto  
 Devuelve S_OK si se ejecuta correctamente, un código de error de COM en caso de error. Consulte **devuelven valores** en **:: accHitTest** en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
### <a name="remarks"></a>Comentarios  
 Esta función forma parte de MFC [Active Accessibility](http://msdn.microsoft.com/library/windows/desktop/dd373592) admite.  
  
 Reemplace esta función en su `CWnd`-clase derivada si tiene elementos de interfaz de usuario nonwindowed (excepto los controles ActiveX sin ventana, que MFC controla).  
 ...
 *pcyHeight*  
 ...
 `varChild`  
 Especifica si la ubicación que se van a recuperar es que el objeto o uno de los elementos secundarios del objeto. Este parámetro puede ser CHILDID_SELF (para obtener información sobre el objeto) o un identificador secundario (para obtener información sobre el elemento secundario del objeto).  
  
### <a name="return-value"></a>Valor devuelto  
 Devuelve S_OK si se ejecuta correctamente, un código de error de COM en caso de error. Consulte **devuelven valores** en **:: accLocation** en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="parameters"></a>Parámetros  
 `navDir`  
 Especifica la dirección para navegar. Consulte `navDir` en [:: accNavigate](http://msdn.microsoft.com/library/windows/desktop/dd318473) en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
 `varStart`  
 Especifica el objeto inicial. Consulte `varStart` en **:: accNavigate** en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
 *pvarEndUpAt*  
 Recibe información sobre el objeto de interfaz de usuario de destino. Consulte *pvarEnd* en **:: accNavigate** en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
### <a name="return-value"></a>Valor devuelto  
 Devuelve S_OK si se ejecuta correctamente, un código de error de COM en caso de error. Consulte **devuelven valores** en **:: accNavigate** en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="parameters"></a>Parámetros  
 `flagsSelect`  
 Especifica cómo cambiar el foco o selección actual. Consulte `flagsSelect` en [:: accSelect](http://msdn.microsoft.com/library/windows/desktop/dd318474) en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
 `varChild`  
 Especifica el objeto que va a seleccionar. Este parámetro puede ser CHILDID_SELF (para seleccionar el objeto en Sí) o un identificador secundario (para seleccionar uno de los elementos secundarios del objeto).  
  
### <a name="return-value"></a>Valor devuelto  
 Devuelve S_OK si se ejecuta correctamente, un código de error de COM en caso de error. Consulte **devuelven valores** en **:: accSelect** en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
##  <a name="animatewindow"></a>CWnd::AnimateWindow  
 Genera los efectos especiales al mostrar u ocultar windows.  
...
### <a name="parameters"></a>Parámetros  
 *dwTime*  
 Especifica cuánto tiempo lleva a reproducir la animación, en milisegundos. Normalmente, una animación tarda 200 milisegundos para reproducir.  
...
### <a name="remarks"></a>Comentarios  
 Esta función miembro emula la funcionalidad de la función [AnimateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632669), como se describe en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="return-value"></a>Valor devuelto  
 El alto de una fila de iconos si la función se realiza correctamente; en caso contrario, 0.  
...
##  <a name="beginpaint"></a>CWnd::BeginPaint  
 Prepara `CWnd` para dibujar y rellena una `PAINTSTRUCT` estructura de datos con información sobre el dibujo.  
...
### <a name="remarks"></a>Comentarios  
 La estructura de pintura contiene una estructura de datos RECT que tenga el rectángulo más pequeño que rodea completamente la región de actualización y una marca que especifica si se ha borrado el fondo.  
  
 Establece la región de actualización del [Invalidate](#invalidate), [InvalidateRect](#invalidaterect), o [InvalidateRgn](#invalidatergn) funciones miembro y por el sistema después de tamaños, mueve, crea, se desplaza o realiza cualquier otra operación que afecte al área de cliente. Si la región de actualización está marcada para eliminación, `BeginPaint` envía una [WM_ONERASEBKGND](#onerasebkgnd) mensaje.  
  
 No llame a la `BeginPaint` función miembro excepto en respuesta a una [WM_PAINT](#onpaint) mensaje. Cada llamada a la `BeginPaint` función miembro debe tener una llamada coincidente a la [EndPaint](#endpaint) función miembro. Si el símbolo de intercalación está en el área que se va a pintar el `BeginPaint` función miembro oculta automáticamente el símbolo de intercalación para evitar que se va a borrar.  
...
##  <a name="binddefaultproperty"></a>CWnd::BindDefaultProperty  
 Enlaza simple enlazada la propiedad default del objeto de llamada (por ejemplo, un control de edición), se marca en la biblioteca de tipos para el cursor subyacente que se define mediante las propiedades del origen de datos, nombre de usuario, contraseña y SQL del control de origen de datos.  
...
### <a name="parameters"></a>Parámetros  
 ...
 `pDSCWnd`  
 Apunta a la ventana de control hosts el origen de datos al que se enlazará la propiedad. Llame a `GetDlgItem` con el identificador de recurso de la ventana de host de los controladores de dominio para recuperar este puntero.  
...
### <a name="example"></a>Ejemplo  
 ...
 [!code-cpp[1 NVC_MFC_AxDataBinding](../../mfc/reference/codesnippet/cpp/cwnd-class_6.cpp)]  
...
##  <a name="bindproperty"></a>CWnd:: BindProperty  
 Enlaza una propiedad enlazada del cursor en un control enlazado a datos (por ejemplo, un control de cuadrícula) a un control de origen de datos y registra la relación con el Administrador de enlace de MFC.  
...
### <a name="example"></a>Ejemplo  
 ...
 [!code-cpp[1 NVC_MFC_AxDataBinding](../../mfc/reference/codesnippet/cpp/cwnd-class_6.cpp)]  
[!code-cpp[NVC_MFC_AxDataBinding Nº&4;](../../mfc/reference/codesnippet/cpp/cwnd-class_9.cpp)]  
[!code-cpp[NVC_MFC_AxDataBinding&3;](../../mfc/reference/codesnippet/cpp/cwnd-class_8.cpp)]  
...
### <a name="parameters"></a>Parámetros  
 [in, out] `lpClientRect`  
 Puntero a una estructura de rectángulo. En la entrada, esta estructura contiene el rectángulo de cliente. Después de que finalice el método, esta estructura contiene el rectángulo de la ventana que puede contener el rectángulo de cliente especificado.  
  
 [in] `nAdjustType`  
 Utilice `CWnd::adjustBorder` para calcular las coordenadas de la ventana sin el `WS_EX_CLIENTEDGE` estilo; de lo contrario, utilice `CWnd::adjustOutside`.  
  
### <a name="remarks"></a>Comentarios  
 El tamaño del rectángulo calculado de ventana no incluye el espacio para una barra de menús.  
  
 Para más restricciones de uso, consulte [AdjustWindowRectEx](http://msdn.microsoft.com/library/windows/desktop/ms632667).  
...
> [!NOTE]
>  Mediante esta función miembro no tiene ningún efecto en la información sobre herramientas administrados por el código. Solo afecta el control de información sobre herramienta administrado por [CWnd:: EnableToolTips](#enabletooltips).  
...
### <a name="parameters"></a>Parámetros  
 `pAlternateOwner`  
 Puntero a una ventana alternativa respecto a la cual será centrado (más allá de la ventana primaria).  
  
### <a name="remarks"></a>Comentarios  
 Normalmente se llama desde [CDialog:: OnInitDialog](../../mfc/reference/cdialog-class.md#oninitdialog) a cuadros de diálogo center en relación con la ventana principal de la aplicación. De forma predeterminada, la función centros ventanas secundarias con respecto a su ventana primaria y las ventanas emergentes en relación con su propietario. Si no es propiedad de la ventana emergente, se centra respecto a la pantalla. Para centrar una ventana con relación a una ventana específica que no es el propietario o el elemento primario, el `pAlternateOwner` se puede establecer el parámetro en una ventana válida. Para forzar a centrar en relación con la pantalla, pasar el valor devuelto por [CWnd::GetDesktopWindow](#getdesktopwindow) como `pAlternateOwner`.  
...
##  <a name="changeclipboardchain"></a>CWnd::ChangeClipboardChain  
 Quita `CWnd` de la cadena de los visores del Portapapeles y facilita la ventana especificada por `hWndNext` el descendiente de la `CWnd` antecesor en la cadena.  
...
##  <a name="checkdlgbutton"></a>CWnd::CheckDlgButton  
 Selecciona (lugares una marca de verificación junto a) o borra (quita una marca de verificación de) un botón, o bien cambia el estado de un botón de tres estados.  
...
### <a name="parameters"></a>Parámetros  
 ...
 `nCheck`  
 Especifica la acción que se realizará. Si `nCheck` es distinto de cero, el `CheckDlgButton` función miembro coloca una marca de verificación situada junto al botón; si es 0, se quita la marca de verificación. Para los botones de tres estados, si `nCheck` es 2, el estado es indeterminado.  
...
##  <a name="checkradiobutton"></a>CWnd::CheckRadioButton  
 Selecciona (agrega una marca de verificación en) un botón de radio determinado en un grupo y borra (quita una marca de verificación) todos los demás botones del grupo.  
...
### <a name="parameters"></a>Parámetros  
 `point`  
 Especifica las coordenadas de cliente del punto se va a probar.  
...
|Valor|Significado|  
|-----------|-------------|  
|**CWP_ALL**|No omita cualquier ventanas secundarias|  
...
### <a name="return-value"></a>Valor devuelto  
 Identifica la ventana secundaria que contiene el punto. Es **NULL** si el punto especificado se encuentra fuera del área de cliente. Si el punto está dentro del área cliente, pero no está dentro de cualquier ventana secundaria, `CWnd` se devuelve.  
  
 Esta función miembro devolverá una ventana secundaria ocultos o deshabilitados que contiene el punto especificado.  
  
 Más de una ventana puede contener el punto especificado. Sin embargo, esta función sólo devuelve el `CWnd`* de la primera ventana encontrada que contiene el punto.  
...
```  
Crear BOOL virtual (LPCTSTR lpszClassName,  
    LPCTSTR lpszWindowName,  
    DWORD dwStyle,  
    RECT const aspecto rect,  
...
```  
virtual HRESULT CreateAccessibleProxy (WPARAM wParam,  
    LPARAM lParam,  
    LRESULT* pResult);
```  
...
BOOL CreateControl (REFCLSID clsid,  
    LPCTSTR pszWindowName,  
    DWORD dwStyle,  
    Const punto* ppt, tamaño const* psize,  
    CWnd* pParentWnd, UINT nID, CFile* pPersist = NULL,  
    BOOL bStorage = FALSE,  
    BSTR bstrLicKey = NULL);
```  
...
```  
void CreateGrayCaret (nWidth int,  
    int nHeight);
```  
...
```  
void CreateSolidCaret (nWidth int,  
    int nHeight);
```  
...
```  
static void DeleteTempMap() de PASCAL;
```  
...
```  
BOOL DlgDirSelect (LPTSTR puntero lpString,  
    int nIDListBox);
```  
...
```  
BOOL DlgDirSelectComboBox (LPTSTR puntero lpString,  
    int nIDComboBox);
```  
...
```  
DoDataExchange void virtual (CDataExchange * pDX);
```  
...
```  
BOOL DrawAnimatedRects (idAni int,  
...
```  
void EnableDynamicLayout (bHabilitar el BOOL = TRUE);
```  
...
```  
void EnableD2DSupport (bHabilitar el BOOL = TRUE,  
...
```  
BOOL EnableScrollBar (nSBFlags int,  
...
```  
void EnableScrollBarCtrl (nBar int,  
    BHabilitar el BOOL = TRUE);
```  
...
```  
virtual EndModalLoop void (Nresultado int);
```  
...
```  
virtual HRESULT get_accChild (variante varChild,  
...
```  
virtual HRESULT get_accDefaultAction (variante varChild,  
...
```  
virtual HRESULT get_accDescription (variante varChild,  
    BSTR* pszDescription);
```  
...
```  
virtual HRESULT get_accHelp (variante varChild,  
    BSTR* pszHelp);
```  
...
```  
virtual get_accHelpTopic HRESULT (BSTR* pszHelpFile, varChild VARIANT, long* pidTopic);
```  
...
```  
virtual HRESULT get_accKeyboardShortcut (variante varChild,  
...
```  
virtual HRESULT get_accName (variante varChild,  
    BSTR* pszName);
```  
...
```  
virtual HRESULT get_accRole (variante varChild,  
    Variant* pvarRole);
```  
...
```  
virtual HRESULT get_accState (variante varChild,  
    Variant* pvarState);
```  
...
```  
virtual HRESULT get_accValue (variante varChild,  
    BSTR* pszValue);
```  
...
```  
GetCaretPos() de PASCAL CPoint estático;
```  
...
```  
int GetCheckedRadioButton (nIDFirstButton int,  
    int nIDLastButton);
```  
...
```  
void GetClientRect (LPRECT lpRect) como const;  
```  
...
```  
const estáticos MSG * GetCurrentMessage() PASCAL;
```  
...
```  
GetDlgItemText (int nID, de tipo int  
    LPTSTR lpStr,  
    int nMaxCount) const;  
  
GetDlgItemText (int nID, de tipo int  
    CString aspecto rString) const;  
```  
...
```  
CWnd estático* GetSafeOwner (CWnd* pParent = NULL,  
    HWND* pWndTop = NULL);
```  
...
```  
int GetScrollPos (nBar int) const;  
```  
...
```  
void GetWindowRect (LPRECT lpRect) como const;  
```  
...
void GetWindowText (CString aspecto rString) const;  
```  
...
```  
virtual HtmlHelp void (DWORD_PTR dwData,  
    UINT nCmd = 0x000F);
```  
...
```  
int (LPCTSTR lpszText, del cuadro de mensajes  
...
```  
void NotifyWinEvent (eventos DWORD,  
    IdObjectType largo,  
    IdObject LONG);
```  
...
```  
afx_msg void OnActivateApp (BOOL bSecuencias de comandos ActiveX,  
    DWORD dwThreadID);
```  
...
```  
afx_msg void OnAppCommand (conquistado CWnd *,  
    UINT nCmd,  
    UINT nDevice,  
    UINT nKey);
```  
...
```  
afx_msg void OnCaptureChanged (CWnd * conquistado);
```  
...
```  
afx_msg void OnChangeUIState (UINT nAcción,  
    UINT nUIElement);
```  
...
```  
afx_msg int OnCompareItem (nIDCtl int,  
...
```  
afx_msg void OnContextMenu (conquistado CWnd *,  
    Pos CPoint);
```  
...
```  
afx_msg BOOL OnCopyData (CWnd* conquistado, COPYDATASTRUCT* pCopyDataStruct);
```  
...
```  
afx_msg HBRUSH OnCtlColor (CDC* pDC, CWnd* conquistado,  
    UINT nCtlColor);
```  
...
```  
afx_msg void OnDeleteItem (nIDCtl int,  
...
```  
virtual void OnDrawIconicThumbnailOrLivePreview (CDC aspecto dc,  
    CRect rect,  
    CSize szRequiredThumbnailSize,  
    BOOL bIsThumbnail,  
    BOOL aspecto bAlphaChannelSet);
```  
...
```  
afx_msg void OnDrawItem (nIDCtl int,  
...
```  
afx_msg void OnEnable (BOOL bHabilitar el);
```  
...
```  
afx_msg void OnInputDeviceChange (uFlag corto sin signo);
```  
...
```  
afx_msg void OnMoving (UINT entro,  
    LPRECT lpRect);
```  
...
```  
afx_msg UINT OnNotifyFormat (conquistado CWnd *,   
    UINT Ncomando);
```  
...
```  
afx_msg BOOL OnSetCursor (conquistado CWnd *,  
    UINT nHitTest,  
    UINT message);
```  
...
```  
afx_msg void OnDestroy (UINT entro,  
    LPRECT lpRect);
```  
...
```  
afx_msg void OnStyleChanged (nStyleType int,  
...
```  
afx_msg void OnStyleChanging (nStyleType int,  
    LPSTYLESTRUCT lpStyleStruct);
```  
...
```  
virtual INT_PTR OnToolHitTest (punto CPoint,  
    TOOLINFO* pTI) const;  
```  
...
```  
virtual BOOL OnTouchInput (pt CPoint,  
    nInputNumber int,  
    nInputsCount int,  
    PTOUCHINPUT pInput);
```  
...
```  
virtual BOOL OnTouchInputs (UINT nInputsCount,  
    PTOUCHINPUT pInputs);
```  
...
```  
afx_msg void OnUpdateUIState (UINT nAcción,  
    UINT nUIElement);
```  
...
```  
BOOL (operador)! = (const CWnd aspecto wnd) const;  
```  
...
```  
virtual de BOOL de PreTranslateMessage(MSG* pMsg);
```  
...
BOOL RedrawWindow (LPCRECT lpRectUpdate = NULL,  
    ...
    Marcas UINT = RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE);
```  
...
```  
int parte de RunModalLoop (DWORD dwFlags = 0);
```  
...
```  
void ScreenToClient (LPPOINT lpPoint) como const;  void ScreenToClient (LPRECT lpRect) como const;  ```  
  
### <a name="parameters"></a>Parámetros  
 `lpPoint`  
 Apunta a un [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) objeto o [estructura POINT](../../mfc/reference/point-structure1.md) que contiene las coordenadas de pantalla que se va a convertir.  
...
### <a name="remarks"></a>Comentarios  
 El `ScreenToClient` función miembro reemplaza las coordenadas de pantalla dados `lpPoint` o `lpRect` con las coordenadas de cliente. Las nuevas coordenadas son relativas a la esquina superior izquierda de la `CWnd` área cliente.  
...
##  <a name="scrollwindow"></a>CWnd::ScrollWindow  
 Desplaza el contenido del área de cliente del actual `CWnd` objeto.  
...
### <a name="parameters"></a>Parámetros  
 ...
 `lpRect`  
 Apunta a un [CRect](../../atl-mfc-shared/reference/crect-class.md) objeto o [estructura RECT](../../mfc/reference/rect-structure1.md) que especifica la parte del área de cliente que se desplazará. Si `lpRect` es **NULL**, se desplaza toda el área cliente. El símbolo de intercalación se cambia de posición si el rectángulo de cursor forma una intersección con el rectángulo de desplazamiento.  
...
### <a name="remarks"></a>Comentarios  
 Si el símbolo de intercalación está en el `CWnd` está desplazando, `ScrollWindow` oculta automáticamente el símbolo de intercalación para evitar que se va a borrar y, a continuación, restaura el símbolo de intercalación después de que finalice el desplazamiento. La posición del símbolo de intercalación se ajustará en consecuencia.  
  
 El área descubiertos por el `ScrollWindow` función miembro no se actualiza pero se combinan en actual `CWnd` región de actualización del objeto. La aplicación recibirá finalmente una [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) mensaje que le notifica que la región debe volver a pintar. Para volver a pintar el área revelaron al mismo tiempo que el desplazamiento se realiza, llame a la [UpdateWindow](#updatewindow) función miembro inmediatamente después de llamar a `ScrollWindow`.  
  
 Si `lpRect` es **NULL**, se calcula el desplazamiento por la cantidad especificada por las posiciones de las ventanas secundarias en la ventana `xAmount` y `yAmount`y zonas (sin revestir) no válidos en el `CWnd` también se calcula el desplazamiento. `ScrollWindow`es más rápido cuando `lpRect` es **NULL**.  
  
 Si `lpRect` no **NULL**, las posiciones de las ventanas secundarias no son áreas modificadas y no válidas en `CWnd` no se calcula el desplazamiento. Para evitar problemas de actualización cuando `lpRect` no **NULL**, llame a la `UpdateWindow` función miembro para volver a pintar `CWnd` antes de llamar a `ScrollWindow`.  
 ...
 *dy*  
 ...
 `lpRectClip`  
 Apunta a un `RECT` estructura que especifica el rectángulo de recorte para desplazarse. Esta estructura tiene prioridad sobre el rectángulo señalado por `lpRectScroll`. Se desplazan solo bits dentro de este rectángulo. Bits fuera de este rectángulo no se ven afectados incluso si están en el `lpRectScroll` rectángulo. Si este parámetro es **NULL**, no se realiza ningún recorte en el rectángulo de desplazamiento.  
  
 *prgnUpdate*  
 Identifica la región en la que se modifica para contener la región desplazable no invalida. Este parámetro puede ser **NULL**.  
...
- **SW_ERASE** cuando se especifica con **SW_INVALIDATE**, borra el área invalidada recién enviando un [WM_ERASEBKGND](http://msdn.microsoft.com/library/windows/desktop/ms648055) mensaje en la ventana.  
  
- **SW_INVALIDATE** invalida la región identificada por *prgnUpdate* después del desplazamiento.  
  
- **SW_SCROLLCHILDREN** desplaza todas las ventanas secundarias que se cruzan el rectángulo señalado `lpRectScroll` por el número de píxeles especificado en `dx` y *dy*. Windows envía una [WM_MOVE](http://msdn.microsoft.com/library/windows/desktop/ms632631) mensaje a todas las ventanas secundarias que se cruzan `lpRectScroll`, incluso si no se mueven. El símbolo de intercalación se cambia de posición cuando se desplaza una ventana secundaria y el rectángulo de cursor forma una intersección con el rectángulo de desplazamiento.  
  
### <a name="return-value"></a>Valor devuelto  
 El valor devuelto es **SIMPLEREGION** (región rectangular invalidado), **COMPLEXREGION** (área invalidada no rectangular, los rectángulos se superponen), o **NULLREGION** (ninguna área invalidada), si la función se realiza correctamente; en caso contrario el valor devuelto es **ERROR**.  
...
### <a name="remarks"></a>Comentarios  
 ...
 Si [SW_INVALIDATE](http://msdn.microsoft.com/library/windows/desktop/bb787593) y [SW_ERASE](http://msdn.microsoft.com/library/windows/desktop/bb787593) no se especifica, el `ScrollWindowEx` función miembro no invalida el área desplazada fuera de. Si se establece cualquiera de estas marcas, `ScrollWindowEx` invalida esta área. El área no se actualiza hasta que la aplicación llame el [UpdateWindow](http://msdn.microsoft.com/library/windows/desktop/dd145167) función miembro, llama el [RedrawWindow](http://msdn.microsoft.com/library/windows/desktop/dd162911) función miembro (especificar [RDW_UPDATENOW](http://msdn.microsoft.com/library/windows/desktop/dd162911) o [RDW_ERASENOW](http://msdn.microsoft.com/library/windows/desktop/dd162911)), o recupera el [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) mensaje desde la cola de la aplicación.  
 ...
 Si el [SW_SCROLLCHILDREN](http://msdn.microsoft.com/library/windows/desktop/bb787593) se especifica la marca, Windows no actualizará correctamente la pantalla, si se desplaza parte de una ventana secundaria. La parte de la ventana secundaria desplazado que se encuentra fuera del rectángulo de origen no se eliminan y no se volverá a dibujar correctamente en su nuevo destino. Utilice la [DeferWindowPos](http://msdn.microsoft.com/library/windows/desktop/ms632681) función de Windows a las ventanas secundarias de movimiento que no se encuentran completamente dentro del `lpRectScroll` rectángulo. El cursor se cambia de posición si la **SW_SCROLLCHILDREN** marca está establecida y el rectángulo de intercalación forma una intersección con el rectángulo de desplazamiento.  
  
 Todas las coordenadas de entrada y salidas (para `lpRectScroll`, `lpRectClip`, `lpRectUpdate`, y *prgnUpdate*) se consideran en coordenadas de cliente, independientemente de si la ventana tiene la **CS_OWNDC** o **CS_CLASSDC** estilo de clase. Utilice la [LPtoDP](http://msdn.microsoft.com/library/windows/desktop/dd145042) y [DPtoLP](http://msdn.microsoft.com/library/windows/desktop/dd162474) funciones de Windows para convertir en coordenadas lógicas y si es necesario.  
  
##  <a name="sendchildnotifylastmsg"></a>CWnd::SendChildNotifyLastMsg  
 El marco de trabajo para proporcionar un mensaje de notificación a una ventana secundaria, en la ventana principal, por lo que la ventana secundaria puede controlar una tarea llama a esta función miembro.  
...
### <a name="parameters"></a>Parámetros  
 `pResult`  
 El resultado generado por la ventana secundaria que será devuelto por la ventana primaria.  
  
### <a name="return-value"></a>Valor devuelto  
 Distinto de cero si la ventana secundaria ha controlado el mensaje enviado a su elemento primario; en caso contrario, 0.  
  
### <a name="remarks"></a>Comentarios  
 `SendChildNotifyLastMsg`enviar el mensaje actual en el origen si es un mensaje reflejado.  
  
 Para obtener más información acerca de la reflexión de mensajes, consulte [controlar mensajes reflejados](../../mfc/handling-reflected-messages.md).  
...
### <a name="remarks"></a>Comentarios  
 El `SendDlgItemMessage` función miembro no vuelve hasta que se haya procesado el mensaje.  
  
 Usando `SendDlgItemMessage` es idéntica a la obtención de un `CWnd`* para el control determinado y llamar a la [SendMessage](#sendmessage) función miembro.  
...
### <a name="remarks"></a>Comentarios  
 El **SendMessage** función miembro llama al procedimiento de ventana directamente y no vuelve hasta ese procedimiento de ventana ha procesado el mensaje. Esto es contrario a la [PostMessage](#postmessage) función de miembro, que coloca el mensaje en cola de mensajes de la ventana y devuelve inmediatamente.  
...
##  <a name="sendmessagetodescendants"></a>CWnd:: SendMessageToDescendants  
 Llame a esta función miembro para enviar el mensaje de Windows especificado para todas las ventanas secundarias.  
...
### <a name="parameters"></a>Parámetros  
 ...
 `bDeep`  
 Especifica el nivel que se va a buscar. Si **TRUE**, buscar de forma recursiva todos los elementos secundarios; si **FALSE**, buscar sólo los elementos secundarios inmediatos.  
  
 `bOnlyPerm`  
 Especifica si windows temporales recibirá el mensaje. Si **TRUE**, windows temporales pueden recibir el mensaje; si **FALSE**sólo windows permanentes recepción el mensaje. Para obtener más información sobre windows temporales vea [Nota técnica 3](../../mfc/tn003-mapping-of-windows-handles-to-objects.md).  
  
### <a name="remarks"></a>Comentarios  
 Si `bDeep` es **FALSE**, se envía el mensaje sólo para los elementos secundarios inmediatos de la ventana; de lo contrario, el mensaje se envía a todas las ventanas secundarias.  
  
 Si `bDeep` y `bOnlyPerm` son **TRUE**, la búsqueda continúa por debajo de windows temporales. En este caso, sólo se ha encontrado durante la búsqueda windows permanentes recepción el mensaje. Si `bDeep` es **FALSE**, se envía el mensaje sólo a los elementos secundarios inmediatos de la ventana.  
...
### <a name="remarks"></a>Comentarios  
 El `SetActiveWindow` función miembro debe utilizarse con cuidado, ya que permite que una aplicación aproveche arbitrariamente a través de la ventana activa y el foco de entrada. Normalmente, Windows se encarga de todas la activación.  
  
##  <a name="setcapture"></a>CWnd::SetCapture  
 Hace que se envíe a la corriente de entrada de mouse todas los `CWnd` objeto independientemente de la posición del cursor.  
...
### <a name="return-value"></a>Valor devuelto  
 Un puntero al objeto de ventana que recibió previamente todos los mouse de entrada. Es `NULL` si no hay ninguna ventana. El puntero devuelto puede ser temporal y no debe almacenarse para su uso posterior.  
  
### <a name="remarks"></a>Comentarios  
 Cuando `CWnd` ya no requiere que todas las entradas del mouse, la aplicación debe llamar a la [ReleaseCapture](http://msdn.microsoft.com/library/windows/desktop/ms646261) función de forma que otras ventanas pueden recibir la entrada del mouse.  
  
 Mientras se captura la entrada del mouse, no `WM_NCHITTEST` o `WM_SETCURSOR` se envían mensajes a la ventana activa.  
...
### <a name="parameters"></a>Parámetros  
 `point`  
 Especifica la nueva x y coordenadas (en coordenadas de cliente) del símbolo de intercalación.  
  
### <a name="remarks"></a>Comentarios  
 El `SetCaretPos` función miembro mueve el símbolo de intercalación sólo si es propiedad de una ventana en la tarea actual. `SetCaretPos`Mueve el símbolo de intercalación o no el símbolo de intercalación está oculto.  
...
### <a name="return-value"></a>Valor devuelto  
 Identificador de la ventana siguiente en la cadena del Visor del Portapapeles si se realiza correctamente. Las aplicaciones deben guardar este identificador (puede almacenarse como una variable miembro) y usarlo al responder a mensajes de cadena del Visor del Portapapeles.  
  
### <a name="remarks"></a>Comentarios  
 Una ventana que es parte de la cadena del Visor del Portapapeles debe responder a [WM_DRAWCLIPBOARD](#ondrawclipboard), [WM_CHANGECBCHAIN](#onchangecbchain), y [WM_DESTROY](#ondestroy) mensajes y pasar el mensaje a la siguiente ventana de la cadena.  
  
 Esta función miembro envía una `WM_DRAWCLIPBOARD` mensaje en la ventana. Puesto que aún no se haya obtenido el identificador de la ventana siguiente en la cadena del Visor del Portapapeles, la aplicación no debe pasar el `WM_DRAWCLIPBOARD` mensaje que recibe durante la llamada a `SetClipboardViewer`.  
  
 Para quitar el propio de la cadena del Visor del Portapapeles, una aplicación debe llamar a la [ChangeClipboardChain](#changeclipboardchain) función miembro.  
...
### <a name="return-value"></a>Valor devuelto  
 El identificador anterior de la ventana, si es correcto; en caso contrario, 0.  
...
### <a name="parameters"></a>Parámetros  
 ...
 `bSigned`  
 Especifica si el valor entero es con o sin signo. Si este parámetro es **TRUE**, `nValue` está firmado. Si este parámetro es **TRUE** y `nValue` es menor que 0, un inicio de sesión se coloca antes del primer dígito en la cadena de menos. Si este parámetro es **FALSE**, `nValue` es sin signo.  
  
### <a name="remarks"></a>Comentarios  
 `SetDlgItemInt`envía una [WM_SETTEXT](http://msdn.microsoft.com/library/windows/desktop/ms632644) mensaje en el control especificado.  
  
### <a name="example"></a>Ejemplo  
  Vea el ejemplo de [CWnd::SetDlgItemText](#setdlgitemtext).  
  
##  <a name="setdlgitemtext"></a>CWnd::SetDlgItemText  
 Establece el título o el texto de un control a la propiedad de un cuadro de diálogo o ventana.  
...
### <a name="remarks"></a>Comentarios  
 `SetDlgItemText`envía una [WM_SETTEXT](http://msdn.microsoft.com/library/windows/desktop/ms632644) mensaje en el control especificado.  
...
### <a name="remarks"></a>Comentarios  
 Entrada de teclado se dirige a la ventana, y se cambian varias indicaciones visuales para el usuario. La ventana de primer plano es la ventana con el que el usuario está trabajando actualmente. La ventana de primer plano se aplica sólo a las ventanas de nivel superior (marco ventanas o cuadros de diálogo cuadros).  
...
### <a name="return-value"></a>Valor devuelto  
 Un puntero al objeto de ventana que previamente tenía el foco de entrada. Es **NULL** si no hay ninguna ventana. El puntero devuelto puede ser temporal y no debe almacenarse.  
  
### <a name="remarks"></a>Comentarios  
 El foco de entrada dirige todas las entradas de teclado subsiguientes a esta ventana. Lo pierde cualquier ventana que previamente tenía el foco de entrada.  
  
 El `SetFocus` función miembro envía una [WM_KILLFOCUS](http://msdn.microsoft.com/library/windows/desktop/ms646282) mensaje en la ventana que pierde el foco de entrada y un [WM_SETFOCUS](http://msdn.microsoft.com/library/windows/desktop/ms646283) mensaje en la ventana que recibe el foco de entrada. También activa la ventana o en su elemento primario.  
  
 Si la ventana actual está activa pero no tiene el foco (es decir, no hay ninguna ventana tiene el foco), cualquier tecla presionada generan los mensajes de [WM_SYSCHAR](#onsyschar), [WM_SYSKEYDOWN](#onsyskeydown), o [WM_SYSKEYUP](#onsyskeyup).  
  
##  <a name="setfont"></a>CWnd::SetFont  
 Envía el `WM_SETFONT` mensaje en la ventana usar la fuente especificada.  
...
### <a name="parameters"></a>Parámetros  
 ...
 `bRedraw`  
 `TRUE`de la ventana para volver a dibujar inmediatamente después de procesa el `WM_SETFONT` mensaje; en caso contrario `FALSE`.  
  
### <a name="remarks"></a>Comentarios  
 Este método no tiene ningún efecto a menos que la ventana procesa el `WM_SETFONT` mensaje. Muchas clases MFC que derivan de `CWnd` procesar este mensaje porque están asociados a una clase de ventana predefinida que incluye un controlador de mensajes para el `WM_SETFONT` mensaje. Para utilizar este método, las clases que derivan de `CWnd` debe definir un controlador de método para el `WM_SETFONT` mensaje.  
  
##  <a name="seticon"></a>CWnd::SetIcon  
 Llame a esta función miembro para establecer el identificador de un icono concreto, identificado por `hIcon`.  
...
### <a name="parameters"></a>Parámetros  
 `crKey`  
 Puntero a un **COLORREF** valor que especifica la clave de color de transparencia que se utilizará al crear la ventana por capas. Todos los píxeles que se dibuje en la ventana en ese color será transparentes. Para generar un **COLORREF**, utilice la macro RGB.  
  
 `bAlpha`  
 Valor alfa que se utiliza para describir la opacidad de la ventana por capas. Para obtener más información, consulte el **SourceConstantAlpha** miembro de la [BLENDFUNCTION](http://msdn.microsoft.com/library/windows/desktop/dd183393) estructura. Cuando `bAlpha` es 0, la ventana es completamente transparente. Cuando `bAlpha` es 255, la ventana es opaca.  
  
 `dwFlags`  
 Especifica la acción que se realizará. Este parámetro puede ser uno o varios de los siguientes valores. Para obtener una lista de valores posibles, consulte [SetLayeredWindowAttributes](http://msdn.microsoft.com/library/windows/desktop/ms633540).  
  
### <a name="return-value"></a>Valor devuelto  
 Es distinto de cero si la función se realiza correctamente; de lo contrario, 0.  
  
### <a name="remarks"></a>Comentarios  
 Esta función miembro emula la funcionalidad de la función [SetLayeredWindowAttributes](http://msdn.microsoft.com/library/windows/desktop/ms633540), como se describe en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="return-value"></a>Valor devuelto  
 Distinto de cero si se cambia el menú; en caso contrario, 0.  
  
### <a name="remarks"></a>Comentarios  
 Hace que la ventana se vuelva a dibujar para reflejar el cambio de menú.  
  
 `SetMenu`se destruirán un menú anterior. Una aplicación debe llamar a la [CMenu::DestroyMenu](../../mfc/reference/cmenu-class.md#destroymenu) función miembro para realizar esta tarea.  
  
### <a name="example"></a>Ejemplo  
  Vea el ejemplo de [CMenu::LoadMenu](../../mfc/reference/cmenu-class.md#loadmenu).  
  
##  <a name="setowner"></a>CWnd::SetOwner  
 Establece el propietario de la ventana actual al objeto de ventana especificado.  
...
### <a name="remarks"></a>Comentarios  
 Este propietario, a continuación, puede recibir mensajes de comandos del objeto de ventana actual. De forma predeterminada, el elemento primario de la ventana actual es su propietario.  
  
 A menudo resulta útil establecer conexiones entre los objetos de ventana que no están relacionadas con la jerarquía de la ventana. Por ejemplo, [CToolBar](../../mfc/reference/ctoolbar-class.md) envía notificaciones a su propietario en lugar de a su elemento primario. Esto permite que la barra de herramientas se convierta en el elemento secundario de una ventana (por ejemplo, una ventana de aplicación de contenedor OLE) al enviar notificaciones a otra ventana (por ejemplo, la ventana de marco en contexto). Además, cuando una ventana de servidor está desactivada o activada durante en el contexto de edición, cualquier propiedad de la ventana de marco de ventana está oculta o se muestra. Esta propiedad se establece explícitamente con una llamada a `SetOwner`.  
  
 El concepto de propiedad de esta función es diferente desde el concepto de propiedad de [GetWindow](http://msdn.microsoft.com/library/windows/desktop/ms633515).  
  
##  <a name="setparent"></a>CWnd::SetParent  
 Cambia la ventana principal de una ventana secundaria.  
...
### <a name="remarks"></a>Comentarios  
 Si está visible la ventana secundaria, Windows realiza el volver a dibujar adecuado y volver a dibujar.  
...
> [!NOTE]
>  Esta función debe llamarse en un `CWnd` objeto que representa un control OLE.  
  
 Para obtener más información acerca del uso de esta función miembro con contenedores de controles ActiveX, vea el artículo [contenedores de controles ActiveX: programar controles ActiveX en un contenedor de controles ActiveX](../../mfc/programming-activex-controls-in-a-activex-control-container.md).  
  
##  <a name="setredraw"></a>CWnd::SetRedraw  
 Una aplicación llama a `SetRedraw` para permitir que los cambios se vuelva a dibujar o para impedir que los cambios que se va a dibujar.  
...
### <a name="parameters"></a>Parámetros  
 `bRedraw`  
 Especifica el estado de la marca de actualización. Si este parámetro es **TRUE**, se establece la marca de renegociación; si **FALSE**, se borra el indicador.  
  
### <a name="remarks"></a>Comentarios  
 Esta función miembro establece o borra la marca de actualización. Aunque el indicador de actualización está desactivado, el contenido no se actualizará después de cada cambio y no se actualiza hasta que se establezca el indicador de actualización. Por ejemplo, una aplicación que necesite agregar varios elementos a un cuadro de lista puede borrar la marca de actualización, agregue los elementos y, a continuación, establezca el indicador de actualización. Por último, la aplicación puede llamar a la [Invalidate](#invalidate) o [InvalidateRect](#invalidaterect) función miembro para hacer que el cuadro de lista se vuelva a dibujar.  
...
### <a name="remarks"></a>Comentarios  
 ...
 Los controladores que indican la posición de la barra de desplazamiento, de mensaje de Windows de MFC [CWnd::OnHScroll](#onhscroll) y [CWnd::OnVScroll](#onvscroll), proporcionar sólo 16 bits de datos de posición. [GetScrollInfo](#getscrollinfo) y `SetScrollInfo` proporcionar 32 bits de datos de posición de la barra de desplazamiento. Por lo tanto, una aplicación puede llamar a `GetScrollInfo` al procesamiento `CWnd::OnHScroll` o `CWnd::OnVScroll` para obtener datos de posición de la barra de desplazamiento de 32 bits.  
...
##  <a name="setscrollpos"></a>CWnd::SetScrollPos  
 Establece la posición actual de un cuadro de desplazamiento y, si se solicita, se vuelve a dibujar la barra de desplazamiento para reflejar la nueva posición del cuadro de desplazamiento.  
...
### <a name="parameters"></a>Parámetros  
 `nBar`  
 Especifica que la barra de desplazamiento se establezca. Este parámetro puede ser uno de los siguientes:  
...
- **SB_VERT** establece la posición del cuadro de desplazamiento en la barra de desplazamiento vertical de la ventana.  
 ...
 `bRedraw`  
 Especifica si se debe dibujar la barra de desplazamiento para reflejar la nueva posición del cuadro de desplazamiento. Si este parámetro es **TRUE**, se actualiza la barra de desplazamiento; si **FALSE**, no se vuelve a dibujar la barra de desplazamiento.  
  
### <a name="return-value"></a>Valor devuelto  
 La posición anterior del cuadro de desplazamiento.  
  
### <a name="remarks"></a>Comentarios  
 Configuración de `bRedraw` a **FALSE** es útil cuando la barra de desplazamiento se volverá a dibujar por una llamada subsiguiente a otra función.  
...
### <a name="parameters"></a>Parámetros  
 `nBar`  
 Especifica que la barra de desplazamiento se establezca. Este parámetro puede ser cualquiera de los siguientes valores:  
...
### <a name="remarks"></a>Comentarios  
 También puede utilizarse para ocultar o mostrar barras de desplazamiento estándar.  
 ...
 Si la llamada a `SetScrollRange` sigue inmediatamente a una llamada a la [SetScrollPos](#setscrollpos) función miembro, el `bRedraw` parámetro en el `SetScrollPos` función miembro debe ser 0 para evitar que la barra de desplazamiento que se dibuja dos veces.  
  
 El intervalo predeterminado para una barra de desplazamiento estándar es de 0 a 100. El intervalo predeterminado para un control de barra de desplazamiento está vacío (tanto la `nMinPos` y `nMaxPos` los valores son 0). La diferencia entre los valores especificados por `nMinPos` y `nMaxPos` no debe ser mayor que **INT_MAX**.  
  
##  <a name="settimer"></a>CWnd::SetTimer  
 Instala a un temporizador del sistema.  
...
### <a name="parameters"></a>Parámetros  
 `nIDEvent`  
 Especifica un identificador de temporizador es distinto de cero. Si el identificador del temporizador es único, se devuelve este mismo valor `SetTimer`. De lo contrario, `SetTimer` determina un nuevo valor y que devuelve. Para un temporizador de ventana (que tiene una función de devolución de llamada NULL), el valor debe ser único sólo para otros temporizadores de windows que están asociados a la ventana actual. Para un temporizador de devolución de llamada, el valor debe ser único para todos los temporizadores en todos los procesos. Por lo tanto, cuando se crea un temporizador de devolución de llamada, es más probable que el valor devuelto podría diferir del valor especificado.  
...
### <a name="return-value"></a>Valor devuelto  
 El identificador del temporizador del temporizador nuevo si la función se realiza correctamente. Este valor puede o no ser igual que el valor que se pasa a través de la `nIDEvent` parámetro. Una aplicación siempre debería pasar el valor devuelto a la [KillTimer](#killtimer) función miembro para eliminar el temporizador. Es distinto de cero si es correcto; de lo contrario, 0.  
  
### <a name="remarks"></a>Comentarios  
 Se especifica un valor de intervalo, y cada vez que transcurre el intervalo, el sistema registra un `WM_TIMER` mensaje a la cola de mensajes de instalación de la aplicación de instalación o pasa el mensaje a una aplicación `TimerProc` función de devolución de llamada.  
  
 El `lpfnTimer` no necesita nombre de la función de devolución de llamada `TimerProc`, pero debe declararse como static y definido como sigue.  
...
### <a name="example"></a>Ejemplo  
 Este ejemplo utiliza `CWnd::SetTimer`, `CWnd::OnTimer`, y `CWnd::KillTimer` ante `WM_TIMER` mensajes. El primer temporizador está configurado para enviar una `WM_TIMER` mensaje en la ventana de marco principal cada 2 segundos en `OnStartTimer`. El `OnTimer` controlador de eventos controla `WM_TIMER` mensajes de la ventana de marco principal. Este método provoca el altavoz emita un sonido cada 2 segundos. El segundo temporizador envía un mensaje a la función de devolución de llamada cada 3,75 segundos. `OnStopTimer`se detendrá ambos temporizadores mediante una llamada a `CWnd::KillTimer` para cada identificador de temporizador.  
...
### <a name="remarks"></a>Comentarios  
 Si una ventana secundaria no tiene un identificador de contexto de ayuda, hereda el identificador de la ventana primaria. Del mismo modo, si una ventana perteneciente no tiene un identificador de contexto de ayuda, hereda el identificador de la ventana propietaria. Esta herencia de identificadores de contexto de Ayuda permite a una aplicación establecer un solo identificador de un cuadro de diálogo y todos sus controles.  
...
##  <a name="setwindowpos"></a>CWnd:: SetWindowPos  
 Cambia el tamaño, la posición y el orden Z de ventanas secundarias, de elementos emergentes y de nivel superior.  
...
### <a name="parameters"></a>Parámetros  
 `pWndInsertAfter`  
 Identifica la `CWnd` objeto que precederá (ser mayor que) Esto `CWnd` objeto en el orden Z. Este parámetro puede ser un puntero a un `CWnd` o un **puntero** a uno de los siguientes valores:  
...
- **wndTopMost** pone windows por encima de todo no es de nivel superior de la ventana. La ventana mantiene su posición superior, incluso cuando está desactivada.  
  
- **wndNoTopMost** cambia de posición de la ventana en la parte superior de todas las ventanas no son de nivel superior (es decir, detrás de todas las ventanas de nivel superiores). Esta marca no tiene ningún efecto si la ventana ya es una ventana no es de nivel superior.  
 ...
 *y*  
 ...
 `nFlags`  
 Especifica el tamaño y opciones de posición. Este parámetro puede ser una combinación de los siguientes indicadores:  
...
- `SWP_NOACTIVATE`No se activa la ventana. Si no se establece este indicador, la ventana se activa y se mueve a la parte superior de la parte superior o el grupo no es de nivel superior (dependiendo del valor de la `pWndInsertAfter` parámetro).  
...
- `SWP_NOMOVE`Conserva la posición actual (omite el *x* y *y* parámetros).  
  
- **SWP_NOOWNERZORDER** no cambia la posición de la ventana de propietario en el orden Z.  
  
- **SWP_NOREDRAW** no volver a dibujar cambios. Si se establece este indicador, ninguna actualización de la pantalla de cualquier tipo se produce. Esto se aplica a cualquier parte de la ventana primaria identificada como resultado de la ventana que se ha movido, del área no cliente (incluido el título y barras de desplazamiento) y el área de cliente. Cuando se establece este marcador, la aplicación debe invalidar explícitamente o volver a dibujar todas las partes de la ventana y la ventana primaria que debe volver a dibujar.  
...
- `SWP_NOSIZE`Conserva el tamaño actual (omite el `cx` y `cy` parámetros).  
...
### <a name="remarks"></a>Comentarios  
 ...
 Se puede mover una ventana a la parte superior del orden Z estableciendo la `pWndInsertAfter` parámetro **aspecto wndTopMost** y asegurarse de que el `SWP_NOZORDER` marca no es establecido o estableciendo una ventana del orden Z para que se encuentra por encima de las ventanas de nivel superiores existentes. Cuando una ventana de nivel superior no se realiza más alto, las ventanas pertenecientes también se realizan más alto. No se cambian sus propietarios.  
  
 Ya no es superior si se vuelve a colocar en la parte inferior de una ventana de nivel superior ( **aspecto wndBottom**) del orden Z, o después de cualquier ventana que no sea de nivel superior. Cuando una ventana de nivel superior se realiza sin de nivel superior, todos los propietarios y las ventanas pertenecientes también contienen windows no son de nivel superior.  
  
 Si no `SWP_NOACTIVATE` ni `SWP_NOZORDER` se especifica (es decir, cuando la aplicación solicita que una ventana se activa y se colocan en el orden Z especificado simultáneamente), el valor especificado en `pWndInsertAfter` solo se usa en las siguientes circunstancias:  
  
-   Ni **aspecto wndTopMost** ni **aspecto wndNoTopMost** se especifica en el `pWndInsertAfter` parámetro.  
  
-   Esta ventana no es la ventana activa.  
  
 Una aplicación no puede activar una ventana inactiva sin también situarlo en la parte superior del orden Z. Las aplicaciones pueden cambiar el orden Z de una ventana activada sin restricciones.  
  
 Una ventana de nivel superior no puede poseer una ventana de nivel superior, pero no viceversa. Cualquier ventana (por ejemplo, un cuadro de diálogo) que pertenece a una ventana de nivel superior es efectuó una ventana de nivel superior para asegurarse de que todas pertenezcan windows permanecen por encima de su propietario.  
  
 Con las versiones 3.1 y versiones posteriores de Windows, windows pueden mover a la parte superior del orden Z y bloqueados allí estableciendo sus **WS_EX_TOPMOST** estilos. Ventana superior mantiene su posición superior, incluso cuando está desactivado. Por ejemplo, seleccionando el comando WinHelp siempre visible hace que la ventana de Ayuda de nivel superior y, a continuación, sigue estando visible cuando se devuelva a la aplicación.  
  
 Para crear una ventana de nivel superior, llame a `SetWindowPos` con el `pWndInsertAfter` igual al parámetro **aspecto wndTopMost**, o establecer el **WS_EX_TOPMOST** al crear la ventana de estilo.  
  
 Si el orden Z contiene todas las ventanas con la **WS_EX_TOPMOST** estilo, una ventana que se mueven con el **aspecto wndTopMost** valor se coloca en la parte superior de todas las ventanas no son de nivel superior, pero por debajo de las ventanas de nivel superiores. Cuando una aplicación activa una ventana inactiva sin el **WS_EX_TOPMOST** de bits, la ventana se mueve por encima de todas las ventanas no son de nivel superior, pero por debajo de las ventanas de nivel superiores.  
  
 Si `SetWindowPos` se llama cuando el `pWndInsertAfter` parámetro es **aspecto wndBottom** y `CWnd` es una ventana de nivel superior, la ventana pierde su estado más alto ( **WS_EX_TOPMOST** está desactivada), y el sistema coloca la ventana en la parte inferior del orden Z.  
  
### <a name="example"></a>Ejemplo  
 [!code-cpp[NVC_MFCWindowing&#120;](../../mfc/reference/codesnippet/cpp/cwnd-class_61.cpp)]  
  
##  <a name="setwindowrgn"></a>CWnd::SetWindowRgn  
 Llame a esta función miembro para establecer la región de la ventana.  
...
### <a name="parameters"></a>Parámetros  
 ...
 `bRedraw`  
 Si **TRUE**, el sistema operativo dibuja de nuevo la ventana después de establecer la región; en caso contrario, no es así. Normalmente, se establece `bRedraw` a **TRUE** si la ventana está visible. Si establece en **TRUE**, el sistema envía el `WM_WINDOWPOSCHANGING` y `WM_WINDOWPOSCHANGED` mensajes en la ventana.  
...
### <a name="remarks"></a>Comentarios  
 Las coordenadas de la región de la ventana de la ventana son relativas a la esquina superior izquierda de la ventana, no al área cliente de la ventana.  
  
 Después de una llamada correcta a `SetWindowRgn`, el sistema operativo posee la región especificada por el identificador de la región `hRgn`. El sistema operativo no hacer una copia de la región, por lo que no hace más llamadas de función con el identificador de región y no cierre el identificador de la región.  
...
##  <a name="showcaret"></a>CWnd::ShowCaret  
 Muestra el símbolo de intercalación en la pantalla en la posición del símbolo de intercalación.  
...
### <a name="remarks"></a>Comentarios  
 ...
 El `ShowCaret` función miembro muestra el símbolo de intercalación sólo si tiene una forma actual y no ha ocultado dos o más veces consecutivamente. Si el símbolo de intercalación no pertenece a esta ventana, no se muestra el símbolo de intercalación.  
  
 Ocultar el símbolo de intercalación es acumulativa. Si el [HideCaret](#hidecaret) función miembro se llamó cinco veces consecutivamente, `ShowCaret` debe llamarse cinco veces para mostrar el símbolo de intercalación.  
  
 El símbolo de intercalación es un recurso compartido. La ventana debe mostrar el símbolo de intercalación sólo cuando tiene el foco de entrada o activo.  
...
### <a name="parameters"></a>Parámetros  
 `bShow`  
 Especifica si las ventanas emergentes se deben mostrarse u ocultarse. Si este parámetro es **TRUE**, se muestran todas las ventanas emergentes ocultas. Si este parámetro es **FALSE**, se ocultan todas las ventanas emergentes visibles.  
...
### <a name="remarks"></a>Comentarios  
 `ShowWindow`debe llamarse una vez por aplicación para la ventana principal con [CWinApp::m_nCmdShow](../../mfc/reference/cwinapp-class.md#m_ncmdshow). Las llamadas subsiguientes a `ShowWindow` debe usar uno de los valores enumerados anteriormente en lugar del especificado por `CWinApp::m_nCmdShow`.  
  
### <a name="example"></a>Ejemplo  
  Vea el ejemplo de [CWnd::CalcWindowRect](#calcwindowrect).  
  
##  <a name="subclassdlgitem"></a>CWnd:: SubclassDlgItem  
 Llamar a esta función miembro para "subclase dinámicamente" un control creado a partir de una plantilla de cuadro de diálogo y adjuntar a este `CWnd` objeto.  
...
### <a name="parameters"></a>Parámetros  
 ...
 `pParent`  
 Principal el control (normalmente un cuadro de diálogo).  
  
### <a name="return-value"></a>Valor devuelto  
 Es distinto de cero si la función se realiza correctamente; de lo contrario, es 0.  
  
### <a name="remarks"></a>Comentarios  
 Cuando un control tiene subclase dinámicamente, mensajes de windows se enrutará a través de la `CWnd`del mapa de mensajes y llamar a los controladores de mensajes en el `CWnd`de la clase en primer lugar. Mensajes que se pasan a la clase base se pasará al controlador de mensajes de forma predeterminada en el control.  
  
 Esta función miembro asocia el control de Windows para una `CWnd` de objetos y reemplaza el control **WndProc** y **AfxWndProc** funciones. La función almacena la antigua **WndProc** en la ubicación devuelta por la `GetSuperWndProcAddr` función miembro.  
...
##  <a name="unlockwindowupdate"></a>CWnd::UnlockWindowUpdate  
 Llame a esta función miembro para desbloquear una ventana que se ha bloqueado con `CWnd::LockWindowUpdate`.  
...
### <a name="remarks"></a>Comentarios  
 Sólo una ventana a la vez puede bloquear mediante `LockWindowUpdate`. Consulte [CWnd::LockWindowUpdate](#lockwindowupdate) o la función de Win32 [LockWindowUpdate](http://msdn.microsoft.com/library/windows/desktop/dd145034) para obtener más información sobre el bloqueo de windows.  
  
##  <a name="unsubclasswindow"></a>CWnd::UnsubclassWindow  
 Llame a esta función miembro para establecer **WndProc** a su valor original y separar la ventana identificada por `HWND` desde el **CWnd** objeto.  
...
### <a name="remarks"></a>Comentarios  
 ...
 El marco de trabajo llama a esta función miembro para los controles en barras de cuadro de diálogo o barras de herramientas como parte de la aplicación de procesamiento en inactividad.  
...
### <a name="parameters"></a>Parámetros  
 `pDCDst`  
 Puntero a un contexto de dispositivo para la pantalla. Se utiliza para la coincidencia de color de paleta cuando se actualiza el contenido de la ventana. Si `pDCDst` es **NULL**, se utilizará la paleta predeterminada.  
  
 If `pDCSrc` is **NULL**, `pDCDst` must be **NULL**.  
  
 `pptDst`  
 Un puntero a un **punto** estructura que especifica la posición de la nueva pantalla de la ventana por capas. Si no cambia la posición actual, `pptDst` puede ser **NULL**.  
  
 *psize*  
 Puntero a un **tamaño** estructura que especifica el nuevo tamaño de la ventana por capas. Si no se cambia el tamaño de la ventana, *psize* puede ser **NULL**.  
  
 If `pDCSrc` is **NULL**, *psize* must be **NULL**.  
  
 `pDCSrc`  
 Puntero a un controlador de dominio para la superficie que define la ventana por capas. Si no va a cambiar la forma y el contexto visual de la ventana, `pDCSrc` puede ser **NULL**.  
 ...
 *pblend*  
 Puntero a un [BLENDFUNCTION](http://msdn.microsoft.com/library/windows/desktop/dd183393) estructura que especifica el valor de transparencia que se utilizará al crear la ventana por capas.  
  
 `dwFlags`  
 Especifica la acción que se realizará. Este parámetro puede ser uno o varios de los siguientes valores. Para obtener una lista de valores posibles, consulte [UpdateLayeredWindow](http://msdn.microsoft.com/library/windows/desktop/ms633556).  
  
### <a name="return-value"></a>Valor devuelto  
 Es distinto de cero si la función se realiza correctamente; de lo contrario, 0.  
  
### <a name="remarks"></a>Comentarios  
 Esta función miembro emula la funcionalidad de la función [UpdateLayeredWindow](http://msdn.microsoft.com/library/windows/desktop/ms633556), como se describe en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="remarks"></a>Comentarios  
 El `UpdateWindow` función miembro envía una `WM_PAINT` mensaje directamente, omitiendo la cola de la aplicación. Si está vacía, la región de actualización `WM_PAINT` no se envía.  
  
### <a name="example"></a>Ejemplo  
 [!code-cpp[NVC_MFCWindowing&#124;](../../mfc/reference/codesnippet/cpp/cwnd-class_65.cpp)]  
  
##  <a name="validaterect"></a>CWnd::ValidateRect  
 Valida el área de cliente dentro del rectángulo especificado quitando el rectángulo de la región de actualización de la ventana.  
...
### <a name="parameters"></a>Parámetros  
 `lpRect`  
 Apunta a un [CRect](../../atl-mfc-shared/reference/crect-class.md) objeto o [estructura RECT](../../mfc/reference/rect-structure1.md) que contiene las coordenadas de cliente del rectángulo que se quita de la región de actualización. Si `lpRect` es **NULL**, se valida toda la ventana.  
  
### <a name="remarks"></a>Comentarios  
 El [BeginPaint](#beginpaint) función miembro valida automáticamente toda el área cliente. Ni el `ValidateRect` ni [ValidateRgn](#validatergn) función miembro debe llamarse si una parte de la región de actualización debe validarse antes de [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) se genera a continuación.  
...
##  <a name="validatergn"></a>CWnd::ValidateRgn  
 Valida el área del cliente dentro de la región mediante la eliminación de la región de la región de actualización actual de la ventana.  
...
### <a name="parameters"></a>Parámetros  
 `pRgn`  
 Un puntero a un [CRgn](../../mfc/reference/crgn-class.md) objeto que identifica una región que define el área que se va a quitar de la región de actualización. Si este parámetro es **NULL**, se quita todo el área cliente.  
  
### <a name="remarks"></a>Comentarios  
 La región determinada debe haber creado previamente una función de la región. Las coordenadas de la región se supone que las coordenadas de cliente.  
  
 El [BeginPaint](#beginpaint) función miembro valida automáticamente toda el área cliente. Ni el [ValidateRect](#validaterect) ni `ValidateRgn` función miembro debe llamarse si una parte de la región de actualización debe validarse antes del siguiente [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) genera el mensaje.  
...
### <a name="remarks"></a>Comentarios  
 `WindowFromPoint`no recupera una ventana oculta o deshabilitada, incluso si el punto está dentro de la ventana. Una aplicación debe utilizar el [ChildWindowFromPoint](#childwindowfrompoint) función miembro de una búsqueda no restrictivos.  
...
### <a name="parameters"></a>Parámetros  
 ...
 `nCmd`  
 Especifica el tipo de ayuda solicitado. Para obtener una lista de valores posibles y cómo afectan a la `dwData` parámetro, consulte la [WinHelp](http://msdn.microsoft.com/library/windows/desktop/bb762267) Windows funcionen en el [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="parameters"></a>Parámetros  
 `bRegister`  
 `TRUE`indica el registro de Windows touch compatibilidad; `FALSE` en caso contrario.  
