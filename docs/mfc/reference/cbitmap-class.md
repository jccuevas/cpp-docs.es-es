...
|[CBitmap::CreateBitmapIndirect](#createbitmapindirect)|Inicializa el objeto con un mapa de bits con el ancho, el alto y el patrón de bits (si se especifica alguno) `BITMAP` en una estructura.|
...
|[CBitmap::FromHandle](#fromhandle)|Devuelve un puntero a un `CBitmap` objeto cuando se proporciona un identificador a un `HBITMAP` mapa de bits de Windows.|
...
|NOMBRE|DESCRIPCIÓN|
|----------|-----------------|
|[CBitmap:: Operator HBITMAP](#operator_hbitmap)|Devuelve el identificador de Windows asociado al `CBitmap` objeto.|

## <a name="remarks"></a>Comentarios

Para usar un `CBitmap` objeto, construya el objeto, adjunte un identificador de mapa de bits a él con una de las funciones miembro de inicialización y, a continuación, llame a las funciones miembro del objeto.

Para obtener más información sobre el uso de `CBitmap`objetos gráficos como, vea [objetos gráficos](../../mfc/graphic-objects.md).
...
Para obtener más información, vea la descripción del `bmBits` campo en la `BITMAP` estructura. La estructura [BITMAP](/windows/desktop/api/wingdi/ns-wingdi-tagbitmap) se describe en la función miembro [CBitmap::CreateBitmapIndirect](#createbitmapindirect) .
...
*lpBitmap*<br/>
Apunta a una estructura de [mapa de bits](/windows/desktop/api/wingdi/ns-wingdi-tagbitmap) que contiene información sobre el mapa de bits.
...
Aunque un mapa de bits no se puede seleccionar directamente para un dispositivo de visualización, puede seleccionarse como el mapa de bits actual para un contexto de dispositivo de memoria mediante [CDC:: SelectObject](../../mfc/reference/cdc-class.md#selectobject) y copiarse en cualquier contexto de dispositivo compatible mediante el uso de CDC [:: bitblt](../../mfc/reference/cdc-class.md#bitblt) o [CDC:: Función StretchBlt](../../mfc/reference/cdc-class.md#stretchblt) . (La función [CDC::P atblt](../../mfc/reference/cdc-class.md#patblt) puede copiar el mapa de bits para el pincel actual directamente en el contexto de dispositivo de pantalla).

Si la `BITMAP` estructura a la que apunta el parámetro *lpBitmap* se ha rellenado con `GetObject` la función, no se especifican los bits del mapa de bits y el mapa de bits no se inicializa. Para inicializar el mapa de bits, una aplicación puede usar una función como [CDC:: bitblt](../../mfc/reference/cdc-class.md#bitblt) o [SetDIBits](/windows/desktop/api/wingdi/nf-wingdi-setdibits) para copiar los bits del mapa de bits identificado por el `CGdiObject::GetObject` primer parámetro de en el `CreateBitmapIndirect`mapa de bits creado por.

Cuando termine con el objeto `CBitmap` creado con `CreateBitmapIndirect` la función, seleccione primero el mapa de bits fuera del contexto del dispositivo y, `CBitmap` a continuación, elimine el objeto.
...
Dado que un contexto de dispositivo de memoria de color puede tener los mapas de bits de color o monocromo seleccionados, el formato `CreateCompatibleBitmap` del mapa de bits devuelto por la función no es siempre el mismo. sin embargo, el formato de un mapa de bits compatible para un contexto de dispositivo que no es de memoria siempre está en el formato del dispositivo.

Cuando termine con el `CBitmap` objeto creado con la `CreateCompatibleBitmap` función, seleccione primero el mapa de bits fuera del contexto del dispositivo y, a `CBitmap` continuación, elimine el objeto.
...
Cuando termine con el `CBitmap` objeto creado con la `CreateDiscardableBitmap` función, seleccione primero el mapa de bits fuera del contexto del dispositivo y, a `CBitmap` continuación, elimine el objeto.

##  <a name="fromhandle"></a>  CBitmap::FromHandle

Devuelve un puntero a un `CBitmap` objeto cuando se proporciona un identificador a un mapa de bits de Windows GDI.
...
*pBitMap*<br/>
Puntero a una estructura de [mapa de bits](/windows/desktop/api/wingdi/ns-wingdi-tagbitmap) que recibirá las propiedades de la imagen. Este parámetro no debe ser NULL.
...
Ancho y alto del mapa de bits, medido en unidades 0,1-milímetro. El alto está en el `cy` miembro `CSize` del objeto y el ancho está en el `cx` miembro. Si el ancho y el alto del mapa de bits no se `SetBitmapDimension`han establecido mediante, el valor devuelto es 0.
...
El mapa de bits cargado se adjunta `CBitmap` al objeto.
...
Puede usar la función [CGdiObject::D eleteobject](../../mfc/reference/cgdiobject-class.md#deleteobject) para eliminar el mapa de bits cargado `LoadBitmap` por la función o `CBitmap` el destructor eliminará el objeto automáticamente.
...
*lpColorMap*<br/>
Puntero a una `COLORMAP` estructura que contiene la información de color necesaria para asignar los mapas de bits. Si este parámetro es NULL, la función utiliza la asignación de colores predeterminada.
...
Para obtener información sobre cómo crear un mapa de bits asignado, vea la función de Windows [CreateMappedBitmap](https://go.microsoft.com/fwlink/p/?linkid=230562) y la estructura [COLORMAP](/windows/desktop/api/commctrl/ns-commctrl-colormap) en el Windows SDK.
...
Utilice este operador para obtener el identificador de GDI de Windows asociado `CBitmap` del objeto.
...
Si es correcto, identificador del objeto GDI de Windows representado por el `CBitmap` objeto; de lo contrario, NULL.

### <a name="remarks"></a>Comentarios

Este operador es un operador de conversión, que admite el uso directo `HBITMAP` de un objeto.

Para obtener más información sobre el uso de objetos gráficos, vea [objetos gráficos](/windows/desktop/gdi/graphic-objects) en el Windows SDK.
...
*lpBits*<br/>
Apunta a la matriz de bytes que contiene los valores de píxel que se `CBitmap` van a copiar en el objeto. Para que el mapa de bits pueda representar su imagen correctamente, se debe dar formato a los valores para ajustarse a los valores de alto, ancho y profundidad de color que se especificaron cuando se creó la instancia de CBitmap. Para obtener más información, vea [CBitmap:: CreateBitmap](#createbitmap).
...
Dimensiones de mapa de bits anteriores. El alto está en `cy` la variable miembro `CSize` del objeto y el ancho está en la `cx` variable miembro.
