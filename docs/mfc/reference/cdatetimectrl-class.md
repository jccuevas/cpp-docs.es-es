...
|[CDateTimeCtrl::Create](#create)|Crea el control de selector de fecha y hora y lo adjunta al `CDateTimeCtrl` objeto.|
...
|[CDateTimeCtrl::GetMonthCalCtrl](#getmonthcalctrl)|Recupera el `CMonthCalCtrl` objeto asociado al control de selector de fecha y hora.|
...
|[CDateTimeCtrl::GetTime](#gettime)|Recupera la hora seleccionada actualmente de un control de selector de fecha y hora y la coloca en una estructura `SYSTEMTIME` especificada.|
...
Crea el control de selector de fecha y hora y lo adjunta al `CDateTimeCtrl` objeto.
...
1. Llame a [CDateTimeCtrl](#cdatetimectrl) para construir `CDateTimeCtrl` un objeto.

1. Llame a esta función miembro, que crea el control selector de fecha y hora de Windows y lo adjunta al `CDateTimeCtrl` objeto.
...
Esta función miembro implementa el comportamiento del mensaje de Win32 [DTM_GETMCCOLOR](/windows/win32/Controls/dtm-getmccolor), tal y como se describe en el Windows SDK.
...
Recupera el `CMonthCalCtrl` objeto asociado al control de selector de fecha y hora.
...
*pMinRange*<br/>
Un puntero a un objeto [COleDateTime](../../atl-mfc-shared/reference/coledatetime-class.md) o un objeto [ctime](../../atl-mfc-shared/reference/ctime-class.md) que contiene la primera hora permitida en `CDateTimeCtrl` el objeto.

*pMaxRange*<br/>
Un puntero a un `COleDateTime` objeto o un `CTime` objeto que contiene la última hora permitida `CDateTimeCtrl` en el objeto.
...
Esta función miembro implementa el comportamiento del mensaje de Win32 [DTM_GETRANGE](/windows/win32/Controls/dtm-getrange), tal y como se describe en el Windows SDK. En la implementación de MFC, puede especificar `COleDateTime` los usos o. `CTime`
...
Recupera la hora seleccionada actualmente de un control de selector de fecha y hora y la coloca en una estructura `SYSTEMTIME` especificada.
...
En la primera versión, es distinto de cero si la hora se escribe correctamente `COleDateTime` en el objeto; de lo contrario, es 0. En la segunda y tercera versiones, un valor DWORD igual al miembro *dwFlag* establecido en la estructura [NMDATETIMECHANGE](/windows/win32/api/commctrl/ns-commctrl-nmdatetimechange) . Vea la sección **comentarios** que aparece a continuación para obtener más información.

### <a name="remarks"></a>Comentarios

Esta función miembro implementa el comportamiento del mensaje de Win32 [DTM_GETSYSTEMTIME](/windows/win32/Controls/dtm-getsystemtime), tal y como se describe en el Windows SDK. En la implementación de MFC `GetTime`de, puede usar `COleDateTime` las `CTime` clases o, o puede usar una `SYSTEMTIME` estructura para almacenar la información de hora.
...
Esta función miembro implementa el comportamiento del mensaje de Win32 [DTM_SETFORMAT](/windows/win32/Controls/dtm-setformat), tal y como se describe en el Windows SDK.
...
Esta función miembro implementa el comportamiento del mensaje de Win32 [DTM_SETMCCOLOR](/windows/win32/Controls/dtm-setmccolor), tal y como se describe en el Windows SDK.
...
Esta función miembro implementa el comportamiento del mensaje de Win32 [DTM_SETMCFONT](/windows/win32/Controls/dtm-setmcfont), tal y como se describe en el Windows SDK.
...
> [!NOTE]
>  Si usa este código, querrá crear un miembro de la `CDialog`clase derivada de denominada *m_MonthFont* de tipo. `CFont`
...
Esta función miembro implementa el comportamiento del mensaje de Win32 [DTM_SETRANGE](/windows/win32/Controls/dtm-setrange), tal y como se describe en el Windows SDK. En la implementación de MFC, puede especificar `COleDateTime` los usos o. `CTime` Si el `COleDateTime` objeto tiene un estado null, se quitará el intervalo. Si el `CTime` puntero o el `COleDateTime` puntero es null, se quitará el intervalo.
...
Esta función miembro implementa el comportamiento del mensaje de Win32 [DTM_SETSYSTEMTIME](/windows/win32/Controls/dtm-setsystemtime), tal y como se describe en el Windows SDK. En la implementación de MFC `SetTime`de, puede usar las `COleDateTime` clases `CTime` o, o puede usar una `SYSTEMTIME` estructura para establecer la información de hora.
