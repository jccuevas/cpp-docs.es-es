...
Para obtener más información sobre estos servicios de objeto en tiempo de ejecución y la serialización [, vea el artículo CObject (clase): Obtener acceso a la información](../../mfc/accessing-run-time-class-information.md)de clase en tiempo de ejecución.
...
|[RUNTIME_CLASS](#runtime_class)|Devuelve la `CRuntimeClass` estructura que corresponde a la clase con nombre.|
...
|[DECLARE_OLECTLTYPE](#declare_olectltype)|Declara las `GetUserTypeNameID` funciones miembro `GetMiscStatus` y de la clase del control.|
...
|[IMPLEMENT_OLECTLTYPE](#implement_olectltype)|Implementa las `GetUserTypeNameID` funciones miembro `GetMiscStatus` y de la clase del control.|
...
Agrega la capacidad de obtener acceso a la información en tiempo de ejecución sobre la clase de un objeto al `CObject`derivar una clase de.
...
Si usa las macros DECLARE_ dinámica y IMPLEMENT_DYNAMIC como se describe, puede usar la macro RUNTIME_CLASS y la `CObject::IsKindOf` función para determinar la clase de los objetos en tiempo de ejecución.

Si DECLARE_DYNAMIC se incluye en la declaración de clase, entonces IMPLEMENT_DYNAMIC debe estar incluido en la implementación de la clase.
...
Permite que los `CObject`objetos de las clases derivadas de se creen dinámicamente en tiempo de ejecución.
...
Si DECLARE_DYNCREATE se incluye en la declaración de clase, entonces IMPLEMENT_DYNCREATE debe estar incluido en la implementación de la clase.
...
Declara las `GetUserTypeNameID` funciones miembro `GetMiscStatus` y de la clase del control.
...
Use la `DECLARE_PROPPAGEIDS` macro al final de la declaración de clase. A continuación, en el archivo. cpp que define las funciones miembro de la clase, use `BEGIN_PROPPAGEIDS` la macro, las entradas de macro para cada una de las páginas de propiedades `END_PROPPAGEIDS` de su control y la macro para declarar el final de la lista de páginas de propiedades.
...
Genera el C++ código de encabezado necesario para `CObject`una clase derivada de que se puede serializar.
...
Si DECLARE_SERIAL se incluye en la declaración de clase, se debe incluir IMPLEMENT_SERIAL en la implementación de la clase.

La macro DECLARE_SERIAL incluye toda la funcionalidad de DECLARE_DYNAMIC y DECLARE_DYNCREATE.

Puede usar la macro AFX_API para exportar automáticamente el `CArchive` operador de extracción para las clases que usan las macros DECLARE_SERIAL e IMPLEMENT_SERIAL. Corchete las declaraciones de clase (que se encuentran en el archivo. h) con el código siguiente:
...
Genera el C++ código necesario para una clase `CObject`derivada dinámica con acceso en tiempo de ejecución al nombre y la posición de la clase dentro de la jerarquía.
...
Use la macro IMPLEMENT_DYNAMIC en un módulo. cpp y, a continuación, vincule el código del objeto resultante una sola vez.
...
Permite que los `CObject`objetos de las clases derivadas de se creen dinámicamente en tiempo de ejecución cuando se usan con la macro DECLARE_DYNCREATE.
...
Si usa las macros DECLARE_DYNCREATE y IMPLEMENT_DYNCREATE, puede usar la macro RUNTIME_CLASS y la `CObject::IsKindOf` función miembro para determinar la clase de los objetos en tiempo de ejecución.
...
Implementa las `GetUserTypeNameID` funciones miembro `GetMiscStatus` y de la clase del control.
...
La `GetUserTypeNameID` función miembro devuelve la cadena de recurso que identifica la clase de control. `GetMiscStatus`Devuelve los bits OLEMISC para el control. Esta enumeración especifica una colección de valores de configuración que describen las características varias del control. Para obtener una descripción completa de la configuración de OLEMISC, consulte [OLEMISC](/windows/win32/api/oleidl/ne-oleidl-olemisc) en el Windows SDK.

> [!NOTE]
>  Los valores de configuración predeterminados que usa ActiveX ControlWizard son los siguientes: OLEMISC_ACTIVATEWHENVISIBLE, OLEMISC_SETCLIENTSITEFIRST, OLEMISC_INSIDEOUT, OLEMISC_CANTLINKINSIDE y OLEMISC_RECOMPOSEONRESIZE.
...
Usar la macro IMPLEMENT_SERIAL en un módulo. cpp; a continuación, vincule solo una vez el código del objeto resultante.
...
RUNTIME_CLASS devuelve un puntero a una estructura [CRuntimeClass](../../mfc/reference/cruntimeclass-structure.md) para la clase especificada por *CLASS_NAME*. Solo `CObject`las clases derivadas de que se declaran con DECLARE_DYNAMIC, DECLARE_DYNCREATE o DECLARE_SERIAL devolverán punteros a una `CRuntimeClass` estructura.
...
Permite crear objetos `CCmdTarget`de clases derivadas de mediante la automatización OLE.
...
Si DECLARE_OLECREATE se incluye en la declaración de clase, entonces IMPLEMENT_OLECREATE debe estar incluido en la implementación de la clase. Una declaración de clase que use DECLARE_OLECREATE también debe usar DECLARE_DYNCREATE o DECLARE_SERIAL.
...
> [!NOTE]
>  Si usa IMPLEMENT_OLECREATE, de forma predeterminada, solo se admite el modelo de subprocesos único. Si usa IMPLEMENT_OLECREATE_FLAGS, puede especificar qué modelo de subprocesos admite el objeto mediante el parámetro *nFlags* .
