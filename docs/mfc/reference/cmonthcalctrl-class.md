...
|Nombre|Descripción|
|----------|-----------------|
|[CMonthCalCtrl:: Create](#create)|Crea un control de calendario mensual y lo adjunta al objeto `CMonthCalCtrl`.|
...
- Hacer clic en el texto hoy para mostrar el día actual (si no se usa el estilo de MCS_NOTODAY).
...
1. Llame a [CMonthCalCtrl](../../mfc/reference/cmonthcalctrl-class.md) para construir un objeto de `CMonthCalCtrl`.

1. Llame a esta función miembro, que crea un control de calendario mensual y lo adjunta al objeto `CMonthCalCtrl`.

Cuando se llama a `Create`, se inicializan los controles comunes. La versión de `Create` a la que se llama determina su tamaño:
...
Este método envía el mensaje de [MCM_GETCALENDARBORDER](/windows/win32/Controls/mcm-getcalendarborder) , que se describe en el Windows SDK.
...
Este método envía el mensaje de [MCM_GETCALENDARCOUNT](/windows/win32/Controls/mcm-getcalendarcount) , que se describe en el Windows SDK.
...
Este método envía el mensaje de [MCM_GETCALENDARGRIDINFO](/windows/win32/Controls/mcm-getcalendargridinfo) , que se describe en el Windows SDK.

### <a name="example"></a>Ejemplo

En el ejemplo de código siguiente se define la variable, `m_monthCalCtrl`, que se usa para tener acceso mediante programación al control de calendario mensual. Esta variable se utiliza en el siguiente ejemplo.

[!code-cpp[NVC_MFC_CMonthCalCtrl_s1#9](../../mfc/reference/codesnippet/cpp/cmonthcalctrl-class_2.h)]

### <a name="example"></a>Ejemplo

En el ejemplo de código siguiente se usa el método `GetCalendarGridInfo` para recuperar la fecha del calendario que se muestra en el control de calendario mensual actual.
...
Este método envía el mensaje de [MCM_GETCALID](/windows/win32/Controls/mcm-getcalid) , que se describe en el Windows SDK.
...
Este método envía el mensaje de [MCM_GETCURRENTVIEW](/windows/win32/Controls/mcm-getcurrentview) , que se describe en el Windows SDK.
...
Esta función miembro implementa el comportamiento del [MCM_GETCURSEL](/windows/win32/Controls/mcm-getcursel)de mensajes de Win32, como se describe en el Windows SDK.

> [!NOTE]
>  Se produce un error en esta función miembro si se establece el MCS_MULTISELECT de estilo.

En la implementación de `GetCurSel`de MFC, puede especificar un uso de `COleDateTime`, un uso de `CTime` o un uso de la estructura de `SYSTEMTIME`.
...
Esta función miembro implementa el comportamiento del [MCM_GETFIRSTDAYOFWEEK](/windows/win32/Controls/mcm-getfirstdayofweek)de mensajes de Win32, como se describe en el Windows SDK. Los días de la semana se representan como enteros, como se indica a continuación.
...
Esta función miembro implementa el comportamiento del [MCM_GETMAXSELCOUNT](/windows/win32/Controls/mcm-getmaxselcount)de mensajes de Win32, como se describe en el Windows SDK. Utilice esta función miembro para los controles con el conjunto de estilos MCS_MULTISELECT.
...
En el ejemplo de código siguiente se muestra el método `GetMaxTodayWidth`.
...
Este método envía el mensaje de [MCM_GETMAXTODAYWIDTH](/windows/win32/Controls/mcm-getmaxtodaywidth) , que se describe en el Windows SDK.
...
Si es correcto, esta función miembro devuelve un valor distinto de cero y `lpRect` recibe la información de límite aplicable. Si no se realiza correctamente, la función miembro devuelve 0.

### <a name="remarks"></a>Observaciones

Esta función miembro implementa el comportamiento del [MCM_GETMINREQRECT](/windows/win32/Controls/mcm-getminreqrect)de mensajes de Win32, como se describe en el Windows SDK.
...
Esta función miembro implementa el comportamiento del [MCM_GETMONTHDELTA](/windows/win32/Controls/mcm-getmonthdelta)de mensajes de Win32, como se describe en el Windows SDK.
...
*pMaxRange*<br/>
Puntero a una estructura de `SYSTEMTIME` que contiene la fecha en el extremo superior del intervalo.
...
Esta función miembro implementa el comportamiento del [MCM_GETMONTHRANGE](/windows/win32/Controls/mcm-getmonthrange)de mensajes de Win32, como se describe en el Windows SDK. En la implementación de `GetMonthRange`de MFC, puede especificar `COleDateTime` uso, un uso de `CTime` o un uso de la estructura de `SYSTEMTIME`.
...
*pMinRange*<br/>
Un puntero a un objeto `COleDateTime`, un objeto `CTime` o una estructura [SYSTEMTIME](/windows/win32/api/minwinbase/ns-minwinbase-systemtime) que contiene la fecha en el extremo inferior del intervalo.

*pMaxRange*<br/>
Un puntero a un objeto `COleDateTime`, un objeto `CTime` o una estructura [SYSTEMTIME](/windows/win32/api/minwinbase/ns-minwinbase-systemtime) que contiene la fecha en el extremo superior del intervalo.
...
Esta función miembro implementa el comportamiento del [MCM_GETRANGE](/windows/win32/Controls/mcm-getrange)de mensajes de Win32, como se describe en el Windows SDK. En la implementación de `GetRange`de MFC, puede especificar un uso de `COleDateTime`, un uso de `CTime` o un uso de la estructura de `SYSTEMTIME`.
...
Esta función miembro implementa el comportamiento del [MCM_GETSELRANGE](/windows/win32/Controls/mcm-getselrange)de mensajes de Win32, como se describe en el Windows SDK. `GetSelRange` producirá un error si se aplica a un control de calendario mensual que no utilice el estilo de MCS_MULTISELECT.

En la implementación de `GetSelRange`de MFC, puede especificar `COleDateTime` uso, un uso de `CTime` o un uso de la estructura de `SYSTEMTIME`.
...
Esta función miembro implementa el comportamiento del [MCM_GETTODAY](/windows/win32/Controls/mcm-gettoday)de mensajes de Win32, como se describe en el Windows SDK. En la implementación de `GetToday`de MFC, puede especificar un uso de `COleDateTime`, un uso de `CTime` o un uso de la estructura de `SYSTEMTIME`.
...
Este método envía el mensaje de [MCM_GETCURRENTVIEW](/windows/win32/Controls/mcm-getcurrentview) , que se describe en el Windows SDK. Si ese mensaje devuelve MCMV_CENTURY, este método devuelve TRUE.
...
Este método envía el mensaje de [MCM_GETCURRENTVIEW](/windows/win32/Controls/mcm-getcurrentview) , que se describe en el Windows SDK. Si ese mensaje devuelve MCMV_DECADE, este método devuelve TRUE.
...
Este método envía el mensaje de [MCM_GETCURRENTVIEW](/windows/win32/Controls/mcm-getcurrentview) , que se describe en el Windows SDK. Si ese mensaje devuelve MCMV_MONTH, este método devuelve TRUE.
...
Este método envía el mensaje de [MCM_GETCURRENTVIEW](/windows/win32/Controls/mcm-getcurrentview) , que se describe en el Windows SDK. Si ese mensaje devuelve MCMV_YEAR, este método devuelve TRUE.
...
Este método envía el mensaje de [MCM_SETCALENDARBORDER](/windows/win32/Controls/mcm-setcalendarborder) , que se describe en el Windows SDK.
...
Este método envía el mensaje de [MCM_SETCALID](/windows/win32/Controls/mcm-setcalid) , que se describe en el Windows SDK.
...
En el ejemplo de código siguiente se establece el control de calendario mensual para mostrar el calendario de la era del Emperador japonés. El método `SetCalID` se realiza correctamente solo si ese calendario está instalado en el equipo.
...
Esta función miembro implementa el comportamiento del [MCM_SETCOLOR](/windows/win32/Controls/mcm-setcolor)de mensajes de Win32, como se describe en el Windows SDK.
...
|Parámetro|Descripción|
|---------------|-----------------|
|*dwNewView*|de Uno de los siguientes valores que especifica una vista mensual, anual, década u Century.<br /><br /> MCMV_MONTH: vista mensual<br /><br /> MCMV_YEAR: vista anual<br /><br /> MCMV_DECADE: vista de década<br /><br /> MCMV_CENTURY: vista Century|
...
Este método envía el mensaje de [MCM_SETCURRENTVIEW](/windows/win32/Controls/mcm-setcurrentview) , que se describe en el Windows SDK.
...
Esta función miembro implementa el comportamiento del [MCM_SETCURSEL](/windows/win32/Controls/mcm-setcursel)de mensajes de Win32, como se describe en el Windows SDK. En la implementación de `SetCurSel`de MFC, puede especificar un uso de `COleDateTime`, un uso de `CTime` o un uso de la estructura de `SYSTEMTIME`.
...
Esta función miembro implementa el comportamiento del [MCM_SETDAYSTATE](/windows/win32/Controls/mcm-setdaystate)de mensajes de Win32, como se describe en el Windows SDK.
...
Esta función miembro implementa el comportamiento del [MCM_SETFIRSTDAYOFWEEK](/windows/win32/Controls/mcm-setfirstdayofweek)de mensajes de Win32, como se describe en el Windows SDK.
...
Esta función miembro implementa el comportamiento del [MCM_SETMAXSELCOUNT](/windows/win32/Controls/mcm-setmaxselcount)de mensajes de Win32, como se describe en el Windows SDK.
...
Esta función miembro implementa el comportamiento del [MCM_SETMONTHDELTA](/windows/win32/Controls/mcm-setmonthdelta)de mensajes de Win32, como se describe en el Windows SDK.
...
*pMaxRange*<br/>
Un puntero a un objeto `COleDateTime`, un objeto `CTime` o una estructura `SYSTEMTIME` que contiene la fecha en el extremo superior del intervalo.
...
Esta función miembro implementa el comportamiento del [MCM_SETRANGE](/windows/win32/Controls/mcm-setrange)de mensajes de Win32, como se describe en el Windows SDK. En la implementación de `SetRange`de MFC, puede especificar `COleDateTime` uso, un uso de `CTime` o un uso de la estructura de `SYSTEMTIME`.
...
Esta función miembro implementa el comportamiento del [MCM_SETSELRANGE](/windows/win32/Controls/mcm-setselrange)de mensajes de Win32, como se describe en el Windows SDK. En la implementación de `SetSelRange`de MFC, puede especificar `COleDateTime` uso, un uso de `CTime` o un uso de la estructura de `SYSTEMTIME`.
...
Esta función miembro implementa el comportamiento del [MCM_SETTODAY](/windows/win32/Controls/mcm-settoday)de mensajes de Win32, como se describe en el Windows SDK.
...
La llamada a `SizeMinReq` muestra correctamente el control de calendario mensual completo para el calendario de un mes.
...
Este método envía el mensaje de [MCM_SIZERECTTOMIN](/windows/win32/Controls/mcm-sizerecttomin) , que se describe en el Windows SDK.
