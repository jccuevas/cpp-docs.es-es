...
## <a name="members"></a>Miembros  
 Las funciones miembro de `CMapWordToOb` son similares a las funciones miembro de clase [CMapStringToOb](../../mfc/reference/cmapstringtoob-class.md). Debido a esta similitud, puede utilizar la documentación de referencia de `CMapStringToOb` para obtener información específica de la función miembro. Siempre que vea un `CString` o un **const** puntero a `char` como un parámetro de función o un valor devuelto, sustituir **WORD**.  
...
## <a name="remarks"></a>Comentarios  
 ...
 Si se necesita un volcado de individuales **WORD** -  `CObject` elementos, debe establecer la profundidad del contexto de volcado en 1 o superior.  
  
 Cuando un `CMapWordToOb` se elimina el objeto, o cuando se quitan sus elementos, el `CObject` se quitan los punteros. Los objetos referenciados por la `CObject` punteros no se destruyen.  
