...
ms.locfileid: "70907837"
---
# <a name="ccmdui-class"></a>CCmdUI (clase)

Solo se usa dentro de `ON_UPDATE_COMMAND_UI` un controlador en `CCmdTarget`una clase derivada de.
...
|[CCmdUI::SetRadio](#setradio)|Como la `SetCheck` función miembro, pero funciona en grupos de radio.|
...
|[CCmdUI::m_pMenu](#m_pmenu)|Apunta al menú representado por el `CCmdUI` objeto.|
|[CCmdUI::m_pOther](#m_pother)|Apunta al objeto de ventana que envió la notificación.|
|[CCmdUI::m_pSubMenu](#m_psubmenu)|Apunta al submenú contenido representado por el `CCmdUI` objeto.|

## <a name="remarks"></a>Comentarios

`CCmdUI`no tiene una clase base.

Cuando un usuario de la aplicación extrae un menú, cada elemento de menú debe saber si debe mostrarse como habilitado o deshabilitado. El destino de un comando de menú proporciona esta información implementando un controlador ON_UPDATE_COMMAND_UI. Para cada uno de los objetos de interfaz de usuario de comandos de la aplicación, use el [Asistente para clases](mfc-class-wizard.md) o la ventana **propiedades** (en **vista de clases**) para crear una entrada de mapa de mensajes y un prototipo de función para cada controlador.

Cuando se extrae el menú, el marco de trabajo busca y llama a cada controlador ON_UPDATE_COMMAND_UI, cada `CCmdUI` controlador llama a funciones `Enable` miembro `Check`como y, y el marco de trabajo muestra, en consecuencia, cada elemento de menú.

Un elemento de menú se puede reemplazar por un botón de barra de control u otro objeto de interfaz de usuario de comandos sin cambiar `ON_UPDATE_COMMAND_UI` el código del controlador.

En la tabla siguiente se resumen las `CCmdUI`funciones miembro del efecto de los distintos elementos de la interfaz de usuario de comandos.
...
|Botón de barra de herramientas|Habilita o deshabilita|Selecciona, anula la selección o indeterminado.|Igual a `SetCheck`|(No aplicable)|
|Panel de barra de estado|Hace que el texto sea visible o invisible|Establece el borde emergente o normal|Igual a `SetCheck`|Establece el texto del panel|
|Botón normal en`CDialogBar`|Habilita o deshabilita|Casilla comprobaciones o desactivaciones|Igual a `SetCheck`|Establece el texto del botón|
...
Se trata de una función miembro avanzada que se debe usar junto con un controlador ON_COMMAND_EX que devuelve FALSE. Para obtener más información, vea la [Nota técnica 6](../../mfc/tn006-message-maps.md).
...
Identificador del elemento de menú, botón de la barra de herramientas u otro objeto de interfaz de usuario `CCmdUI` representado por el objeto.
...
Índice del elemento de menú, botón de la barra de herramientas u otro objeto de interfaz de usuario `CCmdUI` representado por el objeto.
...
Puntero (de `CMenu` tipo) al menú representado por el `CCmdUI` objeto.
...
Puntero (de `CMenu` tipo) al submenú contenido representado por el `CCmdUI` objeto.
...
Puntero (de tipo `CWnd`) al objeto de ventana, como una herramienta o barra de estado, que envió la notificación.
...
NULL si el elemento es un menú o un objeto que `CWnd` no es de.
