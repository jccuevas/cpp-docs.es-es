 ...
 *elemName*  
 Un LPCWSTR que contiene el identificador del elemento HTML de origen del evento, o NULL para administrar eventos del documento.  
...
##  <a name="dhtml_event_axcontrol"></a>  DHTML_EVENT_AXCONTROL  
 Controla el evento identificado por *dispid* desencadenados por el control ActiveX identificado por *nombrecontrol*.  
...
### <a name="remarks"></a>Comentarios  
 ...
 Mapas de eventos incrustado dentro de un mapa de eventos DHTML y la dirección URL consisten en una macro BEGIN_EMBED_DHTML_EVENT_MAP seguida [DHTML_EVENT](#dhtml_event) macros y un [END_EMBED_DHTML_EVENT_MAP](#end_embed_dhtml_event_map) macro.  
  
 Cada mapa de eventos incrustado requiere correspondiente [entrada de evento de la dirección URL](#url_event_entry) para asignar *Nombredemapa* (especificada en BEGIN_EMBED_DHTML_EVENT_MAP) a un recurso de dirección URL o HTML.  
...
### <a name="remarks"></a>Comentarios  
 Dado que un cuadro de diálogo DHTML varias páginas consta de varias páginas HTML, las entradas de eventos de dirección URL se usan para asignar las direcciones URL o HTML recursos a correspondientes [incrustado mapas de eventos DHTML](#begin_embed_dhtml_event_map). Poner las macros URL_EVENT_ENTRY entre BEGIN_URL_ENTRIES y [END_URL_ENTRIES](#end_url_entries) macros.  
