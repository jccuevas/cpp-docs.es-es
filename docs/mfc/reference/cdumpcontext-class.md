...
|[CDumpContext::HexDump](#hexdump)|Volcados de memoria de bytes que contiene una matriz en formato hexadecimal.|  
...
## <a name="remarks"></a>Comentarios  
 ...
 Puede usar [afxDump](http://msdn.microsoft.com/library/4b3cfa3f-fb75-456a-9d99-a5601acbcb11), declarar un con anterioridad `CDumpContext` objeto para la mayor parte de su proceso de volcado. La `afxDump` objeto sólo está disponible en la versión de depuración de la biblioteca Microsoft Foundation Class.  
  
 Varios de la memoria [servicios de diagnóstico](../../mfc/reference/diagnostic-services.md) utilizar `afxDump` para su salida.  
  
 En el entorno de Windows, el resultado de la `afxDump` objeto, conceptualmente similar a la `cerr` de flujo, se enruta al depurador mediante la función de Windows **OutputDebugString**.  
  
 El `CDumpContext` clase tiene una inserción sobrecargada ( ** << **) operador para `CObject` punteros que vuelque los datos del objeto. Si necesita un formato de volcado personalizado para un objeto derivado, reemplace [CObject::Dump](../../mfc/reference/cobject-class.md#dump). Implementa la mayoría de las clases de Microsoft Foundation un invalidado `Dump` función miembro.  
  
 Clases no derivadas de `CObject`, como `CString`, `CTime`, y `CTimeSpan`, tienen sus propios sobrecargados `CDumpContext` operadores de inserción, como estructuras utilizadas con frecuencia como **CFileStatus**, `CPoint`, y `CRect`.  
  
 Si utiliza la [IMPLEMENT_DYNAMIC](../../mfc/reference/run-time-object-model-services.md#implement_dynamic) o [IMPLEMENT_SERIAL](../../mfc/reference/run-time-object-model-services.md#implement_serial) macro en la implementación de la clase, a continuación, `CObject::Dump` imprimirá el nombre de la `CObject`-clase derivada. De lo contrario, se imprimirá `CObject`.  
  
 El `CDumpContext` clase está disponible con las versiones de depuración y lanzamiento de la biblioteca, pero la `Dump` función miembro solo se define en la versión de depuración. Utilice **#ifdef _DEBUG**  /  `#endif` instrucciones para determinar el código de diagnóstico, incluidos personalizado `Dump` funciones miembro.  
...
### <a name="remarks"></a>Comentarios  
 ...
 No escribir en subyacente `CFile` mientras el contexto de volcado de memoria está activo; en caso contrario, interfiere con el volcado de memoria. En el entorno de Windows, se enruta la salida al depurador mediante la función de Windows **OutputDebugString**.  
...
### <a name="parameters"></a>Parámetros  
 *lpszLine*  
 Una cadena de salida en el inicio de una nueva línea.  
  
 *pby*  
 Puntero a un búfer que contiene los bytes que se va a volcar.  
  
 `nBytes`  
 El número de bytes que se va a volcar.  
  
 `nWidth`  
 Número máximo de bytes que se vuelca por línea (no el ancho de la línea de salida).  
...
### <a name="remarks"></a>Comentarios  
 Se sobrecarga el operador de inserción para `CObject` punteros, así como para los tipos más primitivos. Puntero al carácter da como resultado un volcado del contenido de cadena; un puntero a `void` da como resultado un volcado hexadecimal de la dirección solo. Un **LONGLONG** da como resultado un volcado de entero de 64 bits con signo; Un **ULONGLONG** da como resultado un volcado de un entero de 64 bits sin signo.  
  
 Si utiliza la `IMPLEMENT_DYNAMIC` o `IMPLEMENT_SERIAL` macro en la implementación de la clase y, a continuación, el operador de inserción a través de `CObject::Dump`, imprimirá el nombre de la `CObject`-clase derivada. De lo contrario, se imprimirá `CObject`. Si reemplaza el `Dump` función de la clase, a continuación, puede proporcionar un resultado más significativo de contenido del objeto en lugar de un volcado hexadecimal.  
...
### <a name="remarks"></a>Comentarios  
 Si el volcado de un tipo primitivo o simple `CObject` que no contiene punteros a otros objetos, a continuación, basta con un valor de 0. Un valor mayor que 0 especifica un volcado profunda donde todos los objetos son vuelca de forma recursiva. Por ejemplo, un volcado de profundidad de una colección volcará todos los elementos de la colección. Puede utilizar otros valores específicos de profundidad en las clases derivadas.  
  
> [!NOTE]
>  Las referencias circulares no se detectan en profundidad volcados y pueden dar lugar a bucles infinitos.  
  
### <a name="example"></a>Ejemplo  
 [!code-cpp[NVC_MFC_Utilities Nº&16;](../../mfc/codesnippet/cpp/cdumpcontext-class_6.cpp)]  
