...
### <a name="remarks"></a>Comentarios  
 El `COleControl` objeto señalado por *pControl* utiliza [abiertos](../../mfc/reference/cdatapathproperty-class.md#open) y recuperar las clases derivadas. Si *pControl* es **NULL**, el control que se utiliza con `Open` debe establecerse con [SetControl](../../mfc/reference/cdatapathproperty-class.md#setcontrol). Si *lpszPath* es **NULL**, se puede pasar en la ruta de acceso a través de `Open` o establézcalo con [SetPath](../../mfc/reference/cdatapathproperty-class.md#setpath).  
