...
|Vea lo que el control y el Asistente para controles ActiveX que se ofrecen sin agregar una línea de su propio código.|Compilar el control ActiveX y probarlo con Internet Explorer o el [ejemplo TSTCON](../overview/visual-cpp-samples.md).|El control de ejecución tiene la capacidad de cambiar de tamaño y mover. También tiene un **cuadro acerca de** método (si se eligen) que se puede invocar.|
