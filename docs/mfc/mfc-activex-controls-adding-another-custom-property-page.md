...
1. En el cuadro **nombre de clase** del [Asistente para clases MFC](reference/mfc-add-class-wizard.md), escriba un nombre para la nueva clase de cuadro de diálogo. (En este ejemplo, `CAddtlPropPage` ).
...
1. En. Archivo CPP de la nueva clase de página de propiedades (en este ejemplo, `CAddtlPropPage` ) modifique el de `CAddtlPropPage::CAddtlPropPageFactory::UpdateRegistry` modo que [AfxOleRegisterPropertyPageClass](reference/registering-ole-controls.md#afxoleregisterpropertypageclass)pase IDS_SAMPLE_ADDPAGE, como en el ejemplo siguiente:

   [!code-cpp[NVC_MFC_AxUI#33](codesnippet/cpp/mfc-activex-controls-adding-another-custom-property-page_2.cpp)]

1. Modifique el constructor de `CAddtlPropPage` para que IDS_SAMPLE_ADDPPG_CAPTION se pase al `COlePropertyPage` constructor, como se indica a continuación:

   [!code-cpp[NVC_MFC_AxUI#34](codesnippet/cpp/mfc-activex-controls-adding-another-custom-property-page_3.cpp)]

Después de realizar las modificaciones necesarias, recompile el proyecto y use el contenedor de prueba para probar la nueva página de propiedades. Consulte [Probar propiedades y eventos con un contenedor de prueba](testing-properties-and-events-with-test-container.md) para obtener información acerca de cómo acceder al contenedor de prueba.
