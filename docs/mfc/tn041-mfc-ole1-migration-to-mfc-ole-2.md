...
En MFC/OLE1, `COleClientItem::GetBounds` y `SetBounds` se usaron para consultar y manipular el alcance de un elemento (la `left` y `top` los miembros eran siempre cero). En MFC/OLE 2 más directamente es compatible con `COleClientItem::GetExtent` y `SetExtent`, que tratar con un **tamaño** o `CSize` en su lugar.
...
- Zoom para true comportamiento WYSIWYG en relación con el contenedor.
