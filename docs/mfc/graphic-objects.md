...
Cada clase de objeto gráfico en la biblioteca de clases tiene un operador de conversión que convierte un objeto MFC en el identificador de Windows asociado. El identificador resultante es válido hasta que el objeto asociado lo desconecta. Utilice la función miembro del `Detach` objeto para separar el identificador.
...
1. Defina un objeto gráfico en el marco de pila. Inicialice el objeto con la función de creación específica del tipo, como `CreatePen`. Como alternativa, inicialice el objeto en el constructor. Vea la explicación de la [creación de una y dos etapas,](../mfc/one-stage-and-two-stage-construction-of-objects.md)que proporciona código de ejemplo.

1. [Seleccione el objeto en el contexto del dispositivo actual,](../mfc/selecting-a-graphic-object-into-a-device-context.md)guardando el objeto gráfico antiguo que se seleccionó antes.
