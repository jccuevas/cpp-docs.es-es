...
ms.locfileid: "36953007"
---
# <a name="tn031-control-bars"></a>TN031: Barras de control
> [!NOTE]
>  La nota técnica siguiente no se ha actualizado desde que se incluyó por primera vez en la documentación en línea. Como resultado, algunos procedimientos y temas podrían estar obsoletos o ser incorrectos. Para obtener información más reciente, se recomienda buscar el tema de interés en el índice de la documentación en línea.  
  
 Esta nota describe las clases de barra de control de MFC: general [CControlBar](#_mfcnotes_ccontrolbar), [CStatusBar](#_mfcnotes_cstatusbar), [CToolBar](#_mfcnotes_ctoolbar), [CDialogBar](#_mfcnotes_cdialogbar)y `CDockBar`.  
  
## <a name="_mfcnotes_ccontrolbar"></a> CControlBar 
  
 A `ControlBar` es un `CWnd`-deriva la clase que:  
...
- Pin, CBRS_TOP (valor predeterminado) en la barra de controles a la parte superior.  
  
- Pin, CBRS_BOTTOM en la barra de controles a la parte inferior.  
  
- CBRS_NOALIGN no cambia la posición la barra de control cuando cambia el tamaño del elemento primario.  
...
-   Admitir la implementación de clases derivadas.  
 ...
 Los objetos de la barra de control C++ normalmente se insertarán como miembros de una clase derivada `CFrameWnd` , y se borrarán cuando el elemento primario `HWND` y el objeto se destruyan. Si necesita asignar un objeto de barra de control en el montón, simplemente establezca la *m_bAutoDestruct* miembro **TRUE** para que la barra de control "**eliminar este**" cuando el `HWND` se destruye.  
  
> [!NOTE]
>  Si crea su propio `CControlBar`-clase derivada, en lugar de mediante uno de MFC de las clases derivadas, como `CStatusBar`, `CToolBar`, o `CDialogBar`, deberá establecer el *m_dwStyle* miembro de datos. Esto puede hacerse en el reemplazo de `Create`:  
 ...
 **Algoritmo de diseño de la barra de control**  
 ...
 Mensajes Windows privados de MFC, incluidos WM_SIZEPARENT, están documentados en [Nota técnica 24](../mfc/tn024-mfc-defined-messages-and-resources.md).  
 ...
 **Compatibilidad de CCmdUI con CStatusBar**  
 ...
 La forma indicadores se actualizan normalmente es a través del mecanismo ON_UPDATE_COMMAND_UI. En tiempo de inactividad, la barra de estado llamará al controlador ON_UPDATE_COMMAND_UI con el identificador de cadena del panel de indicador.  
...
## <a name="_mfcnotes_ctoolbar"></a>  CToolBar  
 ...
 Una barra de herramientas es una barra de controles con una fila de botones de mapa de bits que pueden incluir separadores. Se admiten dos estilos de botones: pulsadores y botones de casilla. Puede crear la funcionalidad de grupo de radio con botones de casilla de verificación y ON_UPDATE_COMMAND_UI.  
...
## <a name="ccmdui-support-for-ctoolbar"></a>Compatibilidad de CCmdUI con CToolBar  
 La forma de botones de barra de herramientas se actualizan siempre es a través del mecanismo ON_UPDATE_COMMAND_UI. En tiempo de inactividad, la barra de herramientas llamará al controlador ON_UPDATE_COMMAND_UI con el identificador de comando de ese botón. No se llama a ON_UPDATE_COMMAND_UI para los separadores, pero se llama por pulsadores y botones de casilla de verificación.  
...
- `SetRadio`: Una forma abreviada de `SetCheck`.  
 ...
 Las API de personalización le permitirán cambiar el estado de un botón de barra de herramientas determinado, preferiblemente debe cambiar estos Estados en el controlador ON_UPDATE_COMMAND_UI para el comando que representa el botón de barra de herramientas. Recuerde que el procesamiento en inactividad cambiará el estado de los botones de barra de herramientas con el controlador ON_UPDATE_COMMAND_UI, por lo que se pueden perder los cambios realizados a través de SetButtonStyle en estos Estados después del próximo inactivo.  
  
 Botones de barra de herramientas envía mensajes WM_COMMAND como botones normales o elementos de menú y normalmente se controlan mediante un controlador ON_COMMAND en la misma clase que proporciona el controlador ON_UPDATE_COMMAND_UI.  
