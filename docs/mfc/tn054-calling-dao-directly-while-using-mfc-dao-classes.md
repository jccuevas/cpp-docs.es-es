...
## <a name="using-the-dao-ole-interfaces"></a>Uso de las Interfaces OLE de DAO  
 ...
 Para muchos de los métodos de las interfaces DAO, es necesario manipular un `BSTR` objeto (una cadena de longitud fija utilizada en la automatización OLE). El `BSTR` objeto normalmente se encapsula dentro de la **VARIANT** tipo de datos. La clase MFC `COleVariant` a su vez hereda de la **VARIANT** tipo de datos. Dependiendo de si genera el proyecto de ANSI o Unicode, las interfaces DAO devolverá ANSI o Unicode `BSTR`s. Las dos macros, **V_BSTR** y **V_BSTRT**, son útiles para asegurarse de que la interfaz DAO Obtiene el `BSTR` del tipo esperado.  
  
 **V_BSTR** extraerá el **bstrVal** miembro de un `COleVariant`. Esta macro se usa normalmente cuando es necesario pasar el contenido de un `COleVariant` a un método de una interfaz de DAO. El fragmento de código siguiente muestra las declaraciones y el uso real de dos métodos de la interfaz de DAO DAOUser que aprovechan la **V_BSTR** macro:  
...
```  
 ...
 La macro **V_BSTRT**, extraerá ANSI o Unicode **bstrVal** miembro de `COleVariant` según el tipo de compilación (ANSI o Unicode). El código siguiente muestra cómo extraer el `BSTR` valor desde una `COleVariant` en un `CString`:  
