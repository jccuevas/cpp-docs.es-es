...
## <a name="odbcs-cursor-library"></a>Biblioteca de cursores de ODBC  
 ...
 Por ejemplo, mayoría de los controladores 1.0 no admite desplazamiento hacia atrás. La biblioteca de cursores puede detectarlo y se almacene en caché filas desde el controlador y presentarlos como se solicitó en llamadas FETCH_PREV en `SQLExtendedFetch`.  
...
## <a name="cdatabases"></a>CDatabases  
 ...
 `SQLError` se puede llamar con alguno o todos los argumentos es NULL.  
...
## <a name="begintrans-committrans-rollback"></a>BeginTrans, CommitTrans y Rollback  
 `SQLSetConnectOption SQL_AUTOCOMMIT` y `SQLTransact SQL_COMMIT`, **SQL_ROLLBACK** y **SQL_AUTOCOMMIT** se llamará si se realizan las solicitudes de transacción.  
  
## <a name="crecordsets"></a>CRecordsets  
 `SQLAllocStmt`, `SQLPrepare`, `SQLExecute` (Para `Open` y `Requery`), `SQLExecDirect` (para operaciones de actualización), `SQLFreeStmt` deben ser compatibles. `SQLNumResultCols` y `SQLDescribeCol` se llamará en los resultados que se establezca en momentos diferentes.  
...
## <a name="forwardonly-cursors"></a>Cursores ForwardOnly  
 Solo `SQLFetch` es necesaria para la `Move` operaciones. Tenga en cuenta que **forwardOnly** los cursores no son compatibles con las actualizaciones.  
