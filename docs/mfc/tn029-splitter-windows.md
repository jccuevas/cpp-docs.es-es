...
## <a name="terminology-used-by-implementation"></a>Terminología usada por implementación  
 ...
 `CSplitterWnd`:  
 Una ventana que proporciona controles de separación de panel y las barras de desplazamiento que se comparten entre todos los paneles en una fila o columna. Especificar las filas y columnas con números de base cero (el primer panel es la fila = 0 y la columna = 0).  
  
 Panel:  
 Una ventana específica de la aplicación que un `CSplitterWnd` administra. Un panel normalmente es un objeto que se deriva el [CView (clase)](../mfc/reference/cview-class.md), pero puede ser cualquier [CWnd](../mfc/reference/cwnd-class.md) objeto que tiene el identificador de ventana de secundario correspondiente.  
 ...
 Barra de división:  
 Un control que se coloca entre las filas y columnas de paneles. Se puede usar para ajustar los tamaños de las filas o columnas de paneles.  
