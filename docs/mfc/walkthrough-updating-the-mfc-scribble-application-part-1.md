...
- [Si se agrega una categoría de cinta de opciones](#addcategory)
...
3. En scribble.h, modifique la definición de la `CScribbleApp` para que se deriva de la clase [CWinAppEx Class](../mfc/reference/cwinappex-class.md).
...
5. El marco principal de una aplicación de interfaz de múltiples documentos (MDI) ya no se deriva de la clase `CMDIFrameWnd`. En su lugar, se deriva de la [CMDIFrameWndEx](../mfc/reference/cmdiframewndex-class.md) clase.
...
6. En los archivos childfrm.h y childfrm.cpp, reemplace `CMDIChildWnd` por `CMDIChildWndEx`.
     ...
     En el childfrm. archivo h, reemplace `CSplitterWnd` con `CSplitterWndEx`.
...
10. Si piensa vincular la aplicación estáticamente, agregue el código siguiente al principio del archivo de recursos del proyecto (.rc).
    ...
    ```
     ...
     El archivo afxribbon.rc contiene los recursos necesarios en tiempo de ejecución. El [Asistente para aplicaciones MFC](../mfc/reference/mfc-application-wizard.md) incluye este archivo automáticamente cuando se crea una aplicación.
...
3. Importe los mapas de bits para el menú Archivo asociado al botón Aplicación.
   ...
   1. Importe el archivo filesmall.bmp, que contiene diez imágenes de 16x16 (16x160). Dado que necesitamos solo ocho 16 x 16 imágenes (16 x 128), use la **vista de recursos** para cambiar el ancho del mapa de bits de 160 a 128. Cambie el identificador del mapa de bits a IDB_RIBBON_FILESMALL.
...
2. En el **Agregar recurso** cuadro de diálogo, seleccione **cinta** y, a continuación, haga clic en **nuevo**.
...
Ha modificado el ejemplo clásico Scribble de MFC 1.0 para utilizar el diseñador de la cinta de opciones. Ahora, vaya a [parte 2](../mfc/walkthrough-updating-the-mfc-scribble-application-part-2.md).
