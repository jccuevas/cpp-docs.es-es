...
## Comentarios  
 La función de operador devuelve true solo si las secuencias controladas por `left` y `right` tienen la misma longitud y, para cada posición `i`, `left``[i] ==` `right``[i]`.  Se utiliza para probar si `left` se le igual que `right` cuando los dos asignaciones son elemento comparado al lado de elemento.  
