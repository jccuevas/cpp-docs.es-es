...
ms.locfileid: "46402075"
---
# <a name="hashset-stlclr"></a>hash_set (STL/CLR)

La clase de plantilla describe un objeto que controla una secuencia de elementos de longitud variable que tiene acceso bidireccional. Usar el contenedor `hash_set` para administrar una secuencia de elementos como una tabla hash, cada entrada de la tabla almacenar un bidireccional vinculado lista de nodos y cada nodo almacenar un elemento. El valor de cada elemento se utiliza como clave, para ordenar la secuencia.
...
|[hash_set::difference_type (STL/CLR)](#difference_type)|El tipo de una distancia entre dos elementos (posiblemente con signo).|
|[hash_set::generic_container (STL/CLR)](#generic_container)|El tipo de la interfaz genérica para el contenedor.|
|[hash_set::generic_iterator (STL/CLR)](#generic_iterator)|El tipo de iterador para la interfaz genérica para el contenedor.|
|[hash_set::generic_reverse_iterator (STL/CLR)](#generic_reverse_iterator)|El tipo de un iterador inverso de la interfaz genérica para el contenedor.|
...
|[hash_set::size_type (STL/CLR)](#size_type)|El tipo de una distancia (no negativo) entre dos elementos.|
...
|[hash_set::key_comp (STL/CLR)](#key_comp)|Copia al delegado de ordenación de dos claves.|
...
|[hash_set::to_array (STL/CLR)](#to_array)|Copia la secuencia controlada a una nueva matriz.|
|[hash_set::upper_bound (STL/CLR)](#upper_bound)|Busca el final del intervalo que coincide con una clave especificada.|
|[hash_set::value_comp (STL/CLR)](#value_comp)|Copia al delegado de ordenación de dos valores de elemento.|
...
Un objeto hash_set es compatible con los iteradores bidireccionales, lo que significa que puede ejecutar paso a paso para los elementos adyacentes que proporciona un iterador que designa un elemento de la secuencia controlada. Un nodo principal especial que se corresponde con el iterador devuelto por [hash_set:: end (STL/CLR)](../dotnet/hash-set-end-stl-clr.md)`()`. Puede reducir este iterador para alcanzar el último elemento de la secuencia controlada, si está presente. Puede incrementar un iterador hash_set para alcanzar el nodo principal y, a continuación, comparará igual a `end()`. Pero no se puede desreferenciar el iterador devuelto por `end()`.

Tenga en cuenta que no puede hacer referencia a un elemento del objeto hash_set directamente dado su posición numérica--que requiera un iterador de acceso aleatorio.

Un iterador hash_set almacena un identificador a su nodo hash_set asociado, que a su vez almacena un identificador de su contenedor asociado. Puede usar iteradores sólo con sus objetos de contenedor asociado. Un iterador hash_set sigue siendo válido siempre y cuando su nodo asociado hash_set está asociado a algún objeto hash_set. Además, un iterador válido es dereferenceable, puede usar para tener acceso o modificar el valor del elemento designa--siempre y cuando no es igual a `end()`.
...
La función miembro llama eficazmente a [hash_set:: Erase (STL/CLR)](../dotnet/hash-set-erase-stl-clr.md) `(` [hash_set:: begin (STL/CLR)](../dotnet/hash-set-begin-stl-clr.md) `(),` [hash_set:: end (STL/CLR)](../dotnet/hash-set-end-stl-clr.md) `())`. Usarlo para asegurarse de que está vacía la secuencia controlada.
...
La función miembro devuelve true para una secuencia controlada vacía. Equivale a [hash_set:: Size (STL/CLR)](../dotnet/hash-set-size-stl-clr.md)`() == 0`. Usa para comprobar si el objeto hash_set está vacío.
...
La primera función miembro quita el elemento de la secuencia controlada señalada por *donde*y devuelve un iterador que designa el primer elemento que permanece más allá del elemento quitado, o [hash_set:: end (STL / CLR)](../dotnet/hash-set-end-stl-clr.md) `()` si no existe ese elemento. Usa para quitar un elemento único.
...
Si al menos un elemento de la secuencia controlada tiene una ordenación equivalente con *clave*, la función miembro devuelve un iterador que designa uno de esos elementos; de lo contrario, devuelve [hash_set:: end (STL/CLR)](../dotnet/hash-set-end-stl-clr.md) `()`. Usa para buscar un elemento actualmente en la secuencia controlada que coincide con una clave especificada.
...
La función miembro devuelve `(float)` [hash_set:: Size (STL/CLR)](../dotnet/hash-set-size-stl-clr.md) `() /` [hash_set::bucket_count (STL/CLR)](../dotnet/hash-set-bucket-count-stl-clr.md)`()`. Se usa para determinar el tamaño de depósito promedio.
...
La función miembro determina el primer elemento `X` en la secuencia controlada que aplica un algoritmo hash al mismo depósito que *clave* y tiene una ordenación equivalente a *clave*. Si no existe ese elemento, devuelve [hash_set:: end (STL/CLR)](../dotnet/hash-set-end-stl-clr.md)`()`; en caso contrario, devuelve un iterador que designa `X`. Usa para buscar el principio de una secuencia de elementos actualmente en la secuencia controlada que coinciden con una clave especificada.
...
La función miembro se vuelve a generar la tabla hash, lo que garantiza que [hash_set::load_factor (STL/CLR)](../dotnet/hash-set-load-factor-stl-clr.md) `() <=` [hash_set::max_load_factor (STL/CLR)](../dotnet/hash-set-max-load-factor-stl-clr.md). En caso contrario, la tabla hash aumenta de tamaño según sea necesario después de una inserción. (Nunca disminuye de tamaño automáticamente.) Usa para ajustar el tamaño de la tabla hash.
...
La función miembro devuelve la longitud de la secuencia controlada. Se usa para determinar el número de elementos actualmente en la secuencia controlada. Si lo único que interesa es si la secuencia tiene un tamaño distinto de cero, vea [hash_set:: Empty (STL/CLR)](../dotnet/hash-set-empty-stl-clr.md)`()`.
