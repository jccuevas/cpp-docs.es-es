...
Deque mayor o comparación igual.  
...
## Comentarios  
 La función de operador devuelve `!(``left` `<` `right``)`.  Se utiliza para probar si `left` no se le antes de `right` cuando los dos deques son elemento comparado al lado de elemento.  
