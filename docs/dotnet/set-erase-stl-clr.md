...
## Comentarios  
 La primera función miembro quita el elemento de la secuencia controlada designada por a `where`, y devuelve un iterador que señala el primer elemento que permanece más allá del elemento quitado, o [set::end](../dotnet/set-end-stl-clr.md)`()` si no existe ese elemento.  Se utiliza para quitar un solo elemento.  
