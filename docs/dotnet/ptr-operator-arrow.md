...
En este ejemplo implementa una clase CLR que utiliza un `com::ptr` para ajustar su miembro privado `IXMLDOMDocument` objeto. El `WriteDocument` funciones utiliza `operator->` para llamar a la `get_firstChild` miembro del objeto de documento.
