...
## Comentarios  
 La función de operador devuelve `right` `<` `left`.  Se utiliza para probar si `left` se le después de `right` cuando los dos vectores son elemento comparado al lado de elemento.  
