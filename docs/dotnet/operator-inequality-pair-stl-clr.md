...
## Comentarios  
 La función de operador devuelve `!(``left` `==` `right``)`.  Se utiliza para probar si `left` no se le igual que `right` cuando los dos pares son elemento comparado al lado de elemento.  
