---
title: Declaración de propiedad de índice | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-cli
...
Como puede ver aquí, los indizadores se distinguen únicamente por los parámetros adicionales para especificar una de dos o único índice de dimensión. En la nueva sintaxis, los indizadores se distinguen por los corchetes ([,]) después del nombre del indizador y que indica el número y tipo de cada índice:
...
Para indicar un indizador de nivel de clase que se puede aplicar directamente a los objetos de la clase en la nueva sintaxis, la `default` palabra clave se reutiliza para sustituir a un nombre explícito. Por ejemplo:
...
En la nueva sintaxis, al indexar el valor predeterminado se especifica la propiedad, los dos nombres siguientes están reservados: `get_Item` y `set_Item`. Esto es porque estos son los nombres subyacentes generados para la propiedad indizada predeterminada.
