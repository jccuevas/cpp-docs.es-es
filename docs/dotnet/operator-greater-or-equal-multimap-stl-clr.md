...
## Comentarios  
 La función de operador devuelve `!(``left` `<` `right``)`.  Se utiliza para probar si `left` no se le antes de `right` cuando los dos multimaps son elemento comparado al lado de elemento.  
