...
Pila menor o comparación igual.  
...
## Comentarios  
 La función de operador devuelve `!(``right` `<` `left``)`.  Se utiliza para probar si `left` no se le después de `right` cuando las dos pilas son elemento comparado al lado de elemento.  
