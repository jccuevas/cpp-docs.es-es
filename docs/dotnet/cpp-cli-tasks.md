---
title: C++ / CLI tareas | Documentos de Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-cli
ms.topic: conceptual
dev_langs:
- C++
ms.assetid: 5851b113-91f9-4062-9427-c0669f333636
author: mikeblome
ms.author: mblome
ms.workload:
- cplusplus
- dotnet
ms.openlocfilehash: 7689c7759dbaacc9d8f2e22807456d12da245995
ms.sourcegitcommit: 76b7653ae443a2b8eb1186b789f8503609d6453e
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 05/04/2018
ms.locfileid: "33110409"
---
# <a name="ccli-tasks"></a>Tareas de C++/CLI
Los artículos de esta sección de la documentación muestran cómo usar diversas características de C++ / CLI.  
  
## <a name="in-this-section"></a>En esta sección  
 
 - [Cómo: Crear proyectos vacíos de CLR](../dotnet/how-to-create-clr-empty-projects.md)  
 - [Cómo: Crear aplicaciones de consola CLR (C++/CLI)](../dotnet/how-to-create-clr-console-applications-cpp-cli.md)  
 - [Cómo: Usar referencias de seguimiento en C++/CLI](../dotnet/how-to-use-tracking-references-in-cpp-cli.md)  
 - [Cómo: Usar matrices en C++/CLI](../dotnet/how-to-use-arrays-in-cpp-cli.md)  
 - [Cómo: Definir y usar clases y structs (C++/CLI)](../dotnet/how-to-define-and-consume-classes-and-structs-cpp-cli.md)  
 - [Semántica de pila de C++ para los tipos de referencia](../dotnet/cpp-stack-semantics-for-reference-types.md)  
 - [Operadores definidos por el usuario (C++/CLI)](../dotnet/user-defined-operators-cpp-cli.md)  
 - [Conversiones definidas por el usuario (C++/CLI)](../dotnet/user-defined-conversions-cpp-cli.md)  
 - [initonly (C++/CLI)](../dotnet/initonly-cpp-cli.md)  
 - [Cómo: Definir y usar delegados (C++/CLI)](../dotnet/how-to-define-and-use-delegates-cpp-cli.md)  
 - [Cómo: Definir y usar enumeraciones en C++/CLI](../dotnet/how-to-define-and-consume-enums-in-cpp-cli.md)  
 - [Cómo: Usar eventos en C++/CLI](../dotnet/how-to-use-events-in-cpp-cli.md)  
 - [Cómo: Definir un constructor estático de interfaz (C++/CLI)](../dotnet/how-to-define-an-interface-static-constructor-cpp-cli.md)  
 - [Cómo: Declarar especificadores de invalidación en las compilaciones nativas (C++/CLI)](../dotnet/how-to-declare-override-specifiers-in-native-compilations-cpp-cli.md)  
 - [Cómo: Usar propiedades en C++/CLI](../dotnet/how-to-use-properties-in-cpp-cli.md)  
 - [Cómo: Usar safe_cast en C++/CLI](../dotnet/how-to-use-safe-cast-in-cpp-cli.md)  
  
## <a name="related-sections"></a>Secciones relacionadas  
  

 - [Programación de .NET con C++/CLI (Visual C++)](../dotnet/dotnet-programming-with-cpp-cli-visual-cpp.md)