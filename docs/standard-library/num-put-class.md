...
ms.locfileid: "81373645"
---
# <a name="num_put-class"></a>num_put (Clase)

Plantilla de clase que describe un objeto que puede servir como faceta de configuración `CharType`regional para controlar las conversiones de valores numéricos a secuencias de tipo .
...
La primera función miembro protegido virtual genera elementos secuenciales que comienzan en *el siguiente* para producir un campo de salida entero a partir del valor de *val*. La función devuelve un iterador que designa el lugar siguiente para insertar un elemento más allá del campo de salida numérico entero que se ha generado.

El campo de salida entero se genera mediante las mismas reglas utilizadas por las funciones de impresión para generar una serie de elementos **char** en un archivo. Se supone que cada elemento char se `CharType` asigna a un elemento equivalente de tipo mediante una asignación simple de uno a uno. Sin embargo, cuando una función de impresión `do_put` rellena `fill`un campo con espacios o el dígito 0, en su lugar utiliza . La especificación de conversión de impresión equivalente se determina de la manera siguiente:

- Si **iosbase**. [flags](../standard-library/ios-base-class.md#flags) & flags ==  `lo`oct , la especificación de conversión es .[oct](../standard-library/ios-functions.md#oct)`ios_base::basefield``ios_base::`

- Si **iosbase.flags** & **ios_base::basefield** == `ios_base::`[hex](../standard-library/ios-functions.md#hex), `lx`la especificación de conversión es .
...
El relleno solo se produce si el número mínimo de elementos *N* necesarios para especificar el campo de salida es inferior a **iosbase**. [anchura](../standard-library/ios-base-class.md#width). Dicho relleno consiste en una secuencia de *N* - copias de**ancho** de **relleno.** Después, el relleno se produce de la manera siguiente:

- Si **iosbase**. **banderas** & `ios_base::`[left](../standard-library/ios-functions.md#left) **-** a la izquierda, la bandera se antepone.`ios_base::adjustfield` ==  (El relleno se produce después del texto generado).

- Si **iosbase.flags** & **ios_base::adjustfield** == `ios_base::`[internal](../standard-library/ios-functions.md#internal), se antepone el indicador **0.** (Para un campo de salida numérico, el relleno se produce donde las funciones de impresión rellenan con 0).

- De otro modo, no se antepone ninguna marca adicional. (El relleno se produce antes de la secuencia generada).

Por último:

- Si **iosbase**. **flags** & flags`ios_base::`[showpos](../standard-library/ios-functions.md#showpos) es distinto **+** de cero, el indicador se antepone a la especificación de conversión.

- Si **iosbase**. **indicadores** & **ios_base::**[showbase](../standard-library/ios-functions.md#showbase) es **#** distinto de cero, el indicador se antepone a la especificación de conversión.
...
- Si **iosbase**. **indicadores** & `ios_base::floatfield` == `ios_base::`[fijos](../standard-library/ios-functions.md#fixed), `lf`la especificación de conversión es .

- Si **iosbase**. **banderas** & **ios_base::floatfield** == `ios_base::`[scientific](../standard-library/ios-functions.md#scientific), `le`la especificación de conversión es . Si **iosbase**. **banderas** & `ios_base::`[mayúsculas](../standard-library/ios-functions.md#uppercase) es `e` distinto de `E`cero, se sustituye por .

- De otro modo, la especificación de conversión es **lg**. Si **iosbase**. **los indicadores** & **ios_base::mayúsculas** es distinto de cero, `g` se sustituye por `G`.

Si **iosbase**. **marcas** & **ios_base::fixed** es distinto de cero o si **iosbase**. [precision](../standard-library/ios-base-class.md#precision) es mayor que cero, una precisión con el valor **iosbase**. **precision** se antepone a la especificación de conversión. Cualquier relleno se comporta de la misma manera que para un campo de salida numérico entero. El carácter de relleno es **fill**. Por último:

- Si **iosbase**. **flags** & flags`ios_base::`[showpos](../standard-library/ios-functions.md#showpos) es distinto **+** de cero, el indicador se antepone a la especificación de conversión.

- Si **iosbase**. **flags** & flags`ios_base::`[showpoint](../standard-library/ios-functions.md#showpoint) es distinto **#** de cero, el indicador se antepone a la especificación de conversión.
...
se comporta igual que el tercero, `l` excepto que el calificador de la especificación de conversión se sustituye por `L`.
...
se comporta igual que el primero, excepto que genera un campo de salida booleano a partir de *val*.

Un campo de salida booleano adopta una de dos formas. Si `iosbase.flags & ios_base::` [boolalpha](../standard-library/ios-functions.md#boolalpha) es **false**, `do_put(_Next, _Iosbase, _Fill, (long)val)`la función miembro devuelve , que normalmente genera una secuencia generada de 0 (para **false)** o 1 (para **true**). De lo contrario, la secuencia generada es *fac*. [falsename](../standard-library/numpunct-class.md#falsename) (para **false**), o *fac*. [truename](../standard-library/numpunct-class.md#truename) (para **true**).
...
El constructor inicializa su objeto base con la[faceta](../standard-library/locale-class.md#facet_class) **locale::**(_ *Refs*).

## <a name="num_putput"></a><a name="put"></a>num_put::put

Convierte un número en `CharType`una secuencia de s que representa el número formateado para una configuración regional determinada.
...
Todas las funciones `next` `_Iosbase`miembro `_Fill` `val`devuelven [do_put](#do_put)( , , , ).
