...
ms.locfileid: "68447091"
---
# <a name="ostreamiterator-class"></a>ostream_iterator (Clase)

La clase de plantilla ostream_iterator describe un objeto de iterador de salida que escribe elementos sucesivos en el `operator <<`flujo de salida con la extracción.
...
|-|-|
|[operator*](#op_star)|Operador de desreferencia usado para implementar la expresión \* `i`  =  `x`de iterador de salida.|
...
|[operator=](#op_eq)|Operador de asignación usado \* para implementar la expresión `i`  =  `x` de iterador de salida para escribir en un flujo de salida.|
...
Operador de asignación usado para implementar la expresión \* `i`  =  `x` output_iterator para escribir en un flujo de salida.
...
El operador inserta *Val* en el flujo de salida asociado al objeto, seguido del delimitador especificado en el [constructor ostream_iterator](#ostream_iterator) (si existe) y, a continuación, devuelve una `ostream_iterator`referencia a.

### <a name="remarks"></a>Comentarios

Los requisitos de un iterador de salida `ostream_iterator` que debe satisfacer solo requieren que \* la expresión `ii`  =  `t` sea válida y no indica nada sobre el operador o el operador = por su cuenta. Este operador miembro devuelve `*this`.
...
El segundo constructor inicializa el puntero del flujo de salida `&_Ostr` con y el puntero de la cadena de delimitador con *_Delimiter*.
