...
Para desreferenciar un `const_iterator` `cIter` que apunta a un elemento de un hash_multimap, utilice el operador `->`.
...
Para desreferenciar un `const_reverse_iterator` `crIter` que apunta a un elemento de un hash_multimap, utilice el operador `->`.
...
La función miembro devuelve un iterador que direcciona un elemento en el hash_multimap cuya clave de ordenación se `equivalent` a la clave de argumento en un predicado binario que induce a una ordenación basada en una relación de comparabilidad menor que.

Si el valor devuelto de `find` se asigna a `const_iterator`, no se puede modificar el objeto hash_multimap. Si el valor devuelto de `find` se asigna a un `iterator`, el objeto de hash_multimap se puede modificar.
...
El `iterator` definido por hash_multimap apunta a objetos de [value_type](#value_type), que son de tipo `pair`\< **clave const, de tipo**>, cuyo primer miembro es la clave para el elemento y cuyo segundo miembro es la referencia asignada que el elemento conserva.

Para desreferenciar un **iterador**`Iter` que apunta a un elemento de un hash_multimap, utilice el operador `->`.

Para tener acceso al valor de la clave del elemento, use `Iter` -> **primero**, que es equivalente a (\* `Iter`). **first**. Para tener acceso al valor de la referencia asignada del elemento, use `Iter` -> **segundo**, que es equivalente a (\* `Iter`). **first**.
...
**operador bool (clave const &** `left` **, clave const &** `right` **);**
...
Para obtener más información sobre los *rasgos* , vea el tema [hash_multimap clase](../standard-library/hash-multimap-class.md) .
...
Para obtener más información sobre la *clave*, consulte la sección Comentarios del tema [hash_multimap clase](../standard-library/hash-multimap-class.md) .
...
Si el valor devuelto de `lower_bound` se asigna a `const_iterator`, no se puede modificar el objeto hash_multimap. Si el valor devuelto de `lower_bound` se asigna a un `iterator`, el objeto de hash_multimap se puede modificar.
...
Para obtener más información sobre el *tipo* , vea el tema [hash_multimap clase](../standard-library/hash-multimap-class.md) .
...
El `reverse_iterator` definido por hash_multimap apunta a objetos de [value_type](#value_type), que son de tipo `pair`\< **clave const, escriba**>. El valor de la clave está disponible mediante el primer miembro del par y el valor del elemento asignado está disponible mediante el segundo miembro del par.
...
Si el valor devuelto de `upper_bound` se asigna a `const_iterator`, no se puede modificar el objeto hash_multimap. Si el valor devuelto de `upper_bound` se asigna a un `iterator`, el objeto de hash_multimap se puede modificar.
...
En el caso de una hash_multimap *m*, si dos elementos *E1* (*K1*, *D1*) e *E2*(*K2*, *D2*) son objetos de tipo [value_type](#value_type), donde *K1* y *K2* son sus claves de tipo [key_type](#key_type) y *D1* y *D2* son sus datos de tipo [mapped_type](#mapped_type), `m.value_comp()(e1, e2)` es equivalente a `m.key_comp()(k1, k2)`. Un objeto almacenado define la función miembro
...
`value_type` se declara para ser Pair\<const [key_type](#key_type), [mapped_type](#mapped_type)> y no Pair\<key_type mapped_type > porque las claves de un contenedor asociativo no se pueden cambiar mediante un iterador o referencia no constante.
