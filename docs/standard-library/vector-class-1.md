---
title: "vector, clase 1 | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
La clase de vector STL es una clase de plantilla de contenedores de secuencias que organiza los elementos de un tipo determinado en una organización lineal y permite el acceso aleatorio rápido a cualquier elemento. Deberían ser el contenedor más apropiado para una secuencia cuando el rendimiento de acceso aleatorio sea importante.  
...
## <a name="remarks"></a>Comentarios  
 Los vectores permiten inserciones y eliminaciones de tiempo constante al final de la secuencia. Insertar o eliminar elementos en medio de un vector requiere tiempo lineal. El rendimiento de la [clase deque](../standard-library/deque-class.md) contenedor es superior respecto a las inserciones y eliminaciones al principio y al final de una secuencia. La [list (clase)](../standard-library/list-class.md) contenedor es superior respecto a las inserciones y eliminaciones en cualquier ubicación dentro de una secuencia.  
 ...
 El [vector \< bool> clase](../standard-library/vector-bool-class.md) es una especialización de la clase de plantilla vector para los elementos del tipo bool completa con un asignador para el tipo subyacente utilizado por la especialización.  
  
 El [vector \< bool> hacen referencia a clase](../standard-library/vector-bool-class.md#vector_lt_bool_gt___reference_class) es una clase anidada cuyos objetos pueden incluir referencias a elementos (bits únicos) dentro de un vector \< bool> objeto.  
...
|[iterador](#vector__iterator)|Tipo que proporciona un iterador de acceso aleatorio que puede leer o modificar cualquier elemento de un vector.|  
|[puntero](#vector__pointer)|Tipo que proporciona un puntero a un elemento en un vector.|  
|[referencia](#vector__reference)|Tipo que proporciona una referencia a un elemento almacenado en un vector.|  
...
|||  
|-|-|  
|[asignar](#vector__assign)|Borra un vector y copia los elementos especificados al vector vacío.|  
|[en](#vector__at)|Devuelve una referencia al elemento en una ubicación especificada del vector.|  
|[Atrás](#vector__back)|Devuelve una referencia al último elemento del vector.|  
|[comenzar](#vector__begin)|Devuelve un iterador de acceso aleatorio al primer elemento del vector.|  
|[capacidad](#vector__capacity)|Devuelve el número de elementos que puede contener el vector sin asignar más espacio de almacenamiento.|  
...
|[Borrar](#vector__clear)|Borra los elementos del vector.|  
|[datos](#vector__data)|Devuelve un puntero al primer elemento del vector.|  
...
|[vacía](#vector__empty)|Comprueba si el contenedor de vectores está vacío.|  
|[final](#vector__end)|Devuelve un iterador de acceso aleatorio que apunta al final del vector.|  
|[Borrar](#vector__erase)|Quita un elemento o un intervalo de elementos en un vector a partir de posiciones especificadas.|  
|[parte frontal](#vector__front)|Devuelve una referencia al primer elemento de un vector.|  
|[get_allocator](#vector__get_allocator)|Devuelve un objeto a la clase `allocator` usada por un vector.|  
|[Insertar](#vector__insert)|Inserta uno o varios elementos en el vector en una posición especificada.|  
...
|[reservar](#vector__reserve)|Reserva una longitud mínima de almacenamiento para un objeto vectorial.|  
|[cambiar el tamaño](#vector__resize)|Especifica un nuevo tamaño de un vector.|  
|[shrink_to_fit](#vector__shrink_to_fit)|Descarta el exceso de capacidad.|  
|[tamaño](#vector__size)|Devuelve el número de elementos del vector.|  
|[intercambio](#vector__swap)|Intercambia los elementos de dos vectores.|  
  
### <a name="operators"></a>Operadores  
  
|||  
|-|-|  
|[operador &#91; &#93;](#vector__operator_at)|Devuelve una referencia al elemento vector en una posición especificada.|  
|[operador =](#vector__operator_eq)|Reemplaza los elementos del vector con una copia de otro vector.|  
...
### <a name="example"></a>Ejemplo  
  Vea el ejemplo de [get_allocator](#vector__get_allocator) para obtener un ejemplo que usa `allocator_type`.  
...
### <a name="return-value"></a>Valor devuelto  
 Referencia al elemento indicado en el argumento. Si `_Off` es mayor que el tamaño del vector, **en** produce una excepción.  
  
### <a name="remarks"></a>Comentarios  
 Si el valor devuelto de **en** se asigna a un `const_reference`, no se puede modificar el objeto de vector. Si el valor devuelto de **en** se asigna a un **referencia**, el objeto de vector puede modificarse.  
...
### <a name="remarks"></a>Comentarios  
 Si el valor devuelto de **volver** se asigna a un `const_reference`, no se puede modificar el objeto de vector. Si el valor devuelto de **volver** se asigna a un **referencia**, el objeto de vector puede modificarse.  
  
 Al compilar con _SECURE_SCL 1, se producirá un error en tiempo de ejecución si trata de acceder a un elemento en un vector vacío.  Vea [Checked Iterators](../standard-library/checked-iterators.md) para obtener más información.  
...
### <a name="return-value"></a>Valor devuelto  
 Un iterador de acceso aleatorio que direcciona el primer elemento en el `vector` o a la ubicación que sigue vacía `vector`. Siempre debe comparar el valor devuelto con [vector:: end](#vector__end) para asegurarse de que es válido.  
  
### <a name="remarks"></a>Comentarios  
 Si el valor devuelto de `begin` se asigna a un [vector:: const_iterator](#vector__const_iterator), la `vector` no se puede modificar el objeto. Si el valor devuelto de `begin` se asigna a un [vector:: Iterator](#vector__iterator), el `vector` se puede modificar el objeto.  
...
### <a name="remarks"></a>Comentarios  
 La función miembro [cambiar el tamaño de](#vector__resize) será más eficaz si se asigna memoria suficiente para darle cabida. Utilice la función miembro [reservar](#vector__reserve) para especificar la cantidad de memoria asignada.  
...
##  <a name="a-namevectorconstiteratora-vectorconstiterator"></a><a name="vector__const_iterator"></a>  Vector:: const_iterator  
 Un tipo que proporciona un iterador de acceso aleatorio que puede leer un **const** elemento de un vector.  
...
### <a name="example"></a>Ejemplo  
  Vea el ejemplo de [volver](#vector__back) para obtener un ejemplo que usa `const_iterator`.  
  
##  <a name="a-namevectorconstpointera-vectorconstpointer"></a><a name="vector__const_pointer"></a>  Vector:: const_pointer  
 Un tipo que proporciona un puntero a un **const** elemento de un vector.  
...
### <a name="remarks"></a>Comentarios  
 ...
 Un [iterador](#vector__iterator) se utiliza normalmente para tener acceso a un elemento de vector.  
  
##  <a name="a-namevectorconstreferencea-vectorconstreference"></a><a name="vector__const_reference"></a>  Vector:: const_reference  
 Un tipo que proporciona una referencia a un **const** elemento almacenado en un vector para leer y realizar **const** operaciones.  
...
##  <a name="a-namevectorconstreverseiteratora-vectorconstreverseiterator"></a><a name="vector__const_reverse_iterator"></a>  Vector:: const_reverse_iterator  
 Un tipo que proporciona un iterador de acceso aleatorio que puede lee cualquier **const** elemento del vector.  
...
### <a name="remarks"></a>Comentarios  
 Un tipo de `const_reverse_iterator` no se puede modificar el valor de un elemento y se utiliza para recorrer en iteración el vector en orden inverso.  
  
### <a name="example"></a>Ejemplo  
  Consulte [rbegin](#vector__rbegin) para obtener un ejemplo de cómo declarar y usar un iterador.  
...
### <a name="return-value"></a>Valor devuelto  
 Const reverse de iterador de acceso aleatorio que direcciona el primer elemento en invertido [vector](vector%20Class.md) o direccionamiento lo que habría sido el último elemento de la irreversible `vector`.  
...
##  <a name="a-namevectorcrenda-vectorcrend"></a><a name="vector__crend"></a>  Vector:: crend  
 Devuelve un iterador const que direcciona la ubicación del último elemento de un vector inverso.  
...
### <a name="return-value"></a>Valor devuelto  
 Const reverse de iterador de acceso aleatorio que direcciona la ubicación del último elemento de invertido [vector](vector%20Class.md) (la ubicación que había precedido al primer elemento de la irreversible `vector`).  
  
### <a name="remarks"></a>Comentarios  
 `crend` se utiliza con invertido `vector` como [vector:: cend](#vector__cend) se usa con un `vector`.  
  
 Con el valor devuelto de `crend` (convenientemente reduce) el `vector` no se puede modificar el objeto.  
...
### <a name="return-value"></a>Valor devuelto  
 Un puntero al primer elemento en el [vector](vector%20Class.md) o a la ubicación que sigue vacía `vector`.  
...
### <a name="remarks"></a>Comentarios  
 Un `difference_type` también se puede describir como el número de elementos entre dos punteros, dado un puntero a un elemento contiene su dirección.  
...
|`_Where`|La posición en la [vector](vector%20Class.md) donde se inserta el primer elemento.|  
...
### <a name="remarks"></a>Comentarios  
 Las operaciones de inserción pueden ser costosas, vea [vector, clase](vector%20Class.md) para obtener una explicación de `vector` rendimiento.  
...
### <a name="remarks"></a>Comentarios  
 Si el valor devuelto de **final** se asigna a una variable de tipo `const_iterator`, no se puede modificar el objeto de vector. Si el valor devuelto de **final** se asigna a una variable de tipo **iterador**, el objeto de vector puede modificarse.  
...
### <a name="remarks"></a>Comentarios  
 Si el valor devuelto de `front` se asigna a `const_reference`, el objeto vector no se puede modificar. Si el valor devuelto de `front` se asigna a un **referencia**, el objeto de vector puede modificarse.  
...
### <a name="remarks"></a>Comentarios  
 Los asignadores de la clase vector especifican cómo la clase administra el almacenamiento. Los asignadores predeterminados proporcionados con las clases de contenedor STL son suficientes para la mayoría de las necesidades de programación. La escritura y el uso de sus propias clases de asignador son temas avanzados de C++.  
...
### <a name="example"></a>Ejemplo  
  Vea el ejemplo de [comenzar](#vector__begin).  
...
### <a name="remarks"></a>Comentarios  
 ...
 Al compilar con _SECURE_SCL 1 (controlado con [_ITERATOR_DEBUG_LEVEL](../standard-library/iterator-debug-level.md)), se producirá un error de tiempo de ejecución si se intenta obtener acceso a un elemento fuera de los límites del vector.  Vea [Checked Iterators](../standard-library/checked-iterators.md) para obtener más información.  
...
|-|-|  
|Parámetro|Descripción|  
|` right`|El [vector](vector%20Class.md) se copian en el `vector`.|  
...
### <a name="remarks"></a>Comentarios  
 Un tipo **puntero** puede utilizarse para modificar el valor de un elemento.  
...
### <a name="remarks"></a>Comentarios  
 Para obtener un ejemplo de código, consulte [vector:: push_back ()](#vector__push_back).  
...
### <a name="return-value"></a>Valor devuelto  
 Un iterador de acceso aleatorio inverso dirige al primer elemento en un vector inverso o direcciona lo que habría sido el último elemento del vector irreversible.  
  
### <a name="remarks"></a>Comentarios  
 Si el valor devuelto de `rbegin` se asigna a `const_reverse_iterator`, el objeto vector no se puede modificar. Si el valor devuelto de `rbegin` se asigna a un `reverse_iterator`, el objeto de vector puede modificarse.  
...
### <a name="example"></a>Ejemplo  
  Vea [en](#vector__at) para obtener un ejemplo de cómo usar **referencia** en la clase vector.  
  
##  <a name="a-namevectorrenda-vectorrend"></a><a name="vector__rend"></a>  Vector:: rend  
 Devuelve un iterador que direcciona la ubicación del último elemento de un vector inverso.  
...
### <a name="return-value"></a>Valor devuelto  
 Un iterador de acceso aleatorio inverso que direcciona la ubicación del último elemento de un vector inverso (la ubicación que había precedido al primer elemento del vector sin invertir).  
  
### <a name="remarks"></a>Comentarios  
 `rend` se utiliza con un vector inverso como [final](#vector__end) se usa con un vector.  
  
 Si el valor devuelto de `rend` se asigna a un `const_reverse_iterator`, entonces no se puede modificar el objeto de vector. Si el valor devuelto de `rend` se asigna a un `reverse_iterator`, a continuación, el objeto de vector puede modificarse.  
  
 `rend` puede utilizarse para probar si un iterador inverso llegó al final de su vector.  
...
### <a name="remarks"></a>Comentarios  
 ...
 [tamaño](#vector__size) refleja el tamaño actual del vector.  
...
##  <a name="a-namevectorsizea-vectorsize"></a><a name="vector__size"></a>  Size  
...
### <a name="example"></a>Ejemplo  
  Vea el ejemplo de [capacidad](#vector__capacity).  
...
##  <a name="a-namevectorvectora-vectorvector"></a><a name="vector__vector"></a>  vector  
...
### <a name="remarks"></a>Comentarios  
 ...
 Los dos primeros constructores especifican un vector inicial vacío. El segundo especifica explícitamente el tipo de asignador ( `Al`) que se utilizará.  
