...
El segundo constructor inicializa los objetos base mediante una llamada `move(right)`a.
...
El operador miembro llama `swap(right)`a.
...
La función miembro llama `swap(right)`a.
