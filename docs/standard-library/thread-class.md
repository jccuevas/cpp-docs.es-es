...
`F` Una función definida por la aplicación que va a ejecutar el subproceso.

`A` Una lista de argumentos que se pasan a `F`.

`Other` Existente `thread` objeto.
