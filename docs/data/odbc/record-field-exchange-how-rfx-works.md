...
2.  `AddNew` o `Edit` prepara los campos en el búfer de edición para que RFX pueda detectar los miembros de datos de campo modificados.  
...
3.  Establecer directamente los valores de los miembros de datos de campo cuyos valores desea editar o que desea rellenar un nuevo registro. Esto puede incluir la llamada `SetFieldNull`.  
...
5.  Para `AddNew`, `Update` concluye mediante la restauración de los valores almacenados previamente del registro que era el actual antes del `AddNew` llamar. Para `Edit`, los valores nuevos y modificados permanecen en su lugar.  
