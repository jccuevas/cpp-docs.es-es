---
title: "Crear un consumidor OLE DB | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: ""
ms.topic: "article"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "consumidores OLE DB, crear"
ms.assetid: 652309d3-7736-4354-89ec-28ec80a83f9a
caps.latest.revision: 7
caps.handback.revision: 7
author: "mikeblome"
ms.author: "mblome"
manager: "ghogen"
---
# Crear un consumidor OLE DB
[!INCLUDE[vs2017banner](../../assembler/inline/includes/vs2017banner.md)]

Un consumidor de plantillas OLE DB se puede crear con o sin el Asistente para consumidores OLE DB ATL, aunque se recomienda el uso del asistente.  Un consumidor puede funcionar en cualquier aplicación compatible con ATL.  
  
 En esta sección se tratan los siguientes temas:  
  
-   [Crear un consumidor OLE DB mediante un asistente](../../data/oledb/creating-an-ole-db-consumer-using-a-wizard.md)  
  
-   [Crear un consumidor OLE DB sin utilizar un asistente](../../data/oledb/creating-a-consumer-without-using-a-wizard.md)  
  
## Vea también  
 [Plantillas de consumidor OLE DB](../../data/oledb/ole-db-consumer-templates-cpp.md)