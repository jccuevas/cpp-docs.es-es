...
Con MSVC, hay varias maneras de programar con varios subprocesos: Puede usar C++/WinRT y la biblioteca de Windows Runtime, la biblioteca MFC (Microsoft Foundation Class), C++/CLI y el tiempo de ejecución de .net, o la biblioteca en tiempo de ejecución de C y la API Win32. Este artículo trata sobre el multithreading en C. Para ver un ejemplo de código, vea [programa multiproceso de ejemplo en C](sample-multithread-c-program.md).
...
Las bibliotecas de CRT proporcionan las siguientes funciones para la creación y terminación de subprocesos: _ [beginthread, _ beginthreadex](../c-runtime-library/reference/beginthread-beginthreadex.md), _ [endthread y _ endthreadex](../c-runtime-library/reference/endthread-endthreadex.md).
...
> [!NOTE]
> Si llama a rutinas en tiempo de ejecución de C desde un programa compilado con libcmt. lib, debe iniciar los subprocesos `_beginthreadex` con la `_beginthread` función o. No utilice las funciones `ExitThread` y `CreateThread` de Win32. La utilización de `SuspendThread` puede producir un interbloqueo cuando existe más de un subproceso bloqueado en espera de que el subproceso suspendido complete su acceso a una estructura de datos en tiempo de ejecución de C.

###  <a name="_core_the__beginthread_function"></a>Las funciones _ beginthread y _ beginthreadex
...
###  <a name="_core_the__endthread_function"></a>Las funciones _ endthread y _ endthreadex

La función _ [endthread](../c-runtime-library/reference/endthread-endthreadex.md) termina un subproceso creado `_beginthread` por (y, `_endthreadex` de forma similar, finaliza un `_beginthreadex`subproceso creado por). Los subprocesos terminan automáticamente cuando finalizan. `_endthread` y `_endthreadex` son útiles para la terminación condicional desde un subproceso. Por ejemplo, un subproceso dedicado a procesar las comunicaciones puede terminar si es incapaz de obtener el control del puerto de comunicaciones.
...
> [!NOTE]
> Para obtener una explicación similar desde el punto de vista de MFC [, vea multithreading: Sugerencias](multithreading-programming-tips.md) de programación [y multithreading: Cuándo usar las clases](multithreading-when-to-use-the-synchronization-classes.md)de sincronización.
...
El programa Bounce. c que se muestra en el [programa multiproceso de ejemplo](sample-multithread-c-program.md) de `ScreenMutex` c usa una exclusión mutua denominada para coordinar las actualizaciones de la pantalla. Cada vez que uno de los subprocesos de presentación está listo para escribir en la `WaitForSingleObject` pantalla, llama a `ScreenMutex` con el identificador y la constante infinita `WaitForSingleObject` para indicar que la llamada debe bloquearse en la exclusión mutua y no agotar el tiempo de espera. Si `ScreenMutex` es Clear, la función wait establece la exclusión mutua para que otros subprocesos no interfieran con la presentación y continúe ejecutando el subproceso. De lo contrario, el subproceso se bloquea hasta que la exclusión mutua se elimina. Cuando el subproceso completa la actualización de la pantalla, libera la exclusión mutua `ReleaseMutex`mediante una llamada a.

Las escrituras en pantalla y los datos estáticos son sólo dos de los recursos que requieren una cuidadosa administración. Por ejemplo, el programa puede contener varios subprocesos que tengan acceso al mismo archivo. Puesto que otro subproceso puede haber desplazado el puntero del archivo, cada subproceso debe restablecer ese puntero antes de leer o escribir. Además, cada subproceso debe asegurarse de que no se ha adelantado entre el momento en que coloca el puntero y el momento en que tiene acceso al archivo. Estos subprocesos deben usar un semáforo para coordinar el acceso al archivo entre corchetes en cada `WaitForSingleObject` acceso `ReleaseMutex` de archivo con llamadas y. El siguiente ejemplo de código muestra esta técnica:
...
El primer argumento de la `_beginthread` llamada es un puntero a la `BounceProc` función, que ejecuta los subprocesos. El segundo argumento especifica el tamaño de pila predeterminado para el subproceso. El último argumento es un número de identificación que se pasa `BounceProc`a. `BounceProc`usa el número de ID. para inicializar el generador de números aleatorios y seleccionar el atributo de color del subproceso y el carácter de presentación.
...
[Almacenamiento local para el subproceso (TLS)](thread-local-storage-tls.md)\
