---
title: 'Multithreading: uso de las clases de sincronización de MFC'
ms.date: 08/27/2018
helpviewer_keywords:
...
ms.locfileid: "77140505"
---
# <a name="multithreading-how-to-use-the-mfc-synchronization-classes"></a>Multithreading: uso de las clases de sincronización de MFC
...
Para hacer que una clase sea completamente segura para la ejecución de subprocesos, primero agregue la clase de sincronización apropiada a las clases compartidas como un miembro de datos. En el ejemplo anterior de administración de cuentas, se agregaría un miembro de datos `CSemaphore` a la clase de vista, un miembro de datos `CCriticalSection` se agregaría a la clase de lista vinculada y se agregaría un miembro de datos `CEvent` a la clase de almacenamiento de datos.
...
Para obtener información sobre cómo determinar la clase de sincronización que se va a usar en situaciones diferentes, vea [multithreading: Cuándo utilizar las clases de sincronización](multithreading-when-to-use-the-synchronization-classes.md). Para obtener más información acerca de la sincronización, consulte [Synchronization](/windows/win32/Sync/synchronization) in the Windows SDK. Para obtener más información acerca de la compatibilidad con multithreading en MFC, vea [multithreading con C++ y MFC](multithreading-with-cpp-and-mfc.md).
