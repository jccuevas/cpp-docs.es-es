...
La siguiente función, `ProcessImage`, llama al objeto [STD:: function](../../standard-library/function-class.md) determinado para transformar el valor de color de cada píxel en un objeto de [mapa de bits](/windows/win32/api/gdiplusheaders/nl-gdiplusheaders-bitmap) GDI+. La `ProcessImage` función usa el algoritmo [Concurrency::p arallel_for](reference/concurrency-namespace-functions.md#parallel_for) para procesar cada fila del mapa de bits en paralelo.

[!code-cpp[concrt-image-processing-filter#3](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-image-processing-network_2.cpp)]

Las siguientes funciones, `Grayscale` `Sepiatone` `Darken` `ProcessImage` ,, y, llaman a la función para transformar el valor de color de cada píxel de `Bitmap` un objeto. `ColorMask` Cada una de estas funciones usa una expresión lambda para definir la transformación de color de un píxel.

[!code-cpp[concrt-image-processing-filter#4](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-image-processing-network_3.cpp)]

La siguiente función, `GetColorDominance`, también llama a `ProcessImage` la función. Sin embargo, en lugar de cambiar el valor de cada color, esta función usa objetos [Concurrency:: combinable](../../parallel/concrt/reference/combinable-class.md) para calcular si el componente de color rojo, verde o azul domina la imagen.
...
1. En la `ProcessImages` función, cree una `countdown_event` variable. La `countdown_event` clase se muestra más adelante en este tutorial.

   [!code-cpp[concrt-image-processing-filter#8](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-image-processing-network_7.cpp)]

1. Cree un objeto [STD:: Map](../../standard-library/map-class.md) que asocie un `Bitmap` objeto con su nombre de archivo original.
...
1. Espere a que `countdown_event` la variable llegue a cero.
...
|`darken`|`transformer` Objeto que oscurece las imágenes que tienen el rojo como color dominante.|
|`sepiatone`|`transformer` Objeto que aplica tonos sepia a imágenes no creadas por Tom y que no están principalmente en rojo.|
|`save_bitmap`|Objeto que guarda el procesado `image` en el disco como un mapa de bits. `transformer` `save_bitmap`Recupera el nombre de archivo original del `map` objeto y cambia su extensión de nombre de archivo a. bmp.|
|`delete_bitmap`|`transformer` Objeto que libera la memoria de las imágenes.|
|`decrement`|Un objeto [Concurrency:: Call](../../parallel/concrt/reference/call-class.md) que actúa como el nodo terminal de la red. Disminuye el `countdown_event` objeto para indicar a la aplicación principal que se ha procesado una imagen.|

El `loaded_bitmaps` búfer de mensajes es importante porque, como `unbounded_buffer` un objeto, ofrece `Bitmap` objetos a varios receptores. Cuando un bloque de destino acepta `Bitmap` un objeto, `unbounded_buffer` el objeto no ofrece ese `Bitmap` objeto a ningún otro destino. Por lo tanto, es importante el orden en el que `unbounded_buffer` se vinculan los objetos a un objeto. Los `grayscale`bloques `colormask`de mensajes `sepiatone` , y usan un filtro para aceptar solo determinados `Bitmap` objetos. El `decrement` `Bitmap` búfer`loaded_bitmaps` de mensajes es un objetivo importante del búfer de mensajes porque acepta todos los objetos rechazados por otros búferes de mensajes. Un `unbounded_buffer` objeto es necesario para propagar los mensajes en orden. Por lo tanto `unbounded_buffer` , un objeto se bloquea hasta que se vincula un nuevo bloque de destino y acepta el mensaje si ningún bloque de destino actual acepta ese mensaje.

Si su aplicación requiere que varios bloques de mensajes procesen el mensaje, en lugar de simplemente el bloque de mensajes que primero acepta el mensaje, puede utilizar otro tipo de bloque de `overwrite_buffer`mensajes, como. La `overwrite_buffer` clase contiene un mensaje cada vez, pero propaga ese mensaje a cada uno de sus destinos.
...
El `countdown_event` objeto de este ejemplo habilita la red de procesamiento de imágenes para informar a la aplicación principal cuando se han procesado todas las imágenes. La `countdown_event` clase usa un objeto [Concurrency:: Event](../../parallel/concrt/reference/event-class.md) para indicar cuando un valor de contador llega a cero. La aplicación principal incrementa el contador cada vez que envía un nombre de archivo a la red. El nodo terminal de la red disminuye el contador después de procesar cada imagen. Una vez que la aplicación principal atraviesa el directorio especificado, espera a que el `countdown_event` objeto señale que su contador ha alcanzado cero.

En el ejemplo siguiente se `countdown_event` muestra la clase:
...
En el código siguiente se muestra el ejemplo completo. La `wmain` función administra la biblioteca GDI+ y llama a `ProcessImages` la función para procesar los archivos JPEG en `Sample Pictures` el directorio.
...
![Salida de ejemplo para el ejemplo](../../parallel/concrt/media/concrt_imageout.png "Salida de ejemplo para el ejemplo")

`Lighthouse`es creado por Tom ALPHIN y, por tanto, se convierte a escala de grises. `Chrysanthemum`,, `Desert` y`Tulips` tienen rojo como color dominante y, por lo tanto, se quitan los componentes de color azul y verde y se oscurecen. `Koala` `Hydrangeas`, `Jellyfish` y`Penguins` coinciden con los criterios predeterminados y, por tanto, son sepia toned.
