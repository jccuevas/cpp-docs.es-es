...
En este documento se describe la mejor forma de usar eficazmente la biblioteca de patrones de procesamiento paralelos (PPL). PPL proporciona algoritmos, objetos y contenedores de propósito general para realizar paralelismos específicos.  
 ...
 Para obtener más información acerca de la biblioteca PPL, vea [Parallel Patterns Library (PPL)](../../parallel/concrt/parallel-patterns-library-ppl.md).  
  
##  <a name="top"></a> Secciones  
 Este documento contiene las siguientes secciones:  
  
- [No establezca paralelismos en cuerpos de bucle pequeños](#small-loops)  
  
- [Exprese el paralelismo en el nivel más alto posible](#highest)  
  
- [Usar parallel_invoke para resolver problemas de Divide y vencerás](#divide-and-conquer)  
  
- [Use la cancelación o control para interrumpir un bucle paralelo de excepciones](#breaking-loops)  
...
- [No se escriben a los datos compartidos en un bucle paralelo](#shared-writes)  
...
##  <a name="highest"></a> Exprese el paralelismo en el nivel más alto posible  
 ...
 Dado que cada iteración del bucle es independiente, puede paralelizar buena parte del trabajo, como se muestra en el ejemplo siguiente. Este ejemplo se utiliza la [Concurrency:: parallel_for](reference/concurrency-namespace-functions.md#parallel_for) algoritmo en paralelo para paralelizar el bucle exterior.  
...
##  <a name="breaking-loops"></a> Use la cancelación o control para interrumpir un bucle paralelo de excepciones  
 ...
 Para obtener información general sobre la cancelación y mecanismos de control de excepciones que se proporcionan con la biblioteca PPL, vea [cancelación en la biblioteca PPL](cancellation-in-the-ppl.md) y [Exception Handling](../../parallel/concrt/exception-handling-in-the-concurrency-runtime.md).  
...
##  <a name="object-destruction"></a> Comprender cómo afectan cancelación y control de excepciones a la destrucción de objetos  
 ...
 Aunque este patrón no tiene ningún problema por sí mismo, considere el siguiente código, que ejecuta dos tareas en paralelo. La primera tarea crea un objeto `Container` y la segunda tarea cancela la tarea global. A modo de ilustración, el ejemplo utiliza dos [Concurrency:: Event](../../parallel/concrt/reference/event-class.md) objetos para asegurarse de que la cancelación se produce después de la `Container` se crea el objeto y que la `Container` objeto se destruye después de la cancelación se produce la operación.  
...
-   La cancelación de la tarea principal hace que la tarea secundaria, la llamada a [Concurrency:: parallel_invoke](reference/concurrency-namespace-functions.md#parallel_invoke), que también se puede cancelar. Por lo tanto, estos dos recursos no se liberan.  
...
##  <a name="shared-writes"></a> No se escriben a los datos compartidos en un bucle paralelo  
 ...
 Para obtener la versión completa de este ejemplo, vea [Cómo: usar la clase combinable para mejorar el rendimiento](../../parallel/concrt/how-to-use-combinable-to-improve-performance.md). Para obtener más información sobre la `combinable` de clases, consulte [contenedores y objetos paralelos](../../parallel/concrt/parallel-containers-and-objects.md).  
 ...
 *Uso compartido falso* se produce cuando varias tareas simultáneas que se ejecutan en procesadores separados escriben en variables que se encuentran en la misma línea de caché. Cuando una tarea escribe en una de las variables, se invalida la línea de caché de las dos variables. Cada procesador debe volver a cargar la línea de caché cada vez que esta se invalida. Por lo tanto, el uso compartido falso puede causar una disminución del rendimiento de la aplicación.  
 ...
 Se recomienda que realice la [Concurrency:: combinable](../../parallel/concrt/reference/combinable-class.md) clase cuando sea necesario compartir datos entre tareas. La clase `combinable` crea variables locales para los subprocesos de tal manera que es menos probable que se dé un uso compartido falso. Para obtener más información sobre la `combinable` de clases, consulte [contenedores y objetos paralelos](../../parallel/concrt/parallel-containers-and-objects.md).  
...
##  <a name="lifetime"></a> Asegúrese de que las Variables son válidas durante toda la duración de una tarea  
 ...
 En el ejemplo siguiente se usa el [concurrency::task_group::wait](reference/task-group-class.md#wait) método para asegurarse de que la tarea finalice antes de la `perform_action` función devuelve.  
