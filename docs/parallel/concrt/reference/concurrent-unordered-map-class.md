...
|Nombre|Descripción|  
|----------|-----------------|  
|[concurrent_unordered_map (Constructor)](#ctor)|Sobrecargado. Construye un mapa sin ordenar simultáneo.|  
  
### <a name="public-methods"></a>Métodos públicos  
  
|Nombre|Descripción|  
|----------|-----------------|  
|[AT (método)](#at)|Sobrecargado. Busca un elemento en `concurrent_unordered_map` con un valor de clave especificado. Este método es seguro para simultaneidad.|  
|[hash_function (método)](#hash_function)|Obtiene el objeto de función hash almacenado.|  
|[Insert (método)](#insert)|Sobrecargado. Agrega elementos al objeto `concurrent_unordered_map`.|  
|[key_eq (método)](#key_eq)|Obtiene el objeto de función de comparación de igualdad almacenado.|  
|[swap (método)](#swap)|Intercambia el contenido de dos objetos `concurrent_unordered_map`. Este método no es seguro para la simultaneidad.|  
|[unsafe_erase (método)](#unsafe_erase)|Sobrecargado. Quita los elementos de `concurrent_unordered_map` en las posiciones especificadas. Este método no es seguro para la simultaneidad.|  
  
### <a name="public-operators"></a>Operadores públicos  
  
|Nombre|Descripción|  
|----------|-----------------|  
|[operator [] (operador)](#operator_at)|Sobrecargado. Busca o inserta un elemento con la clave especificada. Este método es seguro para simultaneidad.|  
|[operador = (operador)](#operator_eq)|Sobrecargado. Asigna el contenido de otro objeto `concurrent_unordered_map` a este. Este método no es seguro para la simultaneidad.|  
  
## <a name="remarks"></a>Comentarios  
 Para obtener información detallada sobre la `concurrent_unordered_map` de clases, consulte [objetos y contenedores paralelos](../../../parallel/concrt/parallel-containers-and-objects.md).  
...
##  <a name="a-namectora-concurrentunorderedmap"></a><a name="ctor"></a>concurrent_unordered_map) 
 ...
 Construye un mapa sin ordenar simultáneo.  
...
### <a name="parameters"></a>Parámetros  
 ...
 `_Hasher`  
 La función de hash para esta asignación no ordenada.  
...
### <a name="remarks"></a>Comentarios  
 Todos los constructores almacenan un objeto de asignador `_Allocator` e inicializar el mapa sin ordenar.  
  
 El primer constructor especifica una asignación inicial vacía y especifica explícitamente el número de depósitos, escriba función hash, función de igualdad y el asignador para usarse.  
  
 El segundo constructor especifica un asignador para el mapa sin ordenar.  
 ...
 El último constructor especifica un movimiento de la asignación no ordenada simultánea `_Umap`.  
...
### <a name="remarks"></a>Comentarios  
 La primera función miembro determina si existe un elemento X en la secuencia cuya clave tiene una ordenación equivalente a la de `value`. Si no, crea un elemento X y la inicializa con `value`. A continuación, la función determina el iterador `where` que designa X. Si se ha producido una inserción, la función devuelve `std::pair(where, true)`. De lo contrario, devuelve `std::pair(where, false)`.  
...
### <a name="remarks"></a>Comentarios  
 ...
 `operator[]`puede utilizarse para insertar elementos en un mapa `m` con `m[key] = DataValue;`, donde `DataValue` es el valor de la `mapped_type` del elemento con el valor de clave `key`.  
...
### <a name="parameters"></a>Parámetros  
 `_Umap`  
 La `concurrent_unordered_map` objeto intercambiar con.  
