---
title: task (Clase) (Runtime de simultaneidad)
ms.date: 07/30/2019
f1_keywords:
- task
...
ms.locfileid: "77142613"
---
# <a name="task-class-concurrency-runtime"></a>task (Clase) (Runtime de simultaneidad)
...
|[aquél](#scheduler)|Devuelve el programador para esta tarea|
|[then](#then)|Sobrecargado. Agrega una tarea de continuación a esta tarea.|
|[currir](#wait)|Espera que esta tarea alcance un estado terminal. Es posible que `wait` ejecute la tarea alineada, si se cumplen todas las dependencias de tareas, y todavía no se ha detectado para la ejecución de un trabajador en segundo plano.|
...
> [!IMPORTANT]
> En una aplicación Plataforma universal de Windows (UWP), no llame a [Concurrency:: Task:: wait](#wait) o `get` (`wait` llama a `get`) en el código que se ejecuta en el subproceso de la interfaz de usuario. De lo contrario, el Runtime produce [Concurrency:: invalid_operation](invalid-operation-class.md) porque estos métodos bloquean el subproceso actual y pueden provocar que la aplicación deje de responder. Sin embargo, puede llamar al método `get` para recibir el resultado de la tarea antecedente en una continuación basada en tareas porque el resultado está disponible inmediatamente.
...
**true** si la tarea desencapsula una interfaz de `IAsyncInfo` o si desciende de esta tarea; de lo contrario, **false** .
...
*_Param*<br/>
Parámetro desde el que se va a construir la tarea. Puede ser una expresión lambda, un objeto de función, un `task_completion_event<result_type>` objeto o Windows:: Foundation:: IAsyncInfo si usa tareas en la aplicación Windows Runtime. El objeto de función o expresión lambda debe ser un tipo equivalente a `std::function<X(void)>`, donde X puede ser una variable de tipo `result_type`, `task<result_type>`o un Windows:: Foundation:: IAsyncInfo en Windows Runtime aplicaciones.
...
Las tareas que se crean a partir de la interfaz `Windows::Foundation::IAsyncInfo` o de una expresión lambda que devuelve una interfaz `IAsyncInfo` alcanzan su estado de terminal cuando se completa la operación o acción asincrónica insertada en Windows Runtime. Del mismo modo, las tareas creadas a partir de una expresión lambda que devuelve un `task<result_type>` alcanzar su estado de terminal cuando la tarea interna alcanza su estado de terminal y no cuando la expresión lambda vuelve.
...
Variable que especifica dónde debe ejecutarse la continuación. Esta variable solo es útil cuando se usa en una aplicación de UWP. Para obtener más información, vea [task_continuation_context](task-continuation-context-class.md)
