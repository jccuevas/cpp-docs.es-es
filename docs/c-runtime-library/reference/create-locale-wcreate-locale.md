...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "__create_locale (función)"
  - "_create_locale (función)"
  - "create_locale (función)"
  - "configuraciones regionales, crear"
...
## Comentarios  
 La función `_create_locale` permite crear un objeto que represente ciertos valores específicos de la región, para su uso en versiones específicas de la configuración regional de muchas funciones de CRT \(funciones con el sufijo `_l`\).  El comportamiento es parecido al de `setlocale`, salvo que en lugar de aplicar la configuración regional especificada al entorno actual, la configuración se guarda en una estructura de `_locale_t` que se devuelve.  La estructura de `_locale_t` se debe liberar mediante [\_free\_locale](../../c-runtime-library/reference/free-locale.md) cuando ya no se necesita.  
