...
dev_langs: 
  ...
  - "C"
helpviewer_keywords: 
  - "abrir archivos, para E/S de archivos"
  - "wfopen (función)"
  - "tfopen (función)"
  - "_tfopen (función)"
  - "_wfopen (función)"
  - "apertura de archivos [C++]"
  - "fopen (función)"
...
Abre un archivo. Existen versiones más seguras de estas funciones que realizan una validación adicional de los parámetros y devuelven códigos de error: consulte [fopen\_s, \_wfopen\_s](../../c-runtime-library/reference/fopen-s-wfopen-s.md).  
...
|`_tfopen`|`fopen`|`fopen`|`_wfopen`|  
 ...
 Para más información sobre cómo utilizar los modos binario y de texto en Unicode y la E\/S de flujo multibyte, vea [E\/S de archivo en modo texto y en modo binario](../../c-runtime-library/text-and-binary-mode-file-i-o.md) y [E\/S de secuencias Unicode en los modos binario y de texto](../../c-runtime-library/unicode-stream-i-o-in-text-and-binary-modes.md).  
...
|`_wfopen`|\<stdio.h\> o \<wchar.h\>|  
 ...
 `_wfopen` es una extensión de Microsoft. Para más información sobre la compatibilidad, vea [Compatibilidad](../../c-runtime-library/compatibility.md).  
  
 Las opciones `c`, `n`, `t`, `S`, `R`, `T` y `D` `mode` son extensiones de Microsoft para `fopen` y `_fdopen` y no deben usarse si desea usarse la portabilidad ANSI.  
...
```Output  
El archivo 'crt_fopen.c' estaba abierto El archivo 'data2' estaba abierto Número de archivos cerrados por _fcloseall: 1  
```  
  
## Ejemplo  
 El siguiente programa crea un archivo \(o sobrescribe uno si existe\), en el modo de texto con codificación Unicode.  A continuación, escribe dos cadenas en el archivo y cierra el archivo. El resultado es un archivo denominado \_wfopen\_test.xml, que contiene los datos de la sección de salida.  
