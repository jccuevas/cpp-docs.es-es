...
|El búfer de destino es demasiado pequeño para contener la cadena convertida (a menos que el valor de *Count* sea **_TRUNCATE**; para obtener más información, vea la sección comentarios).|**ERANGE**|
...
Si *Count* es el valor especial [_TRUNCATE](../../c-runtime-library/truncate.md), **mbsrtowcs_s** convierte la gran parte de la cadena que quepa en el búfer de destino, a la vez que deja espacio para un terminador null.
...
Si *mbstate* es un puntero nulo, se utiliza el objeto estático interno de la biblioteca **mbstate_t** de estado de conversión. Dado que este objeto estático interno no es seguro para subprocesos, se recomienda pasar su propio valor de *mbstate* .
