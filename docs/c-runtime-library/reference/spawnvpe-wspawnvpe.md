...
## <a name="remarks"></a>Comentarios  
 ...
 Estas funciones validan sus parámetros. Si `cmdname` o `argv` es un puntero nulo, si `argv` señala a un puntero nulo o si `argv[0]` es una cadena vacía, se invoca al controlador de parámetros no válidos, como se describe en [Validación de parámetros](../../c-runtime-library/parameter-validation.md). Si la ejecución puede continuar, estas funciones establecen `errno` en `EINVAL`y devuelven -1. No se genera ningún proceso nuevo.  
