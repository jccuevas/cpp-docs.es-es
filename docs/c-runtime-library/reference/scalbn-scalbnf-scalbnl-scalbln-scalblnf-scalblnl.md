...
Multiplica un número de punto flotante por una potencia integral de FLT\_RADIX.  
...
## Valor devuelto  
 Si son correctas, las funciones `scalbn` devuelven el valor de `x` \* `FLT_RADIX`<sup>exp</sup>.  En caso de desbordamiento, y según cuál sea el signo de `x`, `scalbn` devuelve \+\/– `HUGE_VAL`; el valor de `errno` se establece en `ERANGE`.  
