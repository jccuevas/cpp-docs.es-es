...
helpviewer_keywords: 
  ...
  - "_tfsopen (función)"
  - "_wfsopen (función)"
  - "uso compartido de archivos [C++]"
  - "archivos [C++], abrir"
  - "fsopen (función)"
  - "abrir archivos, secuencias"
  - "tfsopen (función)"
  - "wfsopen (función)"
...
|`D`|Especifica un archivo como temporal.  Se elimina cuando se cierra el puntero del último archivo.|  
 ...
 Si no se especifica `t` o `b` en `mode`, el modo de traducción está definido por la variable de modo predeterminado `_fmode`.  Si se agrega `t` o `b` como prefijo al argumento, se produce un error en la función y devuelve `NULL`.  Para obtener información sobre los modos de texto y binario, vea [E\/S de archivo de modo de texto y binario](../../c-runtime-library/text-and-binary-mode-file-i-o.md).  
...
```  
  ...
  **Nadie más en la red puede escribir en este archivo hasta que finalicemos.**   
