...
ms.locfileid: "81350485"
---
# <a name="asctime-_wasctime"></a>asctime, _wasctime

Convierta una estructura de tiempo **tm** en una cadena de caracteres. Hay disponibles versiones más seguras de estas funciones; consulte [asctime_s, _wasctime_s](asctime-s-wasctime-s.md).
...
**asctime** devuelve un puntero al resultado de la cadena de caracteres; **_wasctime** devuelve un puntero al resultado de cadena de caracteres anchos. No hay ningún error de valor devuelto.

## <a name="remarks"></a>Observaciones

Hay disponibles versiones más seguras de estas funciones; consulte [asctime_s, _wasctime_s](asctime-s-wasctime-s.md).

La función **asctime** convierte un tiempo almacenado como una estructura en una cadena de caracteres. El valor *timeptr* normalmente se obtiene de una llamada a **gmtime** o **localtime**, que devuelven un puntero a una estructura **tm,** definida en TIME. H.
...
El resultado de la cadena generado por **asctime** `Wed Jan 02 02:03:55 1980\n\0`contiene exactamente 26 caracteres y tiene el formato . Se usa un reloj de 24 horas. Todos los campos tienen un ancho constante. El carácter de nueva línea y el carácter nulo ocupan las dos últimas posiciones de la cadena. **asctime** utiliza un único búfer asignado estáticamente para contener la cadena de retorno. Cada llamada a esta función destruye el resultado de la llamada anterior.

**_wasctime** es una versión de caracteres anchos de **asctime**. **_wasctime** y **asctime** se comportan de forma idéntica de lo contrario.

Estas funciones validan sus parámetros. Si *timeptr* es un puntero nulo, o si contiene valores fuera de rango, se invoca el controlador de parámetros no válidos, como se describe en [validación](../../c-runtime-library/parameter-validation.md)de parámetros . Si la ejecución puede continuar, la función devuelve **NULL** y establece **errno** en **EINVAL**.
...
Este programa coloca la hora del sistema en el entero largo **aclock**, lo traduce en la estructura **newtime** y, a continuación, lo convierte en forma de cadena para la salida, utilizando la función **asctime.**
