...
Anexa caracteres de una cadena. Hay disponibles versiones más seguras de estas funciones; vea [strncat_s, _strncat_s_l, wcsncat_s, _wcsncat_s_l, _mbsncat_s, _mbsncat_s_l](../../c-runtime-library/reference/strncat-s-strncat-s-l-wcsncat-s-wcsncat-s-l-mbsncat-s-mbsncat-s-l.md).  
  
> [!IMPORTANT]
>  `_mbsncat` y `_mbsncat_l` no se pueden usar en aplicaciones que se ejecutan en Windows en tiempo de ejecución. Para más información, vea [Funciones de CRT no admitidas con /ZW](http://msdn.microsoft.com/library/windows/apps/jj606124.aspx).  
...
> [!IMPORTANT]
>  `strncat` no comprueba si espacio suficiente en `strDest`, por lo que podría producirse un problema de saturaciones del búfer. Tenga presente que `count` limita el número de caracteres que se anexan, no el tamaño de `strDest`. Vea el ejemplo siguiente. Para obtener más información, vea [Avoiding Buffer Overruns](http://msdn.microsoft.com/library/windows/desktop/ms717795)(Evitar saturaciones del búfer).  
