...
> [!IMPORTANT]
> **_mbsncpy_s** y **_mbsncpy_s_l** no se pueden usar en aplicaciones que se ejecutan en Windows Runtime. Para obtener más información, vea [Funciones de CRT no admitidas en aplicaciones de la Plataforma universal de Windows](../../cppcx/crt-functions-not-supported-in-universal-windows-platform-apps.md).
...
Cero si se realiza correctamente, **STRUNCATE** si se produjo un truncamiento, de lo contrario se produce un código de error.
...
|cualquiera|cualquiera|**Null**|**EINVAL**|*strDest*[0] establecido en 0|
|cualquiera|0|cualquiera|**EINVAL**|no modificado|
|no **NULL**|demasiado pequeño|cualquiera|**ERANGE**|*strDest*[0] establecido en 0|

## <a name="remarks"></a>Observaciones

Estas funciones intentan copiar los primeros caracteres *D* de *strSource* en *strDest*, donde *D* es el menor de *count* y la longitud de *strSource*. Si esos caracteres *D* caben dentro de *strDest* (cuyo tamaño se da como *numberOfElements*) y todavía dejan espacio para un terminador nulo, se copian esos caracteres y se anexa un null de terminación; de lo contrario, *strDest*[0] se establece en el carácter nulo y se invoca el controlador de parámetros no válidos, como se describe en validación de [parámetros](../../c-runtime-library/parameter-validation.md).

Existe una excepción al comportamiento anterior. Si *count* se **_TRUNCATE**, entonces tanto de *strSource* como caben en *strDest* se copia mientras que todavía deja espacio para el null de terminación que siempre se anexa.
...
significa que estamos pidiendo **a strncpy_s** que copie cinco caracteres en un búfer de cinco bytes de largo; esto no dejaría espacio para el terminador nulo, por lo **tanto, strncpy_s** pone a cero la cadena y llama al controlador de parámetros no válidos.

Si se necesita un comportamiento de truncamiento, utilice **_TRUNCATE** o (*tamaño* - 1):
...
Tenga en cuenta que, a diferencia de **strncpy**, si *count* es mayor que la longitud de *strSource*, la cadena de destino NO se rellena con caracteres nulos hasta el *recuento*de longitudes.
...
> [!NOTE]
> **_strncpy_s_l,** **_wcsncpy_s_l** y **_mbsncpy_s_l** no tienen dependencia local y se proporcionan solo para **_tcsncpy_s_l** y no están destinados a ser llamados directamente.
