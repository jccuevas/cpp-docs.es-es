...
helpviewer_keywords: 
  ...
  - "tfopen_s (función)"
  - "Unicode [C++], crear archivos"
  - "Unicode [C++], archivos"
  - "Unicode [C++], escribir archivos"
  - "wfopen_s (función)"
...
Abre un archivo.  Estas versiones de [fopen, \_wfopen](../../c-runtime-library/reference/fopen-wfopen.md) incluyen mejoras de seguridad, tal como se describe en [Características de seguridad de CRT](../../c-runtime-library/security-features-in-the-crt.md).  
...
## Valor devuelto  
 Devuelve cero si se ejecuta correctamente; devuelve un código de error si se produce un error.  Consulte [errno, \_doserrno, \_sys\_errlist y \_sys\_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md) para obtener más información sobre estos códigos de error.  
...
## Compatibilidad con Unicode  
 ...
 Los valores permitidos de `encoding` son `UNICODE`, `UTF-8` y `UTF-16LE`.  Si no se especifica ningún valor para `encoding`, `fopen_s` usa la codificación ANSI.  
...
|`_tfopen_s`|`fopen_s`|`fopen_s`|`_wfopen_s`|  
 ...
 Para obtener más información sobre el uso de los modos de texto y binario en E\/S de flujos Unicode y multibyte, vea [E\/S de archivo de modo de texto y binario](../../c-runtime-library/text-and-binary-mode-file-i-o.md) y [E\/S de flujos Unicode en los modos de texto y binario](../../c-runtime-library/unicode-stream-i-o-in-text-and-binary-modes.md).  
 ...
 Los caracteres válidos para la cadena `mode` que se usa en `fopen_s` y `_fdopen` corresponden a los argumentos `oflag` que se usan en [\_open](../../c-runtime-library/reference/open-wopen.md) y [\_sopen](../../c-runtime-library/reference/sopen-wsopen.md), como se indica a continuación:  
...
## Bibliotecas  
 ...
 Las opciones de `c`, `n` y `t` `mode` son extensiones de Microsoft para `fopen_s` y `_fdopen`, y no deben usarse si desea usarse la portabilidad ANSI.  
...
```  
  ...
  **Se ha abierto el archivo 'crt\_fopen\_s.c'.**  
**El archivo 'data2' estaba abierto**  
**Número de archivos cerrados por \_fcloseall: 1**   
