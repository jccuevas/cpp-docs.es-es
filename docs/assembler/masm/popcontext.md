---
title: POPCONTEXT | Microsoft Docs
ms.custom: ''
ms.date: 08/30/2018
ms.technology:
- cpp-masm
ms.topic: reference
f1_keywords:
- POPCONTEXT
dev_langs:
- C++
helpviewer_keywords:
- POPCONTEXT directive
ms.assetid: 19f59290-a54d-477d-88d8-97d3f63ed417
author: corob-msft
ms.author: corob
ms.workload:
- cplusplus
ms.openlocfilehash: 40263316747030baeba703c78c2d8fc9abccd504
ms.sourcegitcommit: a7046aac86f1c83faba1088c80698474e25fe7c3
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 09/04/2018
ms.locfileid: "43691083"
---
# <a name="popcontext"></a>POPCONTEXT

Restaura todo o parte del actual `context` (guardado por el [PUSHCONTEXT](../../assembler/masm/pushcontext.md) directiva). El `context` puede ser **asume**, `RADIX`, **enumerar**, **CPU**, o **todas**.

## <a name="syntax"></a>Sintaxis

> Contexto POPCONTEXT

## <a name="see-also"></a>Vea también

[Referencia de directivas](../../assembler/masm/directives-reference.md)<br/>