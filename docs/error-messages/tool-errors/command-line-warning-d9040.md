---
title: Advertencia de la línea de comandos D9040 | Documentos de Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-diagnostics
ms.topic: error-reference
f1_keywords:
- D9040
dev_langs:
- C++
helpviewer_keywords:
- D9040
ms.assetid: 415e7f04-c1bd-4ac1-924a-03efa4645140
author: corob-msft
ms.author: corob
ms.workload:
- cplusplus
ms.openlocfilehash: cc360d0bc3c1266dc6de2b8ff2a33e1e60944989
ms.sourcegitcommit: 76b7653ae443a2b8eb1186b789f8503609d6453e
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 05/04/2018
ms.locfileid: "33299931"
---
# <a name="command-line-warning-d9040"></a>Advertencia de la línea de comandos D9040
la opción '/analyze' se omitirá; las advertencias de análisis de código no están disponibles en esta edición del compilador  
  
 El **/ analyze** opción de línea de comandos no está disponible en todas las ediciones de Visual Studio. La solución es pasarse a una edición compatible de Visual Studio Team System o quitar la opción de línea de comandos.  
  
## <a name="see-also"></a>Vea también  
 [Errores de línea de comandos de D8000 a través de D9999](../../error-messages/tool-errors/command-line-errors-d8000-through-d9999.md)   
 [Opciones del compilador](../../build/reference/compiler-options.md)