---
title: Error del compilador de recursos RC2107 | Documentos de Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-diagnostics
ms.topic: error-reference
f1_keywords:
- RC2107
dev_langs:
- C++
helpviewer_keywords:
- RC2107
ms.assetid: f1786128-aa86-4cdb-a095-05a0b66b5608
author: corob-msft
ms.author: corob
ms.workload:
- cplusplus
ms.openlocfilehash: bf6f613b92260d92cf8e6fd55d617b2b6f2a214d
ms.sourcegitcommit: 76b7653ae443a2b8eb1186b789f8503609d6453e
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 05/04/2018
ms.locfileid: "33318433"
---
# <a name="resource-compiler-error-rc2107"></a>Error del compilador de recursos RC2107
**Se esperaba un valor de comando numérico**  
  
 El compilador de recursos esperaba un valor numérico *idvalue* campo el **ACELERADORES** instrucción. Asegúrese de que ha usado un `#define` constante para especificar el valor y que la constante se haya escrito correctamente.