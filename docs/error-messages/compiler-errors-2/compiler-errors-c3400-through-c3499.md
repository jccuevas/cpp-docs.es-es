---
title: Errores de compilador C3400 a C3499
ms.date: 11/17/2017
f1_keywords:
- C3401
- C3402
...
ms.locfileid: "50576727"
---
# <a name="compiler-errors-c3400-through-c3499"></a>Errores de compilador C3400 a C3499
...
|C3401 de Error del compilador|'*especificador*': especificador de acceso de ensamblado no válido; solo 'private' se permite en las plantillas de clase|
...
|C3410 de Error del compilador|'*identificador*': el tipo de la creación de instancias explícita '*tipo*'no coincide con el tipo de la plantilla variable'*tipo*'|
|C3411 de Error del compilador|'*tipo*' no es válido como el tamaño de una matriz que no sea un tipo entero|
...
|C3419 de Error del compilador|Obsoleto.|
...
|C3422 de Error del compilador|'*declaración*': los tipos no coincidentes*tipo*'y'*tipo*'|
|C3423 de Error del compilador|Obsoleto.|
|C3424 de Error del compilador|'*tipo*': no se permite una conversión a un tipo de matriz de estilo de función|
|C3425 de Error del compilador|no se puede iniciar un puntero al objeto de tipo incompleto '*tipo*'|
|C3426 de Error del compilador|no se puede producir el objeto de tipo incompleto '*tipo*'|
...
|C3431 de Error del compilador|'*identificador*': *type1* no se puede volver a declararse como *type2*|
...
|C3433 de Error del compilador|'*identificador*': todas las declaraciones de enumeración debe tener el mismo tipo subyacente, era '*type1*"ahora"*type2*'|
|C3434 de Error del compilador|'*contexto*': valor de enumerador '*número*'no se puede representar como'*tipo*', valor es'*número*'|
...
|C3439 de Error del compilador|layout_version (*número*): número de versión no válido|
|C3440 de Error del compilador|'*declaración*': layout_version (*número*) compatible con una declaración anterior|
|C3441 de Error del compilador|'*declaración*': '*palabra clave*' no se puede aplicar después de haber definido la clase|
|C3442 de Error del compilador|Inicializando varios miembros de unión: '*member1*'y'*member2*'|
|C3443 de Error del compilador|El inicializador de miembro predeterminado para '*clase*' es recursivo|
|C3444 de Error del compilador|Vacía la clase de agregado*clase*'debe inicializarse con'{}'|
...
|C3471 de Error del compilador|nombre del nuevo módulo *nombre* (se configura en la línea de comandos) entra en conflicto con el nombre anterior *nombre*|
|C3472 de Error del compilador|nuevo nombre de archivo de salida *filename* (se configura en la línea de comandos) entra en conflicto con el nombre de archivo anterior *nombre de archivo*|
...
|C3478 de Error del compilador|'*identificador*': una matriz no se pueden capturar por copia|
...
|C3494 de Error del compilador|'' no se puede capturar explícitamente porque no permite un modo de captura envolvente|
...
|Compilador Error C3497|no se puede construir una instancia de una expresión lambda|
