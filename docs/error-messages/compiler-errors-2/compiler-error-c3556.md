...
En el ejemplo de código siguiente, el compilador no puede deducir el tipo del argumento `myFunction` porque `myFunction` está sobrecargado. Para corregir este problema, puede usar `static_cast` crear una instancia de un puntero a ese sobrecarga la función que se especifique en el `decltype` expresión.  
